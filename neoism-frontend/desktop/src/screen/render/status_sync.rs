        ...
        // since the strip is always visible.
        {
            ...
            let (status_mode, primary, primary_kind, branch, active_path, active_cwd) =
                if let Some(code) = current.code.as_ref() {
                    ...
                    let branch = branch_for(active_path.as_deref().or(cwd.as_deref()));
                    ...
                    let branch = branch_for(cwd.as_deref());
                    (
                    ...
                    let branch =
                        branch_for(Some(active_path.as_path()).or(cwd.as_deref()));
                    ...
                    let primary = terminal_agent
                        ...
                        .unwrap_or_default();
                    let branch = branch_for(cwd_buf.as_deref());
                    (
