...
pub(crate) async fn with_clipboard_authorization<F: std::future::Future>(
    authorized: bool,
    future: F,
) -> F::Output {
...
#[cfg(test)]
pub(crate) static TEST_REVOCATION_LOCK: tokio::sync::Mutex<()> =
    tokio::sync::Mutex::const_new(());
#[cfg(test)]
type TestBackend =
    Arc<dyn Fn(&str, Value) -> anyhow::Result<BuiltinMcpCallResult> + Send + Sync>;
#[cfg(test)]
tokio::task_local! { static TEST_BACKEND: TestBackend; }
#[cfg(test)]
pub(crate) async fn with_test_backend<F: std::future::Future>(
    backend: TestBackend,
    future: F,
) -> F::Output {
...
enum Action {
    ...
    Text {
        ...
        #[serde(default)]
        clipboard_policy: typing::ClipboardPolicy,
        #[serde(default)]
...
fn clipboard_effects(value: &mut Value, changed: bool, uncertain: bool) {
    ...
    if changed {
        value["clipboardChanged"] = json!(true);
    }
    if uncertain {
        value["clipboardMayHaveChanged"] = json!(true);
    }
...
fn input_timeout(
    ...
    let mut result = text(value);
    result.is_error = Some(true);
    result
}
fn dispatched(method: &str) -> BuiltinMcpCallResult {
    text(
        json!({"status":"dispatched_unverified","applicationVerified":false,"method":method,"note":"Native event acceptance is not application consumption. Verify the effect with a new observation; do not blindly retry."}),
    )
}
fn attach_timings(result: &mut BuiltinMcpCallResult, timing: latency::Snapshot) {
    let stages = timing
        .stages
        .into_iter()
        .map(|(name, (calls, micros))| {
            (name.to_owned(), json!({"calls":calls,"totalUs":micros}))
        })
...
fn text(value: Value) -> BuiltinMcpCallResult {
    BuiltinMcpCallResult {
        content: vec![BuiltinMcpContent::Text {
...
pub(crate) fn admit<T>(
    read_enabled_state: impl FnOnce() -> anyhow::Result<T>,
) -> anyhow::Result<(u64, T)> {
...
impl BuiltinMcpService for ComputerUse {
    ...
    fn tools(&self) -> Vec<BuiltinMcpTool> {
        let mut tools:Vec<BuiltinMcpTool> = [
            ("capabilities", "Report desktop prerequisites and browser attachment options. For an already-running browser, first list windows then use browser_attach on the selected target; otherwise use native screenshot/input immediately. DOM attachment is optional. Safe before approval and does not probe processes or sockets.", json!({"type":"object","properties":{},"additionalProperties":false})),
            ...
            ("input", "Control the HOST desktop, not a remote browser. Prefer browser_observe/browser_act for attached page elements; use this for pixels, browser chrome and native dialogs. One bounded action; use batch plus a final screenshot for known sequences. Pointer actions require a fresh screenshot frame and image-local coordinates; drag also needs to_x/to_y. type sends literal text (max 512 Unicode characters); use separate key actions for Enter/Tab. method:auto|keyboard|native never implicitly replaces the clipboard and rejects control characters. Linux-only method:paste requires clipboard_policy:replace and separate human consent; it permits LF/Tab and may execute commands. No automatic fallback/retry after partial input. Positive scroll amount means down/right, range -20..20. Native dispatch is not application verification.", json!({"type":"object","properties":{"target":{"type":"string","description":"Required foreground window token unless scope:desktop is explicitly selected. Not a sandbox."},"action":{"type":"string","enum":["move","click","drag","scroll","text","key"]},"frame":{"type":"string"},"x":{"type":"integer","minimum":0},"y":{"type":"integer","minimum":0},"to_x":{"type":"integer","minimum":0},"to_y":{"type":"integer","minimum":0},"button":{"type":"string","enum":["left","right","middle"]},"amount":{"type":"integer","minimum":-20,"maximum":20},"horizontal":{"type":"boolean"},"text":{"type":"string","maxLength":512,"description":"Literal Unicode only; no C0/C1 control characters including newline, CR or Tab. Use explicit key actions for Return/Tab."},"key":{"type":"string","description":"Case-insensitive named key or ASCII letter/digit; Linux also accepts unmodified ASCII punctuation. For underscore on US layouts use key '-' with shift. Super_L/super/meta tap the system modifier; Return=enter, Esc=escape. Use action:text only where literal text is supported."},"modifiers":{"type":"array","items":{"type":"string","description":"Case-insensitive: control/ctrl/Control_L, alt/option/Alt_L, shift/Shift_L, meta/super/Super_L/win/windows/logo/cmd/command."},"maxItems":4}},"required":["action"],"additionalProperties":false})),
            ("windows", "List native windows and explicit target tokens. Unchanged window identities and bounds retain tokens; changes revoke them. Bounds are not screenshot pixel coordinates. Foreground targeting is not isolation.", json!({"type":"object","properties":{},"additionalProperties":false})),
            ...
            ("batch", "Run 1..16 already-decided input actions serially, stopping on first failure/cancellation. Optional final display screenshot is returned as model image with frame token. Do not batch blind semantic decisions: observe between decisions. Example: known field click + control-a + type, with screenshot set, needs one call. Browser forms should prefer browser_act with an expectation. Total text <=512 characters. Failed action may be partially delivered.", json!({"type":"object","properties":{"settle_ms":{"type":"integer","minimum":0,"maximum":3000,"default":0,"description":"Final screenshot defaults to immediate capture (0); positive values opt into visual settling, not application readiness."},"actions":{"type":"array","minItems":1,"maxItems":16,"items":{"type":"object","description":"Same action object as input; no nested batches"}},"target":{"type":"string"},"screenshot":{"type":"string","description":"Display ID for final screenshot"}},"required":["actions"],"additionalProperties":false})),
        ...
        input.input_schema["properties"]["action"]["enum"] = json!([
            "move",
            "click",
            "drag",
            "scroll",
        ...
        input.input_schema["properties"]["clipboard_policy"] =
            json!({"type":"string","enum":["forbid","replace"],"default":"forbid"});
        ...
        tools
            ...
            .find(|t| t.name == "batch")
            .unwrap()
            .input_schema["properties"]["actions"]["items"] = action_schema;
        ...
        let crop_schema = json!({"type":"object","additionalProperties":false,"description":"Optional rectangle in native display-image pixels, BEFORE resizing. Returned frame maps crop-local image coordinates back to the display.","properties":{"x":{"type":"integer","minimum":0},"y":{"type":"integer","minimum":0},"width":{"type":"integer","minimum":1},"height":{"type":"integer","minimum":1}},"required":["x","y","width","height"]});
        for tool in &mut tools {
            if matches!(tool.name.as_str(), "screenshot" | "batch") {
                tool.input_schema["properties"]["crop"] = crop_schema.clone();
            }
        }
    ...
    fn call_tool(
        ...
        match tool {
            ...
            "capabilities" => Ok(text(json!({"backend":platform::NAME,"requirements":platform::REQUIREMENTS,"input":"not_probed","screenshots":"not_probed","sessionApprovalRequired":true,"actionTimeoutSeconds":10,"maxTextCharacters":MAX_TEXT,"browser":browser::setup_info(),"typesafe":typesafe::setup_info()}))),
    ...
    fn call_tool_authorized_async<'a>(
        ...
        Box::pin(async move {
            if tool == "stop" {
                return self.call_tool(directory, tool, arguments);
            }
            ...
            let epoch = revocation_generation.ok_or_else(|| {
                ServiceError::new("Missing computer-use admission generation")
            })?;
            let progress = Arc::new(AtomicUsize::new(0));
            let worker_progress = progress.clone();
            let batch_total = if tool == "batch" {
                arguments["actions"].as_array().map(Vec::len)
            } else {
                None
            };
            let effects = Arc::new(Mutex::new(input_pipeline::Effects::default()));
            let worker_effects = effects.clone();
            let clipboard_authorized =
                CLIPBOARD_AUTHORIZED.try_with(|v| *v).unwrap_or(false);
            let input_requested = tool == "input";
            let browser_action_requested = tool == "browser_act";
            ...
            match result {
                Ok(Ok((result,timing))) => {
                    let mut result=match result {
                        ...
                        Err(error) if browser_action_requested=>browser::unknown(format!("{error:#}")),
                ...
                Ok(Err(e)) if browser_action_requested => Ok(browser::unknown(format!("Browser worker failed: {e}"))),
                ...
                Err(_) if browser_action_requested => Ok(browser::unknown("Browser action timed out; further protocol requests cancelled, in-flight page action cannot be recalled")),
...
struct Check {
    ...
    effects: Arc<Mutex<input_pipeline::Effects>>,
    clipboard_authorized: bool,
    target: Option<windows::Window>,
...
impl Check {
    fn fast(&self) -> anyhow::Result<()> {
        latency::count("cancellation_checks", 1);
            ...
            !self.cancel.load(Ordering::SeqCst)
                ...
                && self.epoch == STOP.load(Ordering::SeqCst)
                && self.started.elapsed() < DEADLINE,
            "Computer operation stopped; partial input may have been delivered"
        );
        Ok(())
    }
    fn check(&self) -> anyhow::Result<()> {
        self.check_probe(|| {
            latency::measure("target_validation", || {
                if let Some(target) = &self.target {
                    windows::validate(target, self.require_foreground)?;
                }
                Ok(())
            })
        })
    ...
    fn check_probe(
        &self,
        probe: impl FnOnce() -> anyhow::Result<()>,
    ) -> anyhow::Result<()> {
        self.fast()?;
        probe()?;
...
fn run_worker<T>(
    ...
    cancel: Arc<AtomicBool>,
    dropped: Arc<AtomicBool>,
    backend: impl FnOnce(&Check) -> anyhow::Result<T>,
) -> anyhow::Result<T> {
...
fn serialized<T>(
    ...
    let _guard = match lock.try_lock() {
        ...
        Err(std::sync::TryLockError::WouldBlock) => {
            bail!("Computer is busy; active native operation has not returned")
        }
...
fn preflight_request(tool: &str, arguments: &Value, check: &Check) -> anyhow::Result<()> {
    if tool == "input" || tool == "batch" {
        let count = if tool == "batch" {
            arguments["actions"].as_array().map_or(0, Vec::len).min(16)
        } else {
            1
        };
        check
            ...
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .records = (0..count).map(input_pipeline::unplanned).collect();
    }
    match tool {
        "input" => {
            let mut value = arguments.clone();
            validate_input_scope(tool, &mut value)?;
        ...
        "batch" => {
            ...
            if let Err(error) = validate_batch(&batch) {
                let failed = batch.actions.iter().position(|v| {
                    serde_json::from_value::<Action>(v.clone())
                        .map_err(anyhow::Error::from)
                        .and_then(|a| validate(&a))
                        .is_err()
                });
                check
                    .effects
    ...
    fn clipboard(value: &Value) -> bool {
        value["action"] == "paste"
            || value["method"] == "paste"
            || value["clipboard_policy"] == "replace"
            || value["actions"]
                .as_array()
                .is_some_and(|a| a.iter().any(clipboard))
    }
    ensure!(
        !clipboard(arguments) || check.clipboard_authorized,
        "Clipboard replacement requires explicit human computer_clipboard permission"
    );
    Ok(())
}
...
fn execute(
    ...
    if tool == "capabilities" {
        ...
        capabilities["browser"] = browser::setup_info();
        capabilities["typesafe"] = typesafe::setup_info();
        check.check()?;
...
fn execute_locked(
    ...
    if matches!(
        ...
        return browser::execute(tool, arguments, check);
    }
    if tool == "windows" {
        ensure!(
            arguments.as_object().is_some_and(|v| v.is_empty()),
            "windows accepts an empty object only"
        );
    ...
    if tool == "focus" {
        ...
        *FRAME
            .lock()
            .map_err(|_| anyhow::anyhow!("Frame state poisoned"))? = None;
        check.check()?;
        windows::focus(&args.target, || check.check())?;
        return Ok(text(
            json!({"focused":true,"target":args.target,"binding":"explicit-per-call","note":"Foreground confirmed now, not reserved. Pass this target to every input/batch and screenshot. No implicit focus inheritance; never automatically refocus after user interference."}),
        ));
    }
    if tool == "wait" {
        #[derive(Deserialize)]
        ...
        let args: Args = serde_json::from_value(arguments)?;
            ...
            (100..=3000).contains(&args.timeout_ms),
            "Wait timeout must be 100..3000 ms"
        );
        ...
        loop {
            ...
            if windows::condition(&target, matches!(args.condition, Condition::Closed))? {
                check.check()?;
                return Ok(text(
                    json!({"matched":true,"elapsedMs":start.elapsed().as_millis(),"note":"Observed condition only; not evidence of application task completion"}),
                ));
            }
            if start.elapsed() >= Duration::from_millis(args.timeout_ms) {
                return Ok(text(json!({"matched":false,"timedOut":true})));
            }
            std::thread::sleep(Duration::from_millis(100));
        }
    }
    if tool == "batch" {
        return batch(arguments, check);
    }
    validate_input_scope(tool, &mut arguments)?;
    let target = arguments
        .as_object_mut()
        .and_then(|v| v.remove("target"))
        .filter(|v| !v.is_null())
        .map(|v| windows::resolve(v.as_str().context("target must be a window token")?))
        .transpose()?;
    let scoped = Check {
        progress: check.progress.clone(),
        effects: check.effects.clone(),
        clipboard_authorized: check.clipboard_authorized,
        cancel: check.cancel.clone(),
        dropped: check.dropped.clone(),
        epoch: check.epoch,
        started: check.started,
        target,
        require_foreground: true,
    };
    let check = if scoped.target.is_some() {
        &scoped
    } else {
        check
    };
    ...
    match tool {
        "screenshot" => {
            ...
            struct Args {
                display: String,
                crop: Option<capture::Region>,
                #[serde(default = "default_settle_ms")]
                settle_ms: u64,
            }
            let args: Args = serde_json::from_value(arguments)?;
            #[cfg(target_os = "linux")]
            let mut capture_session = latency::measure(
                "capture_setup",
                capture::ScopedLinuxCaptureSession::new,
            )?;
            #[cfg(target_os = "linux")]
            ...
            let display = displays
                .into_iter()
                .find(|d| d.id == args.display)
                .context("Unknown display; call capabilities again")?;
            ensure!(
                u64::from(display.width) * u64::from(display.height) <= 64_000_000,
                "Display exceeds capture limit"
            );
            ensure!(args.settle_ms <= 3000, "settle_ms must be 0..3000");
            #[cfg(target_os = "linux")]
            let capture_image =
                || latency::measure("capture", || capture_session.capture(&display));
            #[cfg(not(target_os = "linux"))]
            let capture_image =
                || latency::measure("capture", || platform::capture(&display));
            let (image, settling) = capture_settled_with_checks(
                args.settle_ms,
                || check.check(),
                || check.fast(),
                capture_image,
            ...
            #[cfg(not(target_os = "linux"))]
                ...
                platform::displays()?.contains(&display),
            ...
            let native_height = image.height();
                ...
                u64::from(native_width) * u64::from(native_height) <= 64_000_000,
                "Physical display exceeds capture limit"
            );
            let region = args.crop.unwrap_or(capture::Region {
                x: 0,
                y: 0,
                width: native_width,
                height: native_height,
            });
            region.validate(native_width, native_height)?;
            let image = if args.crop.is_some() {
            ...
            latency::count(
                "png_lossless_retries",
                u64::from(encoded.timings.lossless_retry),
            );
            latency::count("png_bytes", encoded.bytes.len() as u64);
            let current_target = check
                .target
                .as_ref()
                .map(|target| windows::validate(target, true))
                .transpose()?;
            let frame = Frame {
            ...
            check.check()?;
            let mut result = text(
                json!({"frame":frame.id,"display":frame.display,"imageWidth":frame.width,"imageHeight":frame.height,"crop":frame.region,"nativeWidth":native_width,"nativeHeight":native_height,"coordinates":"image pixels, origin top-left of crop; pass frame unchanged","expiresAfterSeconds":30,"targetWindow":frame.target,"settling":settling}),
            );
            *FRAME
                .lock()
                .map_err(|_| anyhow::anyhow!("Frame state poisoned"))? = Some(frame);
            let data = latency::measure("base64", || STANDARD.encode(encoded.bytes));
            result.content.push(BuiltinMcpContent::Image {
                data,
        ...
        "input" => {
            ...
            let displays = if needs_pointer(&action) {
                latency::measure("display_probe", platform::displays)?
            } else {
                Vec::new()
            };
            check.check()?;
...
fn pointer_action(
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let point = |id: &str, x, y| -> anyhow::Result<(Display, Point)> {
            ...
            let (x, y) = frame.region.point(x, y, frame.width, frame.height);
            Ok((
                frame.display,
                Point {
                    x,
        ...
        let (display, command) = match action {
            Action::Move { frame, x, y } => {
                let (d, p) = point(&frame, x, y)?;
                (d, Command::Move(p))
            }
            Action::Click {
                frame,
            ...
            Action::Drag {
                ...
                let (d, a) = point(&frame, x, y)?;
                let (_, b) = point(&frame, to_x, to_y)?;
                (d, Command::Drag(a, b))
            }
            Action::Scroll { amount, horizontal } => {
                ensure!(
                    displays.len() == 1,
                    "Wayland pointer control requires exactly one output"
                );
                (displays[0].clone(), Command::Scroll { amount, horizontal })
            }
            Action::Text { .. } | Action::Key { .. } | Action::Paste { .. } => {
                unreachable!("Linux keyboard dispatched above")
            }
        };
        linux_pointer::send(&display, command, || check.check())
            .inspect_err(|_| invalidate_frame())?;
    ...
    #[cfg(not(target_os = "linux"))]
    {
        ...
        match action {
                ...
                with_button(&mut input, Button::Left, |input| {
                    for step in 1..=20 {
                        ...
                        let axis = |a: i32, b: i32| {
                            (i64::from(a) + (i64::from(b) - i64::from(a)) * step / 20)
                                as i32
                        };
                        platform::move_pointer(
                            input,
                            axis(start.0, end.0),
                            axis(start.1, end.1),
                        )?;
            ...
            Action::Paste { .. } | Action::Text { .. } | Action::Key { .. } => {
                unreachable!("Typing and shortcuts execute only through prepared plans")
            }
        }
        check.check()?;
...
fn validate_input_scope(tool: &str, arguments: &mut Value) -> anyhow::Result<()> {
    ...
    let desktop = scope.as_ref().is_some_and(|s| s == "desktop");
        ...
        scope.is_none() || desktop,
        "Only explicit scope:desktop is supported; otherwise supply target"
    );
    let target = object.get("target");
    if desktop {
        ensure!(
            target.is_none(),
            "scope:desktop and target are mutually exclusive"
        );
...
impl Settling {
    fn observe(&mut self, elapsed: Duration, sample: Vec<u8>) -> bool {
        ...
        elapsed >= Duration::from_millis(300)
            && elapsed.saturating_sub(self.quiet_since) >= Duration::from_millis(250)
    }
}
#[cfg(test)]
fn capture_settled(
    ms: u64,
    check: impl FnMut() -> anyhow::Result<()>,
    capture: impl FnMut() -> anyhow::Result<image_rs::DynamicImage>,
) -> anyhow::Result<(image_rs::DynamicImage, Value)> {
    let check = std::cell::RefCell::new(check);
    capture_settled_with_checks(
        ms,
        || (*check.borrow_mut())(),
        || (*check.borrow_mut())(),
        capture,
...
fn capture_settled_with_checks(
    ms: u64,
    mut check: impl FnMut() -> anyhow::Result<()>,
    mut waiting: impl FnMut() -> anyhow::Result<()>,
    mut capture: impl FnMut() -> anyhow::Result<image_rs::DynamicImage>,
) -> anyhow::Result<(image_rs::DynamicImage, Value)> {
    ...
    if ms == 0 {
        ...
        return Ok((
            image,
            json!({"mode":"immediate","settled":false,"timedOut":false,"elapsedMs":start.elapsed().as_millis(),"captures":1,"samples":0,"note":"Immediate capture; no settling wait or application-readiness acknowledgement."}),
        ));
    }
    ...
    loop {
        ...
        check()?;
        captures += 1;
        let sample = latency::measure("settle_sample", || {
            image
                .resize_exact(64, 64, image_rs::imageops::FilterType::Triangle)
                .into_rgb8()
                .into_raw()
        });
        ...
        if stable || elapsed >= Duration::from_millis(ms) {
            return Ok((
                image,
                json!({"settled":ms!=0 && stable,"timedOut":ms!=0 && !stable,"elapsedMs":elapsed.as_millis(),"captures":captures,"note":"Visual stability only; not application readiness"}),
            ));
        }
        // Check cancellation at <=25ms while waiting between 100ms probes.
        let until =
            (start.elapsed() + Duration::from_millis(100)).min(Duration::from_millis(ms));
        while start.elapsed() < until {
            waiting()?;
            std::thread::sleep(
                Duration::from_millis(25).min(until.saturating_sub(start.elapsed())),
            );
        }
    }
}
...
fn validate_batch(args: &Batch) -> anyhow::Result<()> {
        ...
        args.crop.is_none() || args.screenshot.is_some(),
        "Batch crop requires a final screenshot"
    );
    if let Some(crop) = args.crop {
        crop.validate(u32::MAX, u32::MAX)?;
    }
    ensure!(args.settle_ms <= 3000, "settle_ms must be 0..3000");
    ensure!(
        !args.actions.is_empty() && args.actions.len() <= 16,
        "Batch requires 1..16 actions"
    );
    let mut chars = 0;
    for value in &args.actions {
        let action: Action = serde_json::from_value(value.clone())?;
        validate(&action)?;
        if let Action::Text { text, .. } | Action::Paste { text, .. } = action {
...
fn sequence(
    count: usize,
    mut step: impl FnMut(usize) -> anyhow::Result<()>,
) -> (usize, Option<String>) {
    for index in 0..count {
        if let Err(error) = step(index) {
            return (index, Some(format!("{error:#}")));
        }
    }
    (count, None)
}
fn batch(arguments: Value, check: &Check) -> anyhow::Result<BuiltinMcpCallResult> {
    ...
    let displays = if actions.iter().any(needs_pointer) {
        latency::measure("display_probe", platform::displays)?
    } else {
        Vec::new()
    };
    after_input_capture(
        || input_pipeline::run(actions, &displays, &scoped, true),
        || {
            args.screenshot.map(|display|scoped.check().and_then(|_|execute_locked("screenshot",json!({"display":display,"target":args.target,"crop":args.crop,"settle_ms":args.settle_ms}),&scoped)))
        },
    )
}
fn after_input_capture(
    input: impl FnOnce() -> anyhow::Result<BuiltinMcpCallResult>,
    capture: impl FnOnce() -> Option<anyhow::Result<BuiltinMcpCallResult>>,
) -> anyhow::Result<BuiltinMcpCallResult> {
        ...
        match image {
            ...
            Err(error) => {
                result.content.extend(
                    text(json!({"screenshotError":format!("{error:#}")})).content,
                );
                result.is_error = Some(true);
            }
        }
...
fn batch_with(
    arguments: Value,
    check: &Check,
    mut operation: impl FnMut(&str, Value, &Check) -> anyhow::Result<BuiltinMcpCallResult>,
) -> anyhow::Result<BuiltinMcpCallResult> {
    let args: Batch = serde_json::from_value(arguments)?;
    ...
    let (completed, error) = sequence(args.actions.len(), |index| {
        ...
        if paste {
            clipboard_uncertain = true;
        }
        let dispatched = native_boundary(|| operation("input", action, check))?;
        ensure!(
            dispatched.is_error != Some(true),
            "Native action returned an error; application effects are unknown"
        );
        if paste {
            clipboard_changed = true;
            clipboard_uncertain = false;
        }
        if let Some(progress) = &check.progress {
            progress.store(index + 1, Ordering::SeqCst);
        }
        Ok(())
    });
    ...
    if let Some(display) = args.screenshot {
        ...
        match check.check().and_then(|_|native_boundary(||operation("screenshot",json!({"display":display,"target":args.target,"crop":args.crop,"settle_ms":args.settle_ms}),check))) {
            Ok(image)=>result.content.extend(image.content),
...
#[cfg(any(test, not(target_os = "linux")))]
struct ButtonRelease<'a, M: Mouse> {
    input: &'a mut M,
    button: Button,
    released: bool,
}
    ...
    fn drop(&mut self) {
        if !self.released {
            let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                self.input.button(self.button, Direction::Release)
            }));
        }
    }
}
#[cfg(any(test, not(target_os = "linux")))]
fn with_button<M: Mouse>(
    input: &mut M,
    button: Button,
    action: impl FnOnce(&mut M) -> anyhow::Result<()>,
) -> anyhow::Result<()> {
    let mut guard = ButtonRelease {
        input,
        button,
        released: false,
    };
    ...
    match action {
        Action::Text {
            ...
            #[cfg(target_os = "linux")]
                ...
                *method != typing::Method::Native,
                "Native Unicode injection is unsupported on Linux"
            );
            #[cfg(not(target_os = "linux"))]
            ensure!(
                *method != typing::Method::Keyboard,
                "Forced layout keyboard typing is unsupported on this platform"
            );
            if *method == typing::Method::Paste {
                ensure!(
                    *clipboard_policy == typing::ClipboardPolicy::Replace,
                    "Paste requires explicit clipboard_policy:replace"
                );
                ensure!(
                    text.chars().count() <= MAX_TEXT && !text.contains('\0'),
                    "Paste must contain at most 512 Unicode characters and no NUL"
                );
                #[cfg(not(target_os = "linux"))]
                bail!("Clipboard paste is unsupported on this platform");
                return Ok(());
            }
        ...
        Action::Paste {
            ...
            #[cfg(not(target_os = "linux"))]
            bail!(
                "Explicit clipboard paste is currently supported only on Linux Wayland"
            );
        }
        Action::Scroll { amount, .. } => {
            ensure!((-20..=20).contains(amount), "Scroll amount outside -20..20")
        }
        Action::Key { key, modifiers } => {
            ...
            for m in modifiers {
                    ...
                    seen.insert(modifier_key(m)?),
                    "Duplicate modifier (including aliases)"
                );
            }
        }
        _ => {}
...
fn named_key(key: &str) -> anyhow::Result<Key> {
    if let Ok(modifier) = modifier_key(key) {
        return Ok(modifier);
    }
    ...
    Ok(match normalized.as_str() {
        ...
        "home" => Key::Home,
        "end" => Key::End,
        "page_up" | "pageup" | "pgup" | "prior" => Key::PageUp,
        "page_down" | "pagedown" | "pgdn" | "next" => Key::PageDown,
        #[cfg(target_os = "linux")]
        s if s.len() == 1 && s.as_bytes()[0].is_ascii_punctuation() => {
...
fn coordinate_frame(
    ...
    let frame = FRAME
        .lock()
        .map_err(|_| anyhow::anyhow!("Frame state poisoned"))?
        .clone()
        .context("Take a screenshot first")?;
...
fn validate_frame(
    ...
    epoch: u64,
        ...
        frame.id == id
            && frame.epoch == epoch
            && frame.created.elapsed() < Duration::from_secs(30),
        "Stale screenshot frame; take another screenshot"
    );
    ensure!(
        displays.contains(&frame.display),
        "Display geometry changed; take another screenshot"
    );
    ensure!(
        x < frame.width && y < frame.height,
        "Coordinates outside screenshot"
    );
    Ok(())
}
...
mod tests {
    ...
    impl Mouse for FakeMouse {
        ...
        fn scroll(&mut self, _: i32, _: Axis) -> enigo::InputResult<()> {
            Ok(())
        }
    ...
    #[test]
    fn cropped_coordinates_map_to_native_display() {
        ...
        assert_eq!(region.point(0, 0, 400, 200), (400, 200));
        assert_eq!(region.point(200, 100, 400, 200), (800, 400));
        assert_eq!(region.point(399, 199, 400, 200), (1198, 598));
        assert!(capture::Region {
            x: u32::MAX,
            y: 0,
            width: 2,
            height: 1
        ...
        .validate(1920, 1080)
        .is_err());
        assert!(region.validate(500, 300).is_err());
    }
    #[test]
    fn browser_tools_share_explicit_permission_and_no_direct_dispatch() {
        ...
        for name in [
            ...
            assert!(ComputerUse
                .call_tool(Path::new("/"), name, json!({}))
                .is_err());
        }
        let action = tools.iter().find(|t| t.name == "browser_act").unwrap();
    ...
    #[test]
    fn button_release_survives_errors_and_unwinding() {
        for fail_press in [false, true] {
            ...
            assert!(
                with_button(&mut mouse, Button::Left, |_| bail!("cancelled")).is_err()
            );
            assert_eq!(mouse.events, vec![Direction::Press, Direction::Release]);
        }
        let mut mouse = FakeMouse::default();
        let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            with_button(&mut mouse, Button::Left, |_| panic!("worker panic"))
        }));
        assert_eq!(mouse.events, vec![Direction::Press, Direction::Release]);
    }
    #[tokio::test]
    async fn browser_setup_is_discoverable_before_permission_approval() {
        let result = ComputerUse
            ...
            .await
            .unwrap();
        let BuiltinMcpContent::Text { text, .. } = &result.content[0] else {
            panic!("JSON capabilities result")
        };
        let value: Value = serde_json::from_str(text).unwrap();
        assert_eq!(value["input"], "not_probed");
        let browser = &value["browser"];
        assert_eq!(browser["connection"], "not_probed");
        assert_eq!(browser["docs"], "Neoism Agent/Computer Use.md");
        assert_eq!(
            browser["firefox"]["endpointVariable"],
            "NEOISM_BROWSER_BIDI_URL"
        );
        assert_eq!(
            browser["firefox"]["endpointExample"],
            "ws://127.0.0.1:9223/session"
        );
        assert!(browser["firefox"]["posixSetup"][1]
            .as_str()
            .unwrap()
            .contains("--remote-debugging-port 9223"));
        assert!(browser["firefox"]["cleanup"]
            .as_str()
            .unwrap()
            .contains("browser_disconnect"));
        assert_eq!(
            browser["chromium"]["endpointVariable"],
            "NEOISM_BROWSER_CDP_URL"
        );
        assert!(browser["recommended"]
            .as_str()
            .unwrap()
            .contains("browser_attach"));
    ...
    #[tokio::test]
    async fn unauthorized_async_calls_never_reach_backend() {
        for tool in [
            "input",
            "batch",
            "windows",
    ...
    fn input_finalization_precedes_any_result_capture() {
        ...
        let result = after_input_capture(
            || {
                finished.set(true);
                let mut result =
                    text(json!({"completed":1,"cleanupFailure":"test failure"}));
                result.is_error = Some(true);
                Ok(result)
            },
            || {
        ...
        assert!(after_input_capture(
            || bail!("cancelled before execution"),
            || panic!("capture after failed admission")
        )
        .is_err());
    }
    #[test]
    fn immediate_capture_skips_sampling_and_waits() {
        ...
        let (_, info) = capture_settled_with_checks(
            0,
            || {
                checks.set(checks.get() + 1);
                Ok(())
            },
            || panic!("immediate capture must not wait"),
            || {
                captures.set(captures.get() + 1);
                Ok(image_rs::DynamicImage::new_rgb8(2, 3))
            },
        )
        .unwrap();
    ...
    fn keyboard_only_actions_do_not_need_capture_topology() {
        for value in [
            ...
            json!({"action":"paste","text":"hello","clipboard_policy":"replace"}),
        ] {
            assert!(!needs_pointer(&serde_json::from_value(value).unwrap()));
        }
        assert!(needs_pointer(&Action::Scroll {
            amount: 1,
            horizontal: false
        }));
    }
    ...
    fn linux_punctuation_keys_use_the_original_layout() {
        ...
        .unwrap();
        let slash =
            shortcuts::resolve_in_map(&map, 0, &[named_key("/").unwrap()]).unwrap();
        assert_eq!(slash, vec![61]);
        let codes =
            shortcuts::resolve_in_map(&map, 0, &[Key::Shift, named_key("-").unwrap()])
                .unwrap();
        let mut state = xkbcommon::xkb::State::new(&map);
        for code in &codes {
            state.update_key(
                xkbcommon::xkb::Keycode::new(u32::from(*code)),
                xkbcommon::xkb::KeyDirection::Down,
            );
        }
        assert_eq!(
            state.key_get_utf8(xkbcommon::xkb::Keycode::new(u32::from(codes[1]))),
            "_"
        );
    ...
    fn common_key_aliases_and_modifier_aliases_are_normalized() {
        for alias in [
            "Super_L", "super", "META", "win", "windows", "Command", "cmd", "logo",
        ] {
        ...
        assert!(validate(&Action::Key {
            key: "r".into(),
            modifiers: vec!["super".into(), "Super_L".into()]
        })
        .is_err());
        assert!(validate(&Action::Key {
            key: "l".into(),
    ...
    #[tokio::test]
    async fn native_panic_unwinds_worker_and_next_operation_succeeds() {
        ...
        let run = |panic: bool| {
            run_worker(
                ...
                |_| {
                    ...
                    if panic {
                        panic!("native parser regression");
                    }
                    Ok(42)
                },
            )
        ...
        with_test_backend(
            Arc::new(|tool, _| {
                if tool == "input" {
                    panic!("native parser panic through async worker");
                }
            ...
            async {
                ...
                let failed = ComputerUse
                    .call_tool_authorized_async(
                        Path::new("/"),
                        "input",
                        json!({"action":"text","text":"mock","target":"fixture"}),
                        true,
                        Arc::new(AtomicBool::new(false)),
                        Some(epoch),
                    )
                ...
                assert!(
                    next.is_ok(),
                    "next async screenshot worker must not stay busy/poisoned"
                );
            },
        )
        .await;
        ...
        let _ = std::panic::catch_unwind(|| {
            ...
            panic!("old poisoned unit lock");
        });
        assert!(lock.is_poisoned());
        assert_eq!(serialized(&lock, || Ok(7)).unwrap(), 7);
        assert!(!lock.is_poisoned());
        let guard = lock.lock().unwrap();
        assert!(serialized(&lock, || Ok(7))
            .unwrap_err()
            .to_string()
            .contains("busy"));
        drop(guard);
    ...
    #[test]
    fn visual_settling_restarts_after_animation_and_ignores_tiny_cursor_changes() {
    ...
    #[test]
    fn settling_is_bounded_and_cancellation_prevents_followup_capture() {
        let image = || image_rs::DynamicImage::new_rgb8(2, 2);
        assert!(
            capture_settled(3001, || Ok(()), || panic!("invalid budget captured"))
                .is_err()
        );
        ...
        let mut checks = 0;
        let mut captures = 0;
        assert!(capture_settled(
            1200,
            || {
                checks += 1;
                ensure!(checks < 3, "cancelled during settle");
                Ok(())
            },
            || {
                captures += 1;
    ...
    #[test]
    fn batch_bounds_preflight_and_partial_failure() {
        let parse = |mut v: Value| {
            v["target"] = json!("helium");
            serde_json::from_value::<Batch>(v).unwrap()
        };
        for value in [
            json!({"actions":[]}),
            json!({"actions":vec![json!({"action":"key","key":"a"});17]}),
            json!({"actions":[{"action":"text","text":"x".repeat(300)},{"action":"text","text":"y".repeat(300)}]}),
            json!({"actions":[{"action":"text","text":"ok"},{"action":"batch"}]}),
        ] {
            assert!(validate_batch(&parse(value)).is_err());
        }
        assert!(validate_batch(&parse(
            json!({"actions":[{"action":"text","text":"λ".repeat(512)}]})
        ))
        .is_ok());
        let mut visited = Vec::new();
        let (completed, error) = sequence(5, |index| {
            visited.push(index);
            ensure!(index != 2, "partial native failure");
            Ok(())
        });
        assert_eq!(visited, vec![0, 1, 2]);
        assert_eq!(completed, 2);
        assert!(error.unwrap().contains("partial native failure"));
    }
    #[tokio::test]
    async fn batch_stops_before_next_action_on_cancel_or_revocation() {
        let _revocation = TEST_REVOCATION_LOCK.lock().await;
        for revoke in [false, true] {
            let check = Check {
                ...
                epoch: STOP.load(Ordering::SeqCst),
                started: Instant::now(),
            };
            let mut delivered = 0;
            let (completed, error) = sequence(3, |_| {
                ...
                if revoke {
                    ...
                    check.cancel.store(true, Ordering::SeqCst);
                }
                Ok(())
            });
    ...
    #[tokio::test]
    async fn batch_returns_progress_and_final_image_but_never_captures_after_cancel() {
        let _revocation = TEST_REVOCATION_LOCK.lock().await;
        for (cancel, panics) in [(false, false), (true, false), (false, true)] {
            let check = Check {
                progress: Some(Arc::new(AtomicUsize::new(0))),
                effects: Arc::default(),
                clipboard_authorized: false,
                target: None,
                require_foreground: true,
            ...
            assert_eq!(captures, usize::from(!cancel));
            assert_eq!(check.progress.unwrap().load(Ordering::SeqCst), 1);
            assert_eq!(result.is_error, Some(true));
            let BuiltinMcpContent::Text { text: progress, .. } = &result.content[0]
            else {
                panic!("missing progress")
            };
            let progress: Value = serde_json::from_str(progress).unwrap();
            assert_eq!(progress["completed"], 1);
            assert_eq!(progress["failedIndex"], 1);
            assert_eq!(
                result
                    .content
                    .iter()
                    .any(|c| matches!(c, BuiltinMcpContent::Image { .. })),
                !cancel
            );
        }
    }
    #[test]
    fn explicit_target_never_inherits_another_calls_focus() {
        ...
        for action in [
            ...
            let mut explicit = action.clone();
            explicit["target"] = json!("helium");
            assert!(validate_input_scope("input", &mut explicit).is_ok());
        }
        assert!(validate_input_scope(
            "input",
            &mut json!({"scope":"desktop","action":"text","text":"unsafe"})
        )
        .is_err());
        assert!(validate_input_scope(
            "input",
            &mut json!({"scope":"desktop","target":"helium","action":"key","key":"tab"})
        )
        .is_err());
        assert!(validate_input_scope(
            "input",
            &mut json!({"scope":"desktop","action":"key","key":"tab","modifiers":["alt"]})
        )
        .is_ok());
        assert!(validate_batch(
            &serde_json::from_value(
                json!({"actions":[{"action":"text","text":"hello"}]})
            )
            .unwrap()
        )
        .is_err());
    ...
    #[tokio::test]
    async fn foreground_loss_stops_batch_and_binds_recovery_screenshot() {
        let _revocation = TEST_REVOCATION_LOCK.lock().await;
        let check = Check {
            progress: None,
            effects: Arc::default(),
        ...
        let mut reused = helium.clone();
        reused.pid = 2;
        assert!(windows::validate_list(&helium, true, vec![reused]).is_err());
    }
    #[cfg(target_os = "linux")]
    #[test]
    fn batch_clipboard_effects_survive_success_failure_and_cancellation() {
        let _revocation = TEST_REVOCATION_LOCK.blocking_lock();
        for (fail_at, cancel_after, changed, uncertain, completed) in [
            ...
            assert_eq!(value["applicationVerified"], false);
            assert_eq!(value["completed"], completed);
            assert_eq!(value["total"], 3);
            assert_eq!(
                value.get("clipboardChanged"),
                changed.then_some(&Value::Bool(true))
            );
            assert_eq!(
                value.get("clipboardMayHaveChanged"),
                uncertain.then_some(&Value::Bool(true))
            );
            assert_eq!(value["clipboardPolicy"], "replace");
            assert!(value["clipboardWarning"].is_string());
            assert_eq!(
                check.progress.as_ref().unwrap().load(Ordering::SeqCst),
                completed
            );
            assert_eq!(calls, completed + usize::from(fail_at.is_some()));
            if completed < 3 {
                assert_eq!(value["failedIndex"], completed);
                assert_eq!(result.is_error, Some(true));
            }
        }
    }
    #[cfg(target_os = "linux")]
    #[test]
    fn cancellation_before_paste_does_not_claim_clipboard_effects() {
        let _revocation = TEST_REVOCATION_LOCK.blocking_lock();
        let check = Check {
            progress: None,
            effects: Arc::default(),
        ...
        assert_eq!(value["status"], "partial_unknown");
        assert!(value.get("clipboardChanged").is_none());
        assert!(value.get("clipboardMayHaveChanged").is_none());
    }
    #[test]
    fn timeout_metadata_preserves_clipboard_uncertainty_without_payloads() {
        for (total, completed, changed, uncertain) in [
            (None, 0, false, true),
            (Some(3), 1, true, true),
            (Some(3), 2, true, false),
            (Some(3), 0, false, false),
        ] {
            ...
            if total.is_some() {
                assert_eq!(value["completed"], completed);
                assert_eq!(value["total"], total.unwrap());
            }
            if changed || uncertain {
                assert_eq!(value["clipboardPolicy"], "replace");
            }
    ...
    #[test]
    fn literal_text_rejects_controls_before_any_batch_action() {
        for c in (0u8..=31).chain(127..=159) {
            let value = format!("safe{}tail", char::from(c));
            assert!(validate(&Action::Text {
                text: value.clone(),
                method: Default::default(),
                clipboard_policy: Default::default(),
                paste_shortcut: Default::default()
            })
            .is_err());
        ...
        assert!(validate(&Action::Text {
            text: "e\u{301} \u{200d} العربية".into(),
            method: Default::default(),
            clipboard_policy: Default::default(),
            paste_shortcut: Default::default()
        })
    ...
    #[test]
    fn paste_is_explicit_targeted_and_preflighted() {
        let payload =
            json!({"action":"paste","text":"🦀\n\tمرحبا","clipboard_policy":"replace"});
        ...
        missing.as_object_mut().unwrap().remove("clipboard_policy");
        assert!(serde_json::from_value::<Action>(missing).is_err());
        for patch in [
            json!({"clipboard_policy":"preserve"}),
            json!({"text":"a\u{0000}b"}),
            json!({"text":"a".repeat(513)}),
            json!({"paste_shortcut":"meta-v"}),
            json!({"modifiers":["alt"]}),
        ] {
            ...
            let batch: Batch = serde_json::from_value(
                json!({"target":"window","actions":[{"action":"key","key":"space"},bad]}),
            )
            .unwrap();
            assert!(validate_batch(&batch).is_err());
        }
        let mut no_target = payload.clone();
        assert!(validate_input_scope("input", &mut no_target).is_err());
        let mut desktop = payload.clone();
        desktop["scope"] = json!("desktop");
        assert!(validate_input_scope("input", &mut desktop).is_err());
        let mut targeted = payload;
        targeted["target"] = json!("window");
        assert!(validate_input_scope("input", &mut targeted).is_ok());
        assert_eq!(
            PasteShortcut::ControlV.keys(),
            vec![Key::Control, Key::Unicode('v')]
        );
        assert_eq!(
            PasteShortcut::ControlShiftV.keys(),
            vec![Key::Control, Key::Shift, Key::Unicode('v')]
        );
    ...
    #[test]
    fn native_success_is_dispatch_not_application_verification() {
    ...
    #[test]
    fn batches_publish_input_schema_and_new_tools_require_session() {
        let tools = ComputerUse.tools();
        let batch = tools.iter().find(|t| t.name == "batch").unwrap();
        assert_eq!(batch.input_schema["properties"]["actions"]["maxItems"], 16);
        assert!(
            batch.input_schema["properties"]["actions"]["items"]["properties"]["action"]
                .is_object()
        );
        ...
        assert_eq!(
            input.input_schema["oneOf"][0]["required"],
            json!(["target"])
        );
        assert_eq!(
            input.input_schema["properties"]["scope"]["enum"],
            json!(["desktop"])
        );
        assert!(input.input_schema["properties"]["action"]["enum"]
            .as_array()
            .unwrap()
            .contains(&json!("paste")));
        assert_eq!(
            input.input_schema["properties"]["clipboard_policy"]["enum"],
            json!(["forbid", "replace"])
        );
        assert_eq!(
            input.input_schema["allOf"][0]["then"]["required"],
            json!(["text", "clipboard_policy"])
        );
        assert_eq!(
            batch.input_schema["properties"]["actions"]["items"]["allOf"],
            input.input_schema["allOf"]
        );
        assert!(
            !input.input_schema["oneOf"][1]["properties"]["action"]["enum"]
                .as_array()
                .unwrap()
                .contains(&json!("paste"))
        );
    ...
    fn arguments_are_strict_and_bounded() {
        assert!(serde_json::from_value::<Action>(
            json!({"action":"text","text":"ok","command":"bad"})
        )
        .is_err());
        assert!(validate(&Action::Text {
            text: "a".repeat(513),
        ...
        assert!(validate(&Action::Key {
            key: "enter".into(),
            modifiers: vec!["control".into(), "control".into()]
        })
        .is_err());
        assert!(named_key("shell").is_err());
    }
    #[test]
    fn stale_frames_topology_changes_and_out_of_bounds_are_rejected() {
        let display = Display {
            id: "screen".into(),
            x: -1920,
            y: 0,
    ...
    #[test]
    fn direct_capture_and_input_are_denied() {
        for tool in ["screenshot", "input", "batch", "windows", "focus", "wait"] {
            assert!(ComputerUse
                .call_tool(Path::new("/"), tool, json!({}))
                .is_err());
        }
        assert!(!ComputerUse.enabled_by_default());
    }
    #[tokio::test]
    async fn revocation_during_native_target_probe_cannot_admit_next_input() {
        let _revocation = TEST_REVOCATION_LOCK.lock().await;
        for revoke in [false, true] {
            let check = Check {
                progress: None,
    ...
    #[test]
    fn dropping_call_cancels_worker() {
        let dropped = Arc::new(AtomicBool::new(false));
        let check = Check {
            progress: None,
            effects: Arc::default(),
...
mod input_pipeline {
    ...
    use super::typing::{ClipboardPolicy, Method};
    use super::*;
    #[derive(Default)]
    ...
    pub(super) fn unplanned(index: usize) -> Value {
        json!({"index":index,"method":null,"phase":"preflight","status":"not_dispatched","completedNativeUnits":0,"currentUnitUncertain":false,"clipboard":"unchanged","cleanupFailure":null,"applicationVerified":false})
    }
    impl Effects {
        pub fn report(
            &self,
            ...
            let possible = self.records.iter().any(|r| {
                r["currentUnitUncertain"] == true
                    || r["completedNativeUnits"].as_u64().unwrap_or(0) > 0
                    || matches!(r["clipboard"].as_str(), Some("changed" | "may_changed"))
            });
            let mut records = self.records.clone();
            if error.is_some() {
                if let Some(record) =
                    records.get_mut(self.current.unwrap_or(self.completed))
                {
                    record["status"] = json!(if record["currentUnitUncertain"] == true
                        || record["completedNativeUnits"].as_u64().unwrap_or(0) > 0
                        || matches!(
                            record["clipboard"].as_str(),
                            Some("changed" | "may_changed")
                        ) {
                        "partial_unknown"
                    } else {
                        "failed"
            ...
            if self.records.iter().any(|r| r["clipboard"] == "changed") {
                value["clipboard"] = json!("changed");
                value["clipboardChanged"] = json!(true);
            }
            if self.records.iter().any(|r| r["clipboard"] == "may_changed") {
                value["clipboardMayHaveChanged"] = json!(true);
                if value["clipboard"] != "changed" {
                    value["clipboard"] = json!("may_changed");
                }
            }
            if let Some(total) = total {
                value["completed"] = json!(self.completed);
                value["total"] = json!(total);
                value["failedIndex"] = json!(if error.is_some() {
                    self.current
                        .filter(|i| *i < total)
                        .or((self.completed < total).then_some(self.completed))
                } else {
                    None
                });
            }
            if total.is_none() {
                if let Some(record) = self.records.first() {
                    for field in [
                        "method",
                        "phase",
                        "completedNativeUnits",
                        "currentUnitUncertain",
                        "cleanupFailure",
                        "totalNativeUnits",
                        "nativeUnitKind",
                    ] {
            ...
            if self.cleanup_phase {
                value["phase"] = json!("cleanup");
                value["cleanupPending"] = json!(self.cleanup_failure.is_none());
            }
            if let Some(error) = &self.cleanup_failure {
                value["cleanupFailure"] = json!(error);
            }
            let mut result = text(value);
            result.is_error = error.map(|_| true);
            result
        }
    }
    fn update(check: &Check, index: usize, f: impl FnOnce(&mut Value)) {
        let mut effects = check
            .effects
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
    ...
    impl Prepared {
        fn method(&self) -> &'static str {
            match self {
                ...
                #[cfg(target_os = "linux")]
                Self::Text(plan) => plan.method(),
                #[cfg(not(target_os = "linux"))]
                Self::Native(_) => "native",
                #[cfg(not(target_os = "linux"))]
                Self::Keys(_) => "keyboard",
    ...
    pub(super) fn run(
        ...
        check
            ...
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .records = (0..total).map(unplanned).collect();
        let result = native_boundary(|| {
            ...
            check.check()?;
            #[cfg(target_os = "linux")]
            let mut keyboard = if actions.iter().any(|a| {
                matches!(
                    a,
                    Action::Text { .. } | Action::Key { .. } | Action::Paste { .. }
                )
                        ...
                        for (index, action) in actions.into_iter().enumerate() {
                            ...
                            let plan = match action {
                                Action::Text {
                                    ...
                                    #[cfg(target_os = "linux")]
                                    {
                                        Prepared::Text(typing::prepare_text(
                                            keyboard.as_ref().unwrap(),
                                            &text,
                                            method,
                                            clipboard_policy,
                                            &paste_shortcut.keys(),
                                            ...
                                            check
                                                .clipboard_authorized
                                                .then(typing::ClipboardPermit::granted),
                                            &mut || check.fast(),
                                        )?)
                                    }
                                    #[cfg(not(target_os = "linux"))]
                                    {
                                        ...
                                        select(method, clipboard_policy, false, false)?;
                                        Prepared::Native(platform::prepare_native(&text)?)
                                    }
                                }
                                Action::Paste {
                                    text,
                                ...
                                pointer => {
                                    preflight_pointer(&pointer, displays, check)?;
                            ...
                            match &plan {
                                #[cfg(target_os = "linux")]
                                Prepared::Text(p) => {
                                    record["totalNativeUnits"] = json!(p.unit_count());
                                    record["nativeUnitKind"] = json!(p.unit_kind());
                                }
                                #[cfg(target_os = "linux")]
                                Prepared::Keyboard(p) => {
                                    record["totalNativeUnits"] = json!(p.unit_count());
                                    record["nativeUnitKind"] = json!(p.units_kind);
                                }
                                #[cfg(not(target_os = "linux"))]
                                Prepared::Native(p) => {
                                    record["totalNativeUnits"] = json!(p.total_units());
                                    record["nativeUnitKind"] =
                                        json!(match p.unit_kind() {
                                            platform::UnitKind::Utf16CodeUnits =>
                                                "utf16_code_units",
                                            platform::UnitKind::UnicodeScalars =>
                                                "unicode_scalars",
                                        });
                                }
                                _ => {}
                ...
                for (index, plan) in prepared.into_iter().enumerate() {
                    ...
                    check.check()?;
                    {
                        let mut e = check
                            ...
                            .lock()
                            .unwrap_or_else(std::sync::PoisonError::into_inner);
                        e.current = Some(index);
                        e.started = true;
                    }
                    update(check, index, |r| {
                        r["phase"] = json!("dispatch");
                        r["currentUnitUncertain"] = json!(true);
                    });
                    let mut progress = |units| {
                        update(check, index, |r| {
                            r["completedNativeUnits"] = json!(
                                r["completedNativeUnits"].as_u64().unwrap_or(0)
                                    + units as u64
                            )
                        })
                    };
                    let execution = (|| -> anyhow::Result<()> {
                        match plan {
                            #[cfg(target_os = "linux")]
                            Prepared::Keyboard(plan) => {
                                keyboard.as_mut().unwrap().execute_with_checks(
                                    &plan,
                                    &mut linux_text::Checks {
                                        wait: &mut || check.fast(),
                                        full: &mut || check.check(),
                                    },
                                    &mut progress,
                                )?
                            }
                            #[cfg(target_os = "linux")]
                            Prepared::Text(plan) => typing::execute_text_with_wait(
                            ...
                            #[cfg(not(target_os = "linux"))]
                            Prepared::Keys(keys) => {
                                shortcuts::send(&mut input, &keys, || check.check())?;
                                progress(1);
                            }
                            Prepared::Pointer(action) => {
                                pointer_action(action, displays, check)?;
                                progress(1);
                            }
                        };
                        Ok(())
                    ...
                    if let Err(error) = execution {
                        update(check, index, |r| {
                            r["status"] = json!("partial_unknown");
                            r["error"] = json!(format!("{error:#}"));
                            #[cfg(target_os = "linux")]
                            if let Some(facts) = linux_text::failure_facts(&error) {
                                r["completedNativeUnits"] = json!(facts.completed_units);
                                r["currentUnitUncertain"] =
                                    json!(facts.current_unit_uncertain);
                                if facts.cleanup_failed {
                                    r["cleanupFailure"] = json!(format!("{error:#}"));
                                }
                            }
                            #[cfg(not(target_os = "linux"))]
                            if let Some(facts) =
                                error.downcast_ref::<platform::NativeError>()
                            {
                    ...
                    update(check, index, |r| {
                        r["phase"] = json!("complete");
                        r["status"] = json!("dispatched_unverified");
                        r["currentUnitUncertain"] = json!(false);
                    });
                    check
                        .effects
                        .lock()
            ...
            if let Some(session) = keyboard.as_mut() {
                return finish_outcome(&check.effects, total, outcome, || {
                    session.finish()
                });
            }
            outcome
        ...
        match native_boundary(finish) {
            Err(error) => {
                effects
                    .lock()
                    .unwrap_or_else(std::sync::PoisonError::into_inner)
                    .cleanup_failure = Some(format!("{error:#}"));
    ...
    fn preflight_pointer(
        ...
        let display = match action {
            Action::Move { frame, x, y } | Action::Click { frame, x, y, .. } => {
                coordinate_frame(frame, *x, *y, displays, check)?.display
            }
            Action::Drag {
                frame,
                x,
                ...
                let start = coordinate_frame(frame, *x, *y, displays, check)?;
                coordinate_frame(frame, *to_x, *to_y, displays, check)?;
                start.display
            }
            Action::Scroll { .. } => {
                #[cfg(target_os = "linux")]
                ...
                displays.first().context("No display")?.clone()
            }
            _ => unreachable!(),
        };
        #[cfg(target_os = "linux")]
    ...
    mod effect_tests {
        use super::*;
        fn payload(result: BuiltinMcpCallResult) -> Value {
            let BuiltinMcpContent::Text { text, .. } = &result.content[0] else {
                panic!()
            };
        ...
        #[test]
        fn timeout_reports_actual_selected_method_progress_and_clipboard() {
            let mut effects = Effects::default();
            let before = payload(effects.report(Some(2), Some("timeout".into())));
            assert_eq!(before["status"], "failed");
            assert_eq!(before["clipboard"], "unchanged");
            assert_eq!(before["partialActionPossible"], false);
            effects.started = true;
            effects.current = Some(1);
            effects.completed = 1;
            effects.records = vec![
                json!({"method":"keyboard","completedNativeUnits":3,"currentUnitUncertain":false,"clipboard":"unchanged","status":"dispatched_unverified"}),
                json!({"method":"paste","phase":"clipboard_publication","completedNativeUnits":0,"currentUnitUncertain":false,"clipboard":"may_changed"}),
            ];
            let timed = payload(effects.report(Some(2), Some("timeout".into())));
            assert_eq!(timed["completed"], 1);
            assert_eq!(timed["failedIndex"], 1);
            assert_eq!(timed["status"], "partial_unknown");
            assert_eq!(timed["actions"][1]["method"], "paste");
            assert_eq!(timed["clipboardMayHaveChanged"], true);
            assert_eq!(timed["applicationVerified"], false);
        }
        #[test]
        fn primary_clipboard_error_still_finishes_and_preserves_both_failures() {
            let effects = Mutex::new(Effects {
                started: true,
                completed: 1,
                current: Some(1),
                records: vec![
                    json!({"method":"keyboard","status":"dispatched_unverified","completedNativeUnits":3,"currentUnitUncertain":false,"clipboard":"unchanged"}),
                    json!({"method":"paste","status":"partial_unknown","completedNativeUnits":0,"currentUnitUncertain":false,"clipboard":"may_changed"}),
                ],
                ..Default::default()
            });
            let finished = std::cell::Cell::new(false);
            let primary = finish_outcome(
                &effects,
                2,
                Err(anyhow::anyhow!("clipboard publication failed")),
                || {
                    finished.set(true);
                    bail!("keyboard destroy ACK failed")
                },
            )
            .unwrap_err();
            assert!(finished.get());
            assert_eq!(primary.to_string(), "clipboard publication failed");
            let report = payload(
                effects
                    .lock()
                    .unwrap()
                    .report(Some(2), Some(primary.to_string())),
            );
            assert_eq!(report["completed"], 1);
            assert_eq!(report["failedIndex"], 1);
            assert_eq!(report["actions"][0]["status"], "dispatched_unverified");
            assert_eq!(report["phase"], "cleanup");
            assert_eq!(report["cleanupFailure"], "keyboard destroy ACK failed");
            assert_eq!(report["error"], "clipboard publication failed");
        }
        #[test]
        fn final_cleanup_failure_preserves_completed_actions_without_out_of_range_index()
        {
            let effects = Effects {
                started: true,
                completed: 1,
                current: None,
                cleanup_phase: true,
                cleanup_failure: Some("destroy acknowledgement failed".into()),
                records: vec![
                    json!({"method":"keyboard","status":"dispatched_unverified","phase":"complete","completedNativeUnits":3,"currentUnitUncertain":false,"clipboard":"unchanged"}),
                ],
            };
            let report = payload(effects.report(Some(1), Some("cleanup failed".into())));
            assert_eq!(report["completed"], 1);
            assert!(report["failedIndex"].is_null());
            assert_eq!(report["phase"], "cleanup");
            assert_eq!(report["actions"][0]["status"], "dispatched_unverified");
            assert!(report["cleanupFailure"].is_string());
            assert_eq!(report["applicationVerified"], false);
        }
        #[test]
        fn known_pre_event_runtime_failure_does_not_claim_effects() {
            let effects = Effects {
                started: true,
                current: Some(0),
                records: vec![
                    json!({"method":"keyboard","completedNativeUnits":0,"currentUnitUncertain":false,"clipboard":"unchanged"}),
                ],
                ..Default::default()
            };
            let failed =
                payload(effects.report(None, Some("layout changed before event".into())));
            assert_eq!(failed["status"], "failed");
            assert_eq!(failed["currentUnitUncertain"], false);
            assert_eq!(failed["partialActionPossible"], false);
        }
    }
}
