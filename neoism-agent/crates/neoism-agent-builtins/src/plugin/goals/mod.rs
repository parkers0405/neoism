...
impl GoalsPlugin {
    pub fn new(host: Arc<dyn GoalsHost>) -> Self {
        Self { host }
    }
}
...
impl PluginDefinition for GoalsPlugin {
    fn manifest(&self) -> PluginManifest {
        PluginManifest {
            ...
            internal: true,
            disableable: true,
            capabilities: vec!["neoism.goals".into()],
            requires: Vec::new(),
            event_namespaces: vec!["goal".into()],
    ...
    fn handle<'a>(&'a self, request: RouteRequest) -> PluginFuture<'a, RouteResponse> {
        Box::pin(async move {
            let session_id = request.session_id.as_deref().ok_or_else(|| {
                PluginRuntimeError::new("goal route requires session_id")
            })?;
            let goal = match self.action {
                ...
                GoalRouteAction::Research => {
                    ...
                    let request: GoalResearchRequest =
...
pub async fn scrape_url(url: &str) -> anyhow::Result<ResearchPage> {
    ...
    let response = reqwest::Client::builder()
        ...
        .post(&endpoint)
        .bearer_auth(api_key)
        .json(&json!({"url": url, "formats": ["markdown"], "onlyMainContent": true}))
        .send()
        .await
    ...
    Ok(ResearchPage {
        url: url.into(),
        title: data.metadata.and_then(|value| value.title),
