...
use neoism_ui::panels::agent_pane::question_policy::{
...
impl NeoismAgentPane {
    ...
    pub(crate) fn enqueue_pending_question(
        &mut self,
        question: NeoismAgentPendingQuestion,
    ) {
    ...
    pub(crate) fn remove_pending_question(&mut self, request_id: &str) -> bool {
    ...
    pub(crate) fn clear_pending_question_current(&mut self) {
    ...
    /// Commit the selected row for the current question; the reply is
    /// sent once the request's last question is answered.
    ...
    /// Esc — reject the pending question so the model's run resumes
    /// with a "rejected" error instead of parking forever.
