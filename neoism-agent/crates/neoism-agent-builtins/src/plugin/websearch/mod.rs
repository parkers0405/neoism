...
use neoism_agent_plugin_api::{
    ...
    PluginRuntimeError, PluginToolDefinition, PluginToolInvocation, PluginToolPermission,
    PluginToolResult, RuntimeTool,
};
use serde_json::json;
...
impl PluginDefinition for WebsearchPlugin {
    fn manifest(&self) -> PluginManifest {
        PluginManifest {
            ...
            internal: true,
            disableable: true,
            capabilities: vec!["neoism.websearch".into()],
            requires: Vec::new(),
            event_namespaces: vec!["websearch".into()],
            api_prefix: Some("/v2/tools".into()),
            config: BTreeMap::new(),
        }
    }

    fn required_capabilities(&self) -> Vec<neoism_agent_plugin_api::HostCapability> {
        vec![neoism_agent_plugin_api::HostCapability::Network]
    }
    fn contributions(
        &self,
...
pub async fn search(query: &str) -> anyhow::Result<PluginToolResult> {
    ...
    if query.is_empty() {
        anyhow::bail!("tool argument query is required");
    }
    let endpoint = std::env::var("NEOISM_AGENT_WEBSEARCH_ENDPOINT")
        .unwrap_or_else(|_| "https://duckduckgo.com/html/".into());
    ...
    Ok(PluginToolResult {
        ...
        output,
        metadata: Some(
            json!({"query":query,"endpoint":endpoint,"bytes":bytes.len(),"truncated":truncated}),
        ),
    })
}
...
mod tests {
    ...
    #[test]
    fn strips_provider_html() {
        assert_eq!(render(b"<b>one</b>  two").0, "one two");
    }
}
