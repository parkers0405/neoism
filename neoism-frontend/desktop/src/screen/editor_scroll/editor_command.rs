    ...
    pub fn scroll(&mut self, new_scroll_x_px: f64, new_scroll_y_px: f64) {
            ...
            || self.context_manager.current().epub.is_some()
        {
                ...
                self.context_manager.current_mut().active_markdown_mut()
            {
                ...
                let notebook_scroll = markdown
                    .documentation_notebook
                    .as_ref()
                    .is_some_and(|binding| {
                        binding.session.lock().ok().is_some_and(|mut book| {
                            book.wheel_at(mouse_x, mouse_y, new_scroll_y_px as f32)
                        })
                    });
                if notebook_scroll
                    || markdown.outline_wheel_at(mouse_x, mouse_y, new_scroll_y_px as f32)
                {
                    self.mark_dirty();
                    return;
                }
