...
use neoism_agent_service_api::{
    AgentServices, ConfigLayer, ConfigSnapshot, ConfigSnapshotRequest, ConfigSourceService,
...
impl ConfigSourceService for DesktopNotesConfig {
    fn snapshot(&self, request: &ConfigSnapshotRequest) -> Result<ConfigSnapshot, ServiceError> {
        self.inner.snapshot(request).map(|snapshot| self.decorate(snapshot))
    }
...
fn serve_stdio() -> io::Result<()> {
    ...
    for line in stdin.lock().lines() {
        ...
        let response = match handle_request(request.get("method").and_then(Value::as_str).unwrap_or(""), request.get("params").cloned().unwrap_or_else(|| json!({}))) {
...
fn handle_request(method: &str, params: Value) -> Result<Value, RpcError> {
    match method {
        ...
        _ => Err(RpcError { code: -32601, message: format!("unknown MCP method {method}") }),
    }
}
...
fn tool_definitions() -> Vec<Value> {
    vec![
        tool("list", "List Markdown files in the workspace's linked Neoism Notes vault", json!({"type":"object","properties":{"limit":{"type":"integer","minimum":1}}})),
        tool("search", "Search the workspace's linked Neoism Notes vault", json!({"type":"object","properties":{"query":{"type":"string"},"limit":{"type":"integer","minimum":1}},"required":["query"]})),
        tool("read", "Read a note by vault-relative path", json!({"type":"object","properties":{"path":{"type":"string"}},"required":["path"]})),
        tool("create", "Create a Markdown note in the linked vault", json!({"type":"object","properties":{"title":{"type":"string"},"content":{"type":"string"}},"required":["title"]})),
        tool("write", "Write or replace a note by vault-relative path", json!({"type":"object","properties":{"path":{"type":"string"},"content":{"type":"string"}},"required":["path","content"]})),
        tool("tasks", "List Markdown tasks in the linked vault", json!({"type":"object","properties":{"limit":{"type":"integer","minimum":1}}})),
        tool("taskToggle", "Set or toggle a Markdown task by path and one-based line", json!({"type":"object","properties":{"path":{"type":"string"},"line":{"type":"integer","minimum":1},"checked":{"type":"boolean"}},"required":["path","line"]})),
    ]
}
...
fn call_notes_tool(notes: &Notes, name: &str, arguments: Value) -> Result<String, String> {
    ...
    match name {
        ...
        "search" => {
            ...
            for path in notes.files(10_000)? {
                let text = std::fs::read_to_string(notes.path(&path)?).map_err(|error| error.to_string())?;
                for (line, content) in text.lines().enumerate() {
                    if content.to_lowercase().contains(&query) {
                        hits.push(format!("{path}:{} {}", line + 1, content.trim()));
                        if hits.len() >= limit { return Ok(hits.join("\n")); }
                    }
                }
            }
            Ok(hits.join("\n"))
        ...
        "create" => {
            ...
            std::fs::OpenOptions::new().write(true).create_new(true).open(&path)
                .and_then(|mut file| file.write_all(content.as_bytes())).map_err(|error| error.to_string())?;
            Ok(path.strip_prefix(&notes.root).unwrap_or(&path).to_string_lossy().replace('\\', "/"))
        }
        "write" => {
            let path = required_string_alias(&arguments, "path", "note")?;
            let absolute = notes.path(&path)?;
            if let Some(parent) = absolute.parent() { std::fs::create_dir_all(parent).map_err(|error| error.to_string())?; }
            std::fs::write(&absolute, required_string(&arguments, "content")?).map_err(|error| error.to_string())?;
            ...
            for path in notes.files(10_000)? {
                ...
                for (line, content) in text.lines().enumerate() {
                    ...
                    if trimmed.starts_with("- [ ]") || trimmed.starts_with("- [x]") || trimmed.starts_with("- [X]") {
                        ...
                        if tasks.len() >= limit { return Ok(tasks.join("\n")); }
                    }
                }
            }
            Ok(tasks.join("\n"))
        ...
        "taskToggle" => {
            let path = required_string_alias(&arguments, "path", "note")?;
            let line = arguments.get("line").and_then(Value::as_u64).filter(|line| *line > 0)
                .ok_or_else(|| "line is required".to_string())? as usize;
            let absolute = notes.path(&path)?;
            ...
            let target = lines.get_mut(line - 1).ok_or_else(|| "task line is outside the note".to_string())?;
            let checked = arguments.get("checked").and_then(Value::as_bool)
                .unwrap_or(!target.contains("[x]") && !target.contains("[X]"));
            let marker = if target.contains("[ ]") { "[ ]" } else if target.contains("[x]") { "[x]" } else if target.contains("[X]") { "[X]" } else { return Err("line is not a Markdown task".into()) };
            *target = target.replacen(marker, if checked { "[x]" } else { "[ ]" }, 1);
            let mut content = lines.join("\n");
            if trailing_newline { content.push('\n'); }
...
impl Notes {
    fn for_workspace(workspace: &Path) -> Result<Self, String> {
        ...
        Ok(Self { root: workspace.notes_workspace_dir() })
    }
...
fn collect_markdown_files(root: &Path, directory: &Path, out: &mut Vec<String>, limit: usize) -> Result<(), String> {
...
fn required_string(arguments: &Value, key: &str) -> Result<String, String> {
    arguments.get(key).and_then(Value::as_str).map(str::to_string)
        .filter(|value| !value.trim().is_empty()).ok_or_else(|| format!("{key} is required"))
}

fn required_string_alias(arguments: &Value, key: &str, alias: &str) -> Result<String, String> {
    required_string(arguments, key).or_else(|_| required_string(arguments, alias).map_err(|_| format!("{key} is required")))
}
...
mod tests {
    ...
    fn advertises_the_complete_notes_surface() {
        assert_eq!(tool_definitions().iter().filter_map(|tool| tool["name"].as_str()).collect::<Vec<_>>(), ["list", "search", "read", "create", "write", "tasks", "taskToggle"]);
    }

    #[test]
    fn rejects_paths_outside_the_vault() {
        let notes = Notes { root: PathBuf::from("/tmp/notes") };
    ...
    fn model_can_create_read_write_search_and_toggle_notes() {
        let root = std::env::temp_dir().join(format!("neoism-notes-mcp-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).unwrap();
        let notes = Notes { root: root.clone() };

        assert_eq!(call_notes_tool(&notes, "create", json!({"title":"Roadmap","content":"# Roadmap\n- [ ] Ship MCP\n"})).unwrap(), "Roadmap.md");
        assert!(call_notes_tool(&notes, "list", json!({})).unwrap().contains("Roadmap.md"));
        assert!(call_notes_tool(&notes, "search", json!({"query":"Ship MCP"})).unwrap().contains("Roadmap.md:2"));
        assert!(call_notes_tool(&notes, "tasks", json!({})).unwrap().contains("- [ ] Ship MCP"));
        assert_eq!(call_notes_tool(&notes, "taskToggle", json!({"path":"Roadmap.md","line":2})).unwrap(), "Roadmap.md:2");
        assert!(call_notes_tool(&notes, "read", json!({"path":"Roadmap.md"})).unwrap().contains("- [x] Ship MCP"));
        assert_eq!(call_notes_tool(&notes, "write", json!({"path":"Roadmap.md","content":"# Updated\n"})).unwrap(), "Wrote Roadmap.md");
        assert_eq!(std::fs::read_to_string(root.join("Roadmap.md")).unwrap(), "# Updated\n");
        let _ = std::fs::remove_dir_all(root);
    }

    #[test]
    fn accepts_legacy_note_and_query_argument_aliases() {
        let root = std::env::temp_dir().join(format!("neoism-notes-mcp-aliases-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).unwrap();
        let notes = Notes { root: root.clone() };

        assert_eq!(call_notes_tool(&notes, "create", json!({"query":"Legacy"})).unwrap(), "Legacy.md");
        assert!(call_notes_tool(&notes, "read", json!({"note":"Legacy.md"})).unwrap().contains("# Legacy"));
        assert_eq!(call_notes_tool(&notes, "write", json!({"note":"Legacy.md","content":"- [ ] Done\n"})).unwrap(), "Wrote Legacy.md");
        assert_eq!(call_notes_tool(&notes, "taskToggle", json!({"note":"Legacy.md","line":1})).unwrap(), "Legacy.md:1");
        let _ = std::fs::remove_dir_all(root);
    }
    ...
    fn desktop_config_injects_notes_as_a_read_only_mcp() {
        ...
        let root = std::env::temp_dir().join(format!("neoism-notes-config-{}", std::process::id()));
        std::fs::create_dir_all(&root).unwrap();
        let snapshot = services.config.snapshot(&ConfigSnapshotRequest::new(&root)).unwrap();
