...
impl ScopedLinuxCaptureSession {
    ...
    pub(super) fn displays(&self) -> Vec<super::Display> {
        self.topology.iter().map(|o|super::Display {id:o.name.clone(),x:o.logical_position().x,y:o.logical_position().y,width:o.logical_size().width,height:o.logical_size().height}).collect()
    }
