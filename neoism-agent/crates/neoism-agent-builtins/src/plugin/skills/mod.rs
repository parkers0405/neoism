...
impl PluginDefinition for SkillsPlugin {
    fn manifest(&self) -> PluginManifest {
        PluginManifest {
            ...
            internal: true,
            disableable: true,
            capabilities: vec!["neoism.skills".into()],
            requires: Vec::new(),
            event_namespaces: vec!["skill".into()],
            api_prefix: Some("/v2/skills".into()),
            config: BTreeMap::new(),
        }
    }

    fn required_capabilities(&self) -> Vec<neoism_agent_plugin_api::HostCapability> {
        vec![neoism_agent_plugin_api::HostCapability::WorkspaceRead]
    }
    fn contributions(
        &self,
        ...
        registrar.runtime_route(RouteContribution {
            ...
            handler: Arc::new(SkillsRoute(
                self.config.clone(),
                self.discovery_roots.clone(),
            )),
        });
        registrar.skill_source_runtime(
            "workspace-skills",
            Arc::new(WorkspaceSkills(
                self.config.clone(),
                self.discovery_roots.clone(),
            )),
        );
...
pub async fn load(
    ...
    if directory.starts_with(neoism_agent_service_api::INSTALLATION_CONTEXT_PREFIX) {
        ...
        return load_from_config(
            &snapshot.workspace.to_string_lossy(),
            &document,
            &roots,
        )
        .await;
...
async fn load_from_config(
    ...
    let mut by_name = load_local(directory, &document.skills.paths, &roots)?
        .into_iter()
        .map(|skill| (skill.info.id.clone(), skill))
        .collect::<BTreeMap<_, _>>();
    for skill in load_remote(&document.skills.urls).await {
...
async fn load_remote(urls: &[String]) -> Vec<Skill> {
    ...
    for url in urls {
        if let Ok(mut pulled) = pull_remote_skills(url).await {
            skills.append(&mut pulled);
        }
    }
    skills
}
...
fn skill_files(root: &Path) -> anyhow::Result<Vec<PathBuf>> {
    if root.is_file() {
        return Ok(
            (root.file_name().and_then(|name| name.to_str()) == Some("SKILL.md"))
...
fn default_cache_dir() -> PathBuf {
    if let Some(path) =
        std::env::var_os("NEOISM_AGENT_CACHE_DIR").filter(|path| !path.is_empty())
    {
...
mod tests {
    ...
    struct Host;
    impl SkillsHost for Host {
        fn register_tools(&self, _: &mut PluginContributions) {}
    }
    ...
    #[test]
    fn rejects_remote_parent_traversal() {
