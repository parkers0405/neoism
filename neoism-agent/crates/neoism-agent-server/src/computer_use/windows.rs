...
fn serialize_native_id<S: serde::Serializer>(
    id: &str,
    serializer: S,
) -> Result<S::Ok, S::Error> {
...
fn native_id(w: &Window) -> &str {
    identity_id(w).split('|').next().unwrap_or_default()
}
fn identity_id(w: &Window) -> &str {
    w.id.split('#').next().unwrap_or_default()
}
fn reconcile(
    ...
    for mut window in windows.into_iter().take(MAX_TARGETS) {
        if tokens
            .iter()
            .any(|t| native_id(&t.window) == native_id(&window))
        {
            continue;
        }
        let token = old
            ...
            .find(|t| {
                t.epoch == epoch
                    && same_identity(&t.window, &window)
                    && bounds(&t.window) == bounds(&window)
            })
            .map(|t| t.token.clone())
    ...
    tokens
        .iter()
        .map(|t| (t.token.clone(), t.window.clone()))
        .collect()
}
...
pub(super) fn resolve(token: &str) -> anyhow::Result<Window> {
    resolve_from(
        &TOKENS
            ...
            .map_err(|_| anyhow::anyhow!("Window state poisoned"))?,
        token,
        super::STOP.load(std::sync::atomic::Ordering::SeqCst),
    )
}
fn resolve_from(tokens: &[Token], token: &str, epoch: u64) -> anyhow::Result<Window> {
    tokens
        .iter()
        .find(|t| t.token == token && t.epoch == epoch)
        .map(|t| t.window.clone())
        .context("Unknown/stale window target; list windows again")
}
fn same_identity(a: &Window, b: &Window) -> bool {
    identity_id(a) == identity_id(b) && a.pid == b.pid && a.app == b.app
}
pub(super) fn process_birth(w: &Window) -> anyhow::Result<String> {
    process_start(w.pid)
}
pub(super) fn same_process(a: &Window, b: &Window) -> anyhow::Result<bool> {
    Ok(a.pid == b.pid && a.app == b.app && process_birth(a)? == process_birth(b)?)
}
fn bounds(w: &Window) -> (i32, i32, u32, u32) {
...
pub(super) fn same_target(a: Option<&Window>, b: Option<&Window>) -> bool {
    match (a, b) {
        (None, None) => true,
        (Some(a), Some(b)) => {
            a.id == b.id && same_identity(a, b) && bounds(a) == bounds(b)
        }
        _ => false,
    }
}
fn revoke(target: &Window) {
    if let Ok(mut tokens) = TOKENS.lock() {
        tokens.retain(|t| t.window.id != target.id);
    }
}
pub(super) fn validate(target: &Window, foreground: bool) -> anyhow::Result<Window> {
    if let Some((_, token)) = target.id.split_once('#') {
        resolve(token)?;
    }
    #[cfg(target_os = "linux")]
    if foreground {
        return validate_active_with(target, request, hypr_identity);
    }
    let windows = native_list()?;
    if !windows
        .iter()
        .any(|w| same_identity(w, target) && bounds(w) == bounds(target))
    {
        revoke(target);
    }
...
pub(super) fn validate_list(
    ...
    let mut current = windows
        .into_iter()
        .find(|w| same_identity(w, target))
        .context("Target window disappeared or identity changed")?;
    ensure!(
        bounds(&current) == bounds(target),
        "Target window moved/resized; list windows and take another screenshot"
    );
    if foreground {
        ensure!(
            current.focused,
            "Target is not foreground; observe/focus explicitly before input"
        );
    }
...
fn condition_with(
    target: &Window,
    closed: bool,
    exists: impl FnOnce(&Window) -> anyhow::Result<bool>,
    list: impl FnOnce() -> anyhow::Result<Vec<Window>>,
) -> anyhow::Result<bool> {
    ...
    if closed {
        return exists(target).map(|exists| {
            if !exists {
                revoke(target);
            }
            !exists
    ...
    let windows = list()?;
    if !windows.iter().any(|w| same_identity(w, target)) {
        revoke(target);
    }
    Ok(windows
        .into_iter()
        .any(|w| same_identity(&w, target) && w.focused))
}
pub(super) fn focus(
    token: &str,
    ...
    if !native_exists(&target)? {
        revoke(&target);
        anyhow::bail!("Target window closed or identity changed");
    }
    check()?;
    ...
    check()?;
        ...
        current.focused,
        "OS did not confirm foreground focus; observe again"
    );
    Ok(())
}
...
fn native_exists(target: &Window) -> anyhow::Result<bool> {
    ...
    if thread == 0 {
        ...
        anyhow::bail!(
            "Cannot read native window owner: {}",
            std::io::Error::last_os_error()
        );
    }
    Ok(pid == target.pid
        && qualified_identity(native_id(target), pid)? == identity_id(target))
}
#[cfg(target_os = "macos")]
fn native_exists(target: &Window) -> anyhow::Result<bool> {
    macos::exists(target)
...
fn native_exists(_: &Window) -> anyhow::Result<bool> {
    anyhow::bail!("Window existence probe unsupported")
}
...
fn request_inner(command: &str) -> anyhow::Result<String> {
    ...
    let signature = std::env::var("HYPRLAND_INSTANCE_SIGNATURE")
        .context("Window targeting currently requires Hyprland native IPC on Linux")?;
    ensure!(
        !signature.is_empty()
            && signature
                .bytes()
    ...
    let path = std::path::PathBuf::from(
        std::env::var_os("XDG_RUNTIME_DIR").context("No XDG_RUNTIME_DIR")?,
    )
    .join("hypr")
    .join(signature)
    .join(".socket.sock");
    ...
    stream
        ...
        result.len() <= 4 * 1024 * 1024,
        "Hyprland reply exceeds limit"
    );
    Ok(result)
}
#[cfg(target_os = "linux")]
fn native_list() -> anyhow::Result<Vec<Window>> {
    ...
    clients
        .into_iter()
        .filter(|v| v["mapped"].as_bool() == Some(true))
        .map(|v| {
            let identity = hypr_identity(&v)?;
            let focused =
                active["address"] == v["address"] && hypr_identity(&active)? == identity;
            hypr_window(&v, focused, identity)
        })
        .collect()
}
#[cfg(target_os = "linux")]
fn hypr_identity(v: &serde_json::Value) -> anyhow::Result<String> {
    hypr_identity_with(v, process_start)
}
#[cfg(any(target_os = "linux", test))]
fn hypr_identity_with(
    v: &serde_json::Value,
    start: impl FnOnce(u32) -> anyhow::Result<String>,
    ...
    let id = v["address"].as_str().context("Missing window address")?;
        ...
        id.starts_with("0x")
            && id.len() > 2
            && id.len() <= 18
            && id[2..].bytes().all(|c| c.is_ascii_hexdigit()),
        "Invalid native window address"
    );
...
fn hypr_window(
    ...
    identity: String,
        ...
        v["mapped"].as_bool() == Some(true),
        "Active window is not mapped"
    );
    let w = Window {
        id: identity,
        pid: u32::try_from(v["pid"].as_u64().context("Missing window PID")?)?,
        app: v["class"].as_str().context("Missing window class")?.into(),
        title: v["title"].as_str().unwrap_or_default().into(),
        focused,
        x: i32::try_from(v["at"][0].as_i64().context("Missing x")?)?,
        y: i32::try_from(v["at"][1].as_i64().context("Missing y")?)?,
        width: u32::try_from(v["size"][0].as_u64().context("Missing width")?)?,
        height: u32::try_from(v["size"][1].as_u64().context("Missing height")?)?,
    };
    ensure!(w.width > 0 && w.height > 0, "Invalid window bounds");
    Ok(w)
}
#[cfg(target_os = "linux")]
fn validate_active_with(
    target: &Window,
    mut ipc: impl FnMut(&str) -> anyhow::Result<String>,
    identity: impl FnOnce(&serde_json::Value) -> anyhow::Result<String>,
) -> anyhow::Result<Window> {
...
fn validate_active_snapshot(
    ...
    if native_id(&current) == native_id(target)
        && (!same_identity(&current, target) || bounds(&current) != bounds(target))
    {
        revoke(target);
    }
...
fn hypr_lua_mode(reply: &str) -> anyhow::Result<bool> {
    match reply.trim() {
        "ok" => Ok(true),
        "eval is only supported with the lua config manager" | "unknown request" => {
            Ok(false)
        }
        other => anyhow::bail!(
            "Cannot determine Hyprland dispatch dialect: {}",
            other.chars().take(512).collect::<String>()
        ),
    }
}
#[cfg(any(target_os = "linux", test))]
fn hypr_focus_payload(id: &str, lua: bool) -> anyhow::Result<String> {
    ...
    Ok(if lua {
        format!("/dispatch hl.dsp.focus({{ window = \"address:{id}\" }})")
    } else {
...
fn hypr_ack(reply: &str) -> anyhow::Result<()> {
        ...
        reply.trim() == "ok",
        "Hyprland rejected focus request: {}",
        reply.chars().take(1024).collect::<String>()
    );
    Ok(())
}
...
fn process_start(pid: u32) -> anyhow::Result<String> {
    ...
    let mut user = birth;
    let ok = unsafe {
        GetProcessTimes(process, &mut birth, &mut exit, &mut kernel, &mut user)
    };
...
#[cfg(not(any(target_os = "linux", target_os = "macos", target_os = "windows")))]
fn process_start(_: u32) -> anyhow::Result<String> {
    anyhow::bail!("Process lifetime identity unsupported on this platform")
}
#[cfg(any(target_os = "macos", target_os = "windows"))]
fn native_list() -> anyhow::Result<Vec<Window>> {
    #[cfg(target_os = "windows")]
    let _dpi = super::platform::DpiGuard::new()?;
    #[cfg(target_os = "macos")]
    let focused = macos::focused_id()?;
    xcap::Window::all()?
        .into_iter()
        .map(|w| {
            Ok(Window {
                id: qualified_identity(&w.id()?.to_string(), w.pid()?)?,
                pid: w.pid()?,
                app: w.app_name()?,
                title: w.title()?,
                x: w.x()?,
...
fn native_focus(
    ...
    use windows_sys::Win32::UI::WindowsAndMessaging::SetForegroundWindow;
        ...
        unsafe { SetForegroundWindow(native_id(w).parse::<usize>()? as _) } != 0,
        "Windows rejected foreground activation"
    );
    Ok(())
}
#[cfg(target_os = "macos")]
...
mod macos {
    ...
    fn string(value: &std::ffi::CStr) -> anyhow::Result<Owned> {
        ...
        ensure!(!value.0.is_null(), "Cannot allocate AX attribute");
        Ok(value)
    }
    ...
    pub(super) fn exists(target: &Window) -> anyhow::Result<bool> {
        ...
        let array = Owned(unsafe {
            CGWindowListCopyWindowInfo(1 << 3, native_id(target).parse()?)
        });
        ...
        let mut pid = 0i32;
            ...
            unsafe { CFNumberGetValue(number, 3, (&mut pid as *mut i32).cast()) },
            "Cannot decode native window owner PID"
        );
    ...
    fn attribute(element: Ref, name: &std::ffi::CStr) -> anyhow::Result<Option<Owned>> {
        ...
        let status =
            unsafe { AXUIElementCopyAttributeValue(element, name.0, &mut value) };
        let value = Owned(value);
        Ok(if ax_optional_value(status, !value.0.is_null())? {
            Some(value)
        } else {
            None
        })
    ...
    pub(super) fn focused_id() -> anyhow::Result<Option<u32>> {
            ...
            "Accessibility permission required to query exact foreground window"
        );
        let system = Owned(unsafe { AXUIElementCreateSystemWide() });
        ensure!(!system.0.is_null(), "Cannot create AX system element");
        let Some(app) = attribute(system.0, c"AXFocusedApplication")? else {
            return Ok(None);
        };
        let Some(window) = attribute(app.0, c"AXFocusedWindow")? else {
            return Ok(None);
        };
        let mut id = 0;
        ensure!(
            unsafe { _AXUIElementGetWindow(window.0, &mut id) } == 0,
            "Cannot resolve exact AX window identity"
        );
        Ok(Some(id))
    }
    pub(super) fn focus(
        ...
        let status =
            unsafe { AXUIElementCopyAttributeValue(app.0, windows.0, &mut array) };
        let array = Owned(array);
        ensure!(
            status == 0 && !array.0.is_null(),
            "Cannot enumerate AX windows; grant Accessibility permission"
        );
        ...
        for index in 0..unsafe { CFArrayGetCount(array.0) } {
            ...
            let mut candidate = 0;
            if unsafe { _AXUIElementGetWindow(element, &mut candidate) } == 0
                && candidate == id
            {
                ...
                check()?;
                    ...
                    unsafe {
                        AXUIElementSetAttributeValue(app.0, front.0, kCFBooleanTrue)
                    } == 0,
                    "macOS rejected application foreground request"
                );
                check()?;
                ensure!(
                    unsafe { AXUIElementPerformAction(element, raise.0) } == 0,
                    "macOS rejected exact-window raise"
                );
                return Ok(());
            }
        }
...
#[cfg(not(any(target_os = "linux", target_os = "macos", target_os = "windows")))]
fn native_list() -> anyhow::Result<Vec<Window>> {
    anyhow::bail!("Window targeting unsupported")
}
#[cfg(not(any(target_os = "linux", target_os = "macos", target_os = "windows")))]
fn native_focus(
    _: &Window,
    _: &mut impl FnMut() -> anyhow::Result<()>,
) -> anyhow::Result<()> {
    anyhow::bail!("Window focus unsupported")
}
#[cfg(test)]
mod tests {
    use super::*;
    fn window() -> Window {
        Window {
            id: "0x123|5|birth1|stable1".into(),
            pid: 5,
            app: "editor".into(),
            title: String::new(),
            x: 0,
    ...
    #[test]
    fn unchanged_list_preserves_tokens_without_a_turn_timeout() {
        let mut tokens = Vec::new();
        let a = reconcile(&mut tokens, vec![window()], 7);
        let mut renamed = window();
        renamed.title = "renamed".into();
        renamed.focused = false;
        for _ in 0..100 {
    ...
    #[test]
    fn changed_lifetime_geometry_and_missing_windows_never_retarget() {
        for change in [
            "process", "stable", "pid", "app", "x", "y", "width", "height", "missing",
        ] {
            let mut tokens = Vec::new();
    ...
    #[test]
    fn cache_is_bounded_and_does_not_evict_unchanged_entries_on_reorder() {
        let mut tokens = Vec::new();
        let windows: Vec<_> = (0..MAX_TARGETS + 10)
            .map(|n| {
                let mut w = window();
                w.id = format!("0x{n:x}|birth");
                w
            })
            .collect();
        let a = reconcile(&mut tokens, windows.clone(), 1);
        assert_eq!(tokens.len(), MAX_TARGETS);
        let mut retained = windows[..MAX_TARGETS].to_vec();
        retained.reverse();
    ...
    fn client() -> serde_json::Value {
        serde_json::json!({"address":"0x123","pid":5,"stableId":"1a2b","mapped":true,"class":"editor","title":"private","at":[0,0],"size":[10,10]})
    }
    fn fixture_identity(v: &serde_json::Value) -> anyhow::Result<String> {
        hypr_identity_with(v, |_| Ok("birth1".into()))
    }
    #[test]
    fn focused_snapshot_requires_exact_identity_and_bounds() {
        let v = client();
        let target = hypr_window(&v, true, fixture_identity(&v).unwrap()).unwrap();
        assert!(
            validate_active_snapshot(&target, &v.to_string(), fixture_identity).is_ok()
        );
        for (field, value) in [
            ("address", serde_json::json!("0x124")),
            ("pid", serde_json::json!(6)),
            ("stableId", serde_json::json!(18)),
            ("class", serde_json::json!("other")),
            ("mapped", serde_json::json!(false)),
            ("at", serde_json::json!([1, 0])),
            ("size", serde_json::json!([10, 11])),
            ("size", serde_json::json!([0, 10])),
        ] {
            let mut changed = v.clone();
            changed[field] = value;
            assert!(
                validate_active_snapshot(&target, &changed.to_string(), fixture_identity)
                    .is_err(),
                "{field}"
            );
        }
        for field in [
            "address", "pid", "stableId", "class", "mapped", "at", "size",
        ] {
            let mut changed = v.clone();
        ...
        for bad in ["{}", "null", "[]", "not json"] {
            assert!(validate_active_snapshot(&target, bad, fixture_identity).is_err());
        }
        assert!(
            validate_active_snapshot(&target, &v.to_string(), |_| anyhow::bail!(
                "process access denied"
            ))
            .is_err()
        );
        assert!(validate_active_snapshot(&target, &v.to_string(), |v| {
            hypr_identity_with(v, |_| Ok("birth2".into()))
        })
        .is_err());
        let mut renamed = v.clone();
        renamed["title"] = serde_json::json!("new");
        assert!(validate_active_snapshot(
            &target,
            &renamed.to_string(),
            fixture_identity
        )
    ...
    #[test]
    fn another_active_window_is_focus_loss_not_identity_loss() {
        let target = window();
        let error = validate_active_snapshot(&target, r#"{"address":"0x999"}"#, |_| {
            panic!("unrelated window identity must not be inspected")
        })
        .unwrap_err();
    ...
    #[test]
    fn background_guard_preserves_lifetime_and_bounds_without_requiring_focus() {
        let mut tokens = Vec::new();
        let mut listed = window();
        listed.focused = false;
        ...
        assert!(validate_list(&target, true, vec![target.clone()])
            .unwrap_err()
            .to_string()
            .contains("not foreground"));

        let mut moved = target.clone();
        moved.x += 1;
        assert!(validate_list(&target, false, vec![moved])
            .unwrap_err()
            .to_string()
            .contains("moved/resized"));
        let mut reused = target.clone();
        reused.id = "0x123|5|birth2|stable2".into();
        assert!(validate_list(&target, false, vec![reused])
            .unwrap_err()
            .to_string()
            .contains("disappeared or identity changed"));
        assert!(validate_list(&target, false, vec![])
            .unwrap_err()
            .to_string()
            .contains("disappeared or identity changed"));
        ...
        assert_ne!(
            listed[0].0, reopened[0].0,
            "close/reopen must not resurrect the listed target"
        );
    }
    #[cfg(target_os = "linux")]
    #[test]
    fn each_focused_validation_uses_one_fresh_ipc_request() {
        let mut requests = Vec::new();
        let v = client();
        let target = hypr_window(&v, true, fixture_identity(&v).unwrap()).unwrap();
        for _ in 0..100 {
            assert!(validate_active_with(
                &target,
                |command| {
                    requests.push(command.to_owned());
                    Ok(v.to_string())
                },
                fixture_identity
            )
        ...
        assert!(
            validate_active_with(&target, |_| Ok("{}".into()), fixture_identity).is_err()
        );
        assert!(validate_active_with(
            &target,
            |_| anyhow::bail!("IPC denied"),
            fixture_identity
        )
        .is_err());
    }
    #[test]
    fn lifetime_evidence_is_strict_and_process_stat_handles_nested_comm() {
        let v = client();
        let strong = fixture_identity(&v).unwrap();
        let mut old = v.clone();
        old.as_object_mut().unwrap().remove("stableId");
        ...
        fields[19] = "12345";
        assert_eq!(
            proc_start(&format!("5 (name (with) spaces) {}", fields.join(" "))).unwrap(),
            12345
        );
        for bad in ["", "5 (name) S", "5 no paren"] {
            assert!(proc_start(bad).is_err());
        }
        assert_eq!(native_id(&window()), "0x123");
    }
    #[test]
    fn hyprland_focus_dialects_and_replies_are_exact_and_bounded() {
        assert!(hypr_lua_mode("ok\n").unwrap());
        assert!(
            !hypr_lua_mode("eval is only supported with the lua config manager").unwrap()
        );
        assert!(!hypr_lua_mode("unknown request").unwrap());
        assert!(hypr_lua_mode("permission denied").is_err());
        assert_eq!(
            hypr_focus_payload("0x123abc", false).unwrap(),
            "/dispatch focuswindow address:0x123abc"
        );
        assert_eq!(
            hypr_focus_payload("0x123abc", true).unwrap(),
            "/dispatch hl.dsp.focus({ window = \"address:0x123abc\" })"
        );
        ...
        for bad in [
            ...
            "[string]: syntax error near 'address'",
        ] {
    ...
    #[test]
    fn closed_uses_lifetime_not_visibility_or_focus() {
        let target = Window {
            id: "123".into(),
            pid: 5,
            app: "editor".into(),
        ...
        for _visibility in ["hidden", "minimized", "off-space", "no AX focus"] {
            assert!(!condition_with(
                &target,
                true,
                |_| Ok(true),
                || panic!("closed queried presentation list")
            )
            .unwrap());
        }
        assert!(condition_with(
        ...
        assert!(condition_with(
            &target,
            true,
            |_| anyhow::bail!("native probe denied"),
            || Ok(Vec::new())
        )
        .is_err());
        assert!(!condition_with(
            &target,
            false,
            |_| panic!("foreground queried lifetime"),
            || Ok(vec![target.clone()])
        )
        .unwrap());
    }
    #[test]
    fn mac_absent_focus_is_optional_but_permission_and_transport_errors_are_not() {
        ...
        for status in [-25211, -25204, -25205, -25202, -25200] {
            assert!(
                ax_optional_value(status, false).is_err(),
                "AX error {status} must propagate"
            );
        }
        assert!(
            ax_optional_value(0, false).is_err(),
            "malformed successful reply is not absent focus"
        );
    }
    #[test]
    fn identity_does_not_depend_on_title_but_rejects_reused_handle() {
        let a = Window {
            id: "123".into(),
            pid: 5,
            app: "editor".into(),
