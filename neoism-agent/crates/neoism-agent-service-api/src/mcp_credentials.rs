...
impl LocalMcpCredentialStore {
    ...
    pub fn from_environment() -> Self {
        let path = std::env::var_os("NEOISM_AGENT_MCP_AUTH_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|| {
                std::env::var_os("NEOISM_AGENT_STATE_DIR")
                    .map(PathBuf::from)
                    .unwrap_or_else(|| {
    ...
    fn read(&self) -> Result<LocalDocument, ServiceError> {
        ...
        decode_local_document(&content).map_err(|error| {
    ...
    fn mutate<T>(
        &self,
        operation: impl FnOnce(&mut LocalDocument) -> Result<T, ServiceError>,
    ) -> Result<T, ServiceError> {
        let _guard = self
            .mutation
            .lock()
            .map_err(|_| ServiceError::new("MCP credential store lock poisoned"))?;
...
struct LocalDocument {
    #[serde(default)]
    credentials: BTreeMap<String, LocalCredential>,
    #[serde(default)]
    attempts: BTreeMap<String, McpOAuthAttempt>,
}
...
fn ensure_local_scope(scope: &CredentialScope) -> Result<(), ServiceError> {
    if scope == &CredentialScope::local() {
        Ok(())
    } else {
        Err(ServiceError::new(
            "local MCP credential store cannot serve hosted tenant/workspace scopes",
        ))
    }
}
...
fn decode_local_document(content: &str) -> Result<LocalDocument, serde_json::Error> {
    ...
    for (connection_id, entry) in legacy {
        ...
        let credential = McpCredential {
            tokens,
            client_registration: entry.client_info,
            scopes,
        };
        document.credentials.insert(
            local_key(&CredentialScope::local(), &connection),
            LocalCredential {
                connection: connection.clone(),
                credential,
            },
        );
        if let (Some(state), Some(code_verifier), Some(redirect_uri), Some(directory)) = (
            entry.oauth_state,
            entry.code_verifier,
            entry.oauth_redirect_uri,
            entry.oauth_directory,
        ) {
...
impl McpCredentialStore for LocalMcpCredentialStore {
    ...
    fn get<'a>(
        ...
        Box::pin(async move {
            ...
            Ok(self
                .read()?
                .credentials
                .get(&local_key(scope, connection))
        ...
        Box::pin(async move {
            ...
            self.mutate(|document| {
                let removed = document
                    .credentials
                    .remove(&local_key(scope, connection))
                    .is_some();
                document.attempts.retain(|_, attempt| {
                    &attempt.scope != scope || &attempt.connection != connection
                });
                Ok(removed)
            })
        })
    ...
    fn put_attempt<'a>(
        ...
        Box::pin(async move {
            ...
            self.mutate(|document| {
                document.attempts.retain(|_, old| {
                    old.scope != attempt.scope || old.connection != attempt.connection
                });
    ...
    fn consume_attempt<'a>(
        ...
        Box::pin(async move {
            ...
            self.mutate(|document| {
                let matches_scope = document.attempts.get(state).is_some_and(|attempt| {
                    scope.is_none_or(|scope| &attempt.scope == scope)
                });
                if !matches_scope {
                    return Ok(None);
                }
...
mod tests {
    ...
    fn path(name: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "neoism-mcp-credentials-{name}-{}",
            std::process::id()
        ))
    }
    fn connection(name: &str, url: &str) -> McpConnectionRef {
        McpConnectionRef {
            connection_id: name.into(),
            server_url: url.into(),
        }
    }
    fn credential(secret: &str) -> McpCredential {
        McpCredential {
    ...
    async fn local_store_is_url_scoped_and_owner_only() {
        ...
        store
            .put(
                &scope,
                &connection("tools", "https://a/mcp"),
                credential("secret"),
            )
            .await
            .unwrap();
        assert!(store
            .get(&scope, &connection("tools", "https://b/mcp"))
            .await
            .unwrap()
            .is_none());
        #[cfg(unix)]
    ...
    async fn local_store_migrates_legacy_shape() {
        ...
        let value = store
            .get(
                ...
                &connection("github", "https://example/mcp"),
            )
            .await
            .unwrap()
            .unwrap();
    ...
    async fn local_store_fails_closed_for_hosted_scope() {
        ...
        let hosted = CredentialScope {
            tenant_id: "tenant-a".into(),
            workspace_id: Some("workspace-a".into()),
        };
        assert!(store
            .get(&hosted, &connection("github", "https://example/mcp"))
            .await
            .is_err());
    }
    ...
    async fn oauth_attempt_is_scoped_expiring_and_one_use() {
        ...
        store
            .put_attempt(McpOAuthAttempt {
                scope: scope.clone(),
                connection: connection("x", "https://x/mcp"),
                state: "nonce".into(),
            ...
            .unwrap();
        assert!(store
            .consume_attempt("nonce", Some(&scope))
            .await
            .unwrap()
            .is_some());
        assert!(store
