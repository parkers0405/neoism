...
impl LspPlugin {
    pub fn new(host: Arc<dyn LspHost>) -> Self {
        Self { host }
    }
}
...
impl PluginDefinition for LspPlugin {
    fn manifest(&self) -> PluginManifest {
        PluginManifest {
            ...
            internal: true,
            disableable: true,
            capabilities: vec!["neoism.lsp".into()],
            requires: Vec::new(),
            event_namespaces: vec!["lsp".into()],
    ...
    fn required_capabilities(&self) -> Vec<neoism_agent_plugin_api::HostCapability> {
        ...
        vec![WorkspaceRead, ProcessSpawn]
    }
    fn contributions(
        &self,
        ...
        for (id, method, suffix, action) in [
            ...
            (
                ...
                RouteMethod::Get,
                "/inlay-hints",
                LspAction::InlayHints,
            ),
            (
                "v2.plugins.lsp.documentHighlights",
                RouteMethod::Get,
                "/document-highlights",
                LspAction::DocumentHighlights,
            ),
            (
