...
pub fn config_descriptors() -> Vec<D> {
    let mut rows = vec![
        ...
        d(
            "editor.git-blame",
            "Inline Git blame",
            "Show a GitHub avatar, author and relative age after the current code line; hover for commit details. Fetches public GitHub avatars only while enabled.",
            Kind::Boolean,
            json!(false),
            &[],
            false,
        ...
        d(
            "editor.git-blame-delay-ms",
            "Inline blame delay (ms)",
            "Hide attribution until the cursor settles. 0 shows immediately; 250 enables a quarter-second delay.",
            Kind::Integer, json!(0), &[], false, C::Editor, Control::Number,
        ),
        d(
            "editor.git-blame-hide-on-scroll",
            "Hide inline blame while scrolling",
            "Hide during scrolling, then restore 150ms after the viewport settles. Independent of cursor delay.",
            Kind::Boolean, json!(false), &[], false, C::Editor, Control::Toggle,
        ),
        d(
            "editor.minimap",
            "Minimap",
