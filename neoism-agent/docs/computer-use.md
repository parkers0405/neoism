...
- Some games, including the tested Minecraft 26.2 client, do not honor temporary literal keymaps: `text` can produce physical-position characters instead. Use ordinary `key` events (including Linux punctuation) against the original layout, and verify the complete command before Enter. Do not retry corrupted text or silently switch to clipboard paste.
