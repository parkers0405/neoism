...
struct Compositor<'a> {
    ...
    next_id: u64,
    ime_device: Option<u64>,
    ime_map: Option<String>,
    ime_state: xkb::State,
    calls: Vec<&'static str>,
    output: Vec<(String, bool)>,
    parser: &'a mut dyn FnMut(&str) -> anyhow::Result<()>,
    failure: u8,
    fail_sync_at: usize,
    sync_count: usize,
}
impl<'a> Compositor<'a> {
    fn new(source: &str, parser: &'a mut dyn FnMut(&str) -> anyhow::Result<()>) -> Self {
        Self {
            ...
            current: source.into(),
            state: xkb::State::new(&compile(source.into()).unwrap()),
            pending: Vec::new(),
            returned: Vec::new(),
            injector: None,
            next_id: 1,
            ime_device: None,
            ime_map: None,
            ime_state: xkb::State::new(&compile(source.into()).unwrap()),
            calls: Vec::new(),
            output: Vec::new(),
            parser,
            failure: 0,
    ...
    fn assert_restored(&self) {
        ...
        assert!(
            ...
            "returning with queued cleanup is not restoration"
        );
        assert!(
            self.returned.is_empty(),
            "Fcitx return updates must also be drained"
        );
        assert_eq!(self.state.serialize_mods(xkb::STATE_MODS_EFFECTIVE), 0);
        assert_eq!(
            &self.calls[self.calls.len() - 5..],
            &["map", "mods", "sync", "destroy", "sync"]
        );
    }
}
impl Wire for Compositor<'_> {
    fn map(&mut self, source: &str) -> anyhow::Result<()> {
        (self.parser)(source)?;
        self.calls.push("map");
        self.pending.push(Pending::Map(source.into()));
        Ok(())
    }
    fn restore(&mut self, source: &str) -> anyhow::Result<()> {
        (self.parser)(source)?;
        self.calls.push("map");
        self.pending.push(Pending::Restore(source.into()));
        Ok(())
    }
    fn mods(&mut self, d: u32, l: u32, k: u32, g: u32) -> anyhow::Result<()> {
        self.calls.push("mods");
        self.pending.push(Pending::Mods(d, l, k, g));
        Ok(())
    }
    fn key(&mut self, code: u32, down: bool) -> anyhow::Result<()> {
        self.calls.push(if down { "down" } else { "up" });
        self.pending.push(Pending::Key(code, down));
        if down && self.failure != 0 {
            let failure = std::mem::take(&mut self.failure);
            if failure == 2 {
                panic!("in-flight key panic");
            }
    ...
    fn destroy(&mut self) -> anyhow::Result<()> {
        ...
        let injector = self
            .injector
            .as_ref()
            .expect("destroy requires a live injector");
        ...
        self.pending.push(Pending::Destroy);
        Ok(())
    }
    fn sync(&mut self) -> anyhow::Result<()> {
        ...
        for event in std::mem::take(&mut self.returned) {
            match event {
                Returned::Map(source) => {
                    self.state = xkb::State::new(&compile(source.clone())?);
                    self.current = source;
                }
                Returned::Mods(d, l, k, g) => {
                    self.state.update_mask(d, l, k, 0, 0, g);
                }
        ...
        for event in std::mem::take(&mut self.pending) {
            match event {
                ...
                Pending::Mods(d, l, k, g) => {
                    self.injector
                        .as_mut()
                        .expect("modifiers need an injector")
                        .state
                        .update_mask(d, l, k, 0, 0, g);
                    self.route_keyboard()?;
                ...
                Pending::Key(code, down) => {
                    ...
                    if down {
                        ...
                        let control = self.ime_state.mod_name_is_active(
                            xkb::MOD_NAME_CTRL,
                            xkb::STATE_MODS_EFFECTIVE,
                        );
                        let text = match sym.raw() {
                            0xff0d => "<Return>".into(),
                            0xffe3 => "<Control>".into(),
                            _ => xkb::keysym_to_utf8(sym),
                        };
            ...
            self.sync_count != self.fail_sync_at,
            "simulated missing compositor ACK"
        );
        Ok(())
    }
}
...
fn chord(map: &xkb::Keymap, keys: &[Key]) -> Vec<u32> {
    super::super::shortcuts::resolve_in_map(map, 0, keys)
        .unwrap()
        .into_iter()
        .map(u32::from)
        .collect()
...
pub(crate) fn sequence(mut parser: impl FnMut(&str) -> anyhow::Result<()>) {
    for layout in ["us", "de", "fr"] {
        ...
        for url in [
            "https://www.google.com/search?q=Neoism+AA!!",
            "https://example.com/?q=λ❤é&&x=RepeatRepeat",
        ] {
            ...
            assert!(
                validate_baseline(&overlay_map).is_err(),
                "must not snapshot our overlay as original"
            );
            let from = wire.output.len();
            transaction(&mut wire, &original, &overlay, 0, &codes, false, || Ok(()))
                .unwrap();
            assert_eq!(
                wire.output[from..]
                    .iter()
            ...
            transaction(&mut wire, &original, &original, 0, &enter, true, || Ok(()))
                .unwrap();
            assert_eq!(wire.output.last(), Some(&("<Return>".into(), false)));
            wire.assert_restored();
            let control_l = chord(&base, &[Key::Control, Key::Unicode('l')]);
            transaction(&mut wire, &original, &original, 0, &control_l, true, || {
                Ok(())
            })
            .unwrap();
        ...
        for failure in [0, 1, 2] {
            ...
            let from = wire.output.len();
            let result =
                transaction(&mut wire, &original, &overlay, 0, &codes, false, || {
                    checks += 1;
                    ensure!(
                        failure != 0 || checks < 3,
                        "cancelled after first character"
                    );
                    Ok(())
                });
            ...
            wire.assert_restored();
            assert_eq!(
                wire.output.len() - from,
                1,
                "no subsequent text after failure/cancel"
            );
            let enter = chord(&base, &[Key::Return]);
            transaction(&mut wire, &original, &original, 0, &enter, true, || Ok(()))
                .unwrap();
        ...
        assert!(transaction(
            ...
            || {
                checks += 1;
                ensure!(checks < 3, "cancel after Control down");
                Ok(())
            }
        )
        .is_err());
...
#[test]
fn ime_identity_cache_requires_a_fresh_restore_device() {
    let base = desktop("us");
    let original = base.get_as_string(xkb::KEYMAP_FORMAT_TEXT_V1);
    let (overlay, _) = text_overlay(&base, "abc").unwrap();
    let mut parser = |source: &str| {
        compile(source.into())?;
        Ok(())
    };
    let mut wire = Compositor::new(&original, &mut parser);
    wire.map(&overlay).unwrap();
    wire.mods(0, 0, 0, 0).unwrap();
    wire.sync().unwrap();
    let old_id = wire.injector.as_ref().unwrap().id;
    assert_eq!(
        wire.current, original,
        "injector ACK does not drain Fcitx's new return requests"
    );
    wire.map(&original).unwrap();
    wire.mods(0, 0, 0, 0).unwrap();
    wire.sync().unwrap();
    ...
    assert_eq!(
        wire.current, overlay,
        "same-device restore is invisible to the IME"
    );
    wire.restore(&original).unwrap();
    wire.mods(0, 0, 0, 0).unwrap();
    wire.sync().unwrap();
    ...
    assert_eq!(
        wire.current, overlay,
        "replacement ACK still precedes Fcitx's return"
    );
    wire.destroy().unwrap();
    wire.sync().unwrap();
    wire.assert_restored();
    assert!(
        ...
        "restoration must never manufacture a keypress"
    );
    assert!(
        !wire.calls.contains(&"down"),
        "no keydown even before asynchronous delivery"
    );
}

#[test]
fn replacement_without_modifiers_does_not_restore_ime_or_surviving_vk() {
    let base = desktop("us");
    let original = base.get_as_string(xkb::KEYMAP_FORMAT_TEXT_V1);
    ...
    assert_eq!(wire.injector.as_ref().unwrap().map, original);
    assert_eq!(
        wire.ime_device, cached_id,
        "replacement map alone cannot refresh grab identity"
    );
    assert_eq!(wire.ime_map.as_deref(), Some(overlay.as_str()));
    wire.destroy().unwrap();
    wire.sync().unwrap();
    ...
    assert_eq!(
        wire.current, overlay,
        "destroy must not invent a physical-map fallback"
    );
    assert_eq!(wire.ime_map.as_deref(), Some(overlay.as_str()));
...
#[test]
fn routed_key_refreshes_identity_but_map_publication_alone_does_not() {
    let base = desktop("us");
    let original = base.get_as_string(xkb::KEYMAP_FORMAT_TEXT_V1);
    ...
    wire.key(codes[0], true).unwrap();
    wire.key(codes[0], false).unwrap();
    wire.sync().unwrap();
    ...
    assert_eq!(
        wire.output,
        vec![("λ".into(), false)],
        "restore adds no input"
    );
}

#[test]
fn focus_loss_mid_unicode_text_stops_and_restores_keyboard() {
    let base = desktop("us");
    let original = base.get_as_string(xkb::KEYMAP_FORMAT_TEXT_V1);
    ...
    let result = transaction(&mut wire, &original, &overlay, 0, &codes, false, || {
        ...
        // before character two. Production calls windows::validate here.
            ...
            checks < 3,
            "Target is not foreground (Neoism replaced Helium)"
        );
        Ok(())
    });
...
#[test]
fn native_text_enter_control_l_text_enter_and_error_restoration() {
    sequence(|source| {
        compile(source.into())?;
        Ok(())
    });
...
#[test]
fn old_text_only_map_cannot_be_a_restore_baseline() {
    ...
    assert!(
        validate_baseline(&unnamed).is_err(),
        "legacy unnamed text maps are also not valid baselines"
    );
    let map = compile(source).unwrap();
    assert!(super::super::shortcuts::resolve_in_map(&map, 0, &[Key::Return]).is_err());
    assert!(validate_baseline(&map)
        .unwrap_err()
        .to_string()
        .contains("temporary Neoism keymap"));
}
#[test]
fn unicode_chunks_use_only_browser_recognized_positions() {
    for layout in ["us", "de", "fr", "us,de"] {
        let base = desktop(layout);
        let chars = (0..512)
            .map(|i| char::from_u32(0x400 + i).unwrap())
            .collect::<String>();
        ...
        for chunk in chunks {
            ...
            for group in 0..base.num_layouts() {
                ...
                for (ch, code) in chunk.chars().zip(&codes) {
                    ...
                    assert_eq!(
                        state.key_get_utf8(xkb::Keycode::new(*code)),
                        ch.to_string()
                    );
                }
            }
        ...
        assert!(
            text_overlay(&base, &chars).is_err(),
            "one map must not exceed the recognized pool"
        );
    }
}
#[test]
fn text_preflight_rejects_implicit_control_keys_and_preserves_unicode_boundaries() {
    ...
    assert_eq!(
        calls.get(),
        0,
        "unsupported text must fail before connecting or dispatching its prefix"
    );
...
#[test]
fn cancellation_after_a_chunk_reports_partial_input_without_starting_the_next() {
    let base = desktop("us");
    let original = base.get_as_string(xkb::KEYMAP_FORMAT_TEXT_V1);
    let prepared = vec![
        text_overlay(&base, "first").unwrap(),
        text_overlay(&base, "second").unwrap(),
    ];
    ...
    let mut wire = Compositor::new(&original, &mut parser);
    let error = run_chunks(
        &prepared,
        || {
            ensure!(completed.get() == 0, "cancelled");
            Ok(())
        },
        |source, codes, check| {
...
#[test]
fn literal_overlay_neutralizes_custom_printable_modifier_actions() {
    ...
    for group in 0..2 {
        ...
        assert_eq!(
            map.key_get_syms_by_level(xkb::Keycode::new(24), group, 0),
            base.key_get_syms_by_level(xkb::Keycode::new(24), group, 0)
        );
    }
    let mut parser = |source: &str| {
        compile(source.into())?;
...
#[test]
fn chunked_transactions_restore_before_followup_shortcuts() {
    let base = desktop("us");
    let original = base.get_as_string(xkb::KEYMAP_FORMAT_TEXT_V1);
    let text = (0..120)
        .map(|i| char::from_u32(0x400 + i).unwrap())
    ...
    let prepared = text_chunks(&text, literal_keycodes(&base).len())
        .unwrap()
        .into_iter()
        .map(|chunk| text_overlay(&base, chunk))
...
fn assert_original_text(layout: &str, group: u32, text: &str) {
    ...
    for (ch, unit) in text.chars().zip(&plan.units) {
        ...
        assert_eq!(
            state.key_get_utf8(xkb::Keycode::new(*unit.last().unwrap())),
            ch.to_string()
        );
...
#[test]
fn original_us_de_fr_case_punctuation_and_level3() {
    assert_original_text(
        "us",
        0,
        "Hello AaZz 0123456789 !@#$%^&*()_+-=[]{};:'\",.<>/?\\|`",
    );
    assert_original_text("de", 0, "Hallo ÄäÖöÜüß @€{}[]\\|~");
    assert_original_text("fr", 0, "Bonjour AaZz éèàç @€{}[]\\|~");
}
#[test]
fn original_group_only_and_dead_keys_not_composed() {
    let map = original_layout("us,de");
    assert!(layout_plan::text(&map, 0, "ä").unwrap().is_none());
    assert_original_text("us,de", 1, "äÄ@€");
    assert!(layout_plan::text(&original_layout("us"), 0, "café")
        .unwrap()
        .is_none());
    assert!(layout_plan::text(&original_layout("de"), 0, "ê")
        .unwrap()
        .is_none());
}
#[test]
fn original_full_preflight_validates_suffix_and_unicode_policy() {
    let map = original_layout("us");
    for text in ["prefixλ", "prefix😀", "e\u{301}", "a\u{200d}"] {
        assert!(layout_plan::text(&map, 0, text).unwrap().is_none());
    }
    for text in ["prefix\0", "prefix\n", "prefix\t", "prefix\u{85}", "😀\n"] {
        assert!(layout_plan::text(&map, 0, text).is_err());
    }
    assert_eq!(
        layout_plan::text(&map, 0, &"a".repeat(512))
            .unwrap()
            .unwrap()
            .unit_count(),
        512
...
#[test]
fn original_custom_modifier_poison_rejected() {
...
struct PlannedWire {
    ...
    panic_guard: usize,
    acknowledged: usize,
    facts: KeyboardFailureFacts,
}
impl StrokeWire for PlannedWire {
    fn completed(&mut self) {
        self.facts.completed_units += 1;
        self.facts.current_unit_uncertain = false;
    }
    fn guard(
        &mut self,
        ...
        check()?;
        self.guards += 1;
        if self.guards == self.panic_guard {
    ...
    fn stroke(&mut self, code: u32, down: bool, _: &xkb::State) -> anyhow::Result<()> {
        if down {
            self.held.push(code);
            self.facts.current_unit_uncertain = true;
        } else {
            assert_eq!(self.held.pop(), Some(code));
        }
        self.events.push((code, down));
        Ok(())
    }
}
#[test]
fn original_dispatch_is_per_scalar_chords_with_release_ack() {
    let map = original_layout("de");
    let plan = layout_plan::text(&map, 0, "aA@b").unwrap().unwrap();
    let mut wire = PlannedWire::default();
    let mut completed = Vec::new();
    dispatch_plan(&mut wire, &map, &plan, &mut || Ok(()), &mut |n| {
        completed.push(n)
    })
    .unwrap();
    ...
    let expected = plan
        ...
        .flat_map(|unit| {
            unit.iter()
                .map(|c| (*c, true))
                .chain(unit.iter().rev().map(|c| (*c, false)))
        })
        .collect::<Vec<_>>();
    assert_eq!(wire.events, expected);
}
#[test]
fn original_dispatch_stops_on_changed_snapshot_without_retrying_prefix() {
    let map = original_layout("us");
    let plan = layout_plan::text(&map, 0, "ab").unwrap().unwrap();
    let mut wire = PlannedWire {
        fail_guard: 4,
        ..Default::default()
    };
    let mut completed = Vec::new();
    assert!(
        dispatch_plan(&mut wire, &map, &plan, &mut || Ok(()), &mut |n| completed
            .push(n))
        .is_err()
    );
...
#[test]
fn original_dispatch_cancel_or_panic_before_release_never_claims_completion() {
    let map = original_layout("us");
    let plan = layout_plan::text(&map, 0, "Ab").unwrap().unwrap();
    for panic in [false, true] {
        let mut wire = PlannedWire {
            panic_guard: if panic { 3 } else { 0 },
            ..Default::default()
        };
        let mut checks = 0;
        let mut completed = Vec::new();
        let error = catch_native(|| {
            dispatch_plan(
                ...
                &mut || {
                    checks += 1;
                    ensure!(panic || checks != 3, "cancelled");
                    Ok(())
                },
                &mut |n| completed.push(n),
            )
        });
        assert!(error.is_err());
        ...
        assert_eq!(
            wire.held.len(),
            2,
            "session cleanup must release both owned keys"
        );
...
#[test]
fn original_failure_facts_survive_context_without_string_matching() {
    let facts = KeyboardFailureFacts {
        completed_units: 2,
        current_unit_uncertain: true,
        cleanup_failed: true,
    };
    let error = keyboard_error(anyhow::anyhow!("arbitrary transport error"), facts)
        .context("outer facade context");
    assert_eq!(failure_facts(&error), Some(facts));
    assert_eq!(failure_facts(&anyhow::anyhow!("cleanup failed")), None);
}
#[test]
fn original_guard_before_first_key_has_no_native_effects() {
    let map = original_layout("us");
    let plan = layout_plan::text(&map, 0, "Ab").unwrap().unwrap();
    let mut wire = PlannedWire {
        fail_guard: 1,
    ...
    assert!(
        dispatch_plan(&mut wire, &map, &plan, &mut || Ok(()), &mut |_| panic!(
            "must not report progress"
        ))
        .is_err()
    );
...
#[test]
fn original_progress_panic_after_ack_does_not_make_unit_uncertain() {
    let map = original_layout("us");
    let plan = layout_plan::text(&map, 0, "Ab").unwrap().unwrap();
    let mut wire = PlannedWire::default();
    assert!(catch_native(|| dispatch_plan(
        &mut wire,
        &map,
        &plan,
        &mut || Ok(()),
        &mut |_| panic!("progress panic")
    ))
    .is_err());
    assert!(wire.held.is_empty());
    assert_eq!(
...
#[test]
fn original_all_ascii_uses_real_printing_positions_not_consumer_aliases() {
    let text = (0x20..0x7f)
    ...
    assert_eq!(
        *plan.units[0].last().unwrap(),
        12,
        "# must be Shift+3, not XF86NumericPound"
    );
    assert_eq!(
        *plan.units[1].last().unwrap(),
        17,
        "* must be Shift+8, not XF86NumericStar"
    );
    ...
    for layout in ["us", "de", "fr"] {
        ...
        for ch in text.chars() {
            if let Some(plan) = layout_plan::text(&map, 0, &ch.to_string()).unwrap() {
                assert!(
                    plan.units[0].iter().all(|code| *code <= 135),
                    "{layout} {ch:?}: {:?}",
                    plan.units
                );
            }
...
#[test]
fn original_consumer_alias_on_printing_position_is_not_direct_text() {
    let map = original_layout("us");
    let mut source = map.get_as_string(xkb::KEYMAP_FORMAT_TEXT_V1);
    append_section(
        &mut source,
        "xkb_symbols",
        "\n replace key <AC01> { [ XF86NumericPound ] };",
    )
    .unwrap();
    let map = compile(source).unwrap();
    let plan = layout_plan::text(&map, 0, "#").unwrap().unwrap();
...
fn disconnected_session(with_device: bool) -> KeyboardSession {
    ...
    let keyboard = if with_device {
        ...
        let manager = registry.bind::<manager::ZwpVirtualKeyboardManagerV1, _, _>(
            2,
            1,
            &queue.handle(),
            (),
    ...
    KeyboardSession {
        ...
        facts: KeyboardFailureFacts::default(),
    }
}
#[test]
fn original_finish_without_device_is_idempotent_and_closes_session() {
...
fn original_finish_reports_native_cleanup_failure_without_inventing_pending_text() {
    ...
    assert_eq!(
        failure_facts(&session.finish().unwrap_err()),
        Some(expected)
    );
}

#[test]
fn original_identical_fd_notifications_reuse_compilation_but_remain_fresh() {
    ...
    for _ in 0..2048 {
        state.record_map_source(super::super::shortcuts::read_map_source(
            &file,
            (source.len() + 1) as u32,
        ));
    ...
    println!(
        "2048 fresh original-map FD observations: {:?}, {} compilation",
        start.elapsed(),
        state.compiled_maps
    );
    state.group = Some(1);
    state.group_revision = 19;
    state.record_map_source(Ok(source));
    assert_eq!(state.map_revision, 2049);
    assert_eq!(state.group, Some(1));
    assert_eq!(
        state.group_revision, 19,
        "map cache must not manufacture group freshness"
    );
}
#[test]
fn original_map_cache_recompiles_changes_and_invalidates_errors() {
    ...
    assert!(
        state.map.is_none() && state.canonical_map.is_none() && state.raw_map.is_none()
    );
    assert!(state.error.is_some());
    state.record_map_source(Ok(de));
    assert_eq!(
        state.compiled_maps, 3,
        "error cannot leave a cache entry that bypasses revalidation"
    );
    assert!(state.error.is_none());
...
#[test]
fn original_enumerated_candidates_preserve_deterministic_chord_cost() {
    let ascii = (0x20..0x7f)
    ...
    for (ch, unit) in ascii.chars().zip(&full.units) {
        let single = layout_plan::text(&map, 0, &ch.to_string())
            .unwrap()
            .unwrap();
        assert_eq!(&single.units[0], unit);
    }
    let repeated = layout_plan::text(&map, 0, &"A".repeat(512))
        .unwrap()
        .unwrap();
    assert_eq!(repeated.unit_count(), 512);
    assert!(repeated.units.iter().all(|unit| unit == &vec![50, 38]));
    assert_eq!(
        layout_plan::text(&map, 0, "#").unwrap().unwrap().units,
        vec![vec![50, 12]]
    );
}
...
fn pending_sync_callback_has_zero_subsequent_blocking_polls() {
    ...
    for _ in 0..64 {
        ...
        assert!(
            ...
            "callback must be pending, not dispatched"
        );
        pump_until(
            &conn,
            &mut queue,
    ...
    assert_eq!(
        polls, 0,
        "dispatching completion must not enter a blocking poll"
    );
...
fn cancellation_during_read_only_sync_wait_uses_only_cheap_check() {
    ...
    let mut full = 0;
    let mut wait = || {
        super::super::latency::count("test_wait_checks", 1);
        waits += 1;
        ensure!(waits < 2, "cancelled while waiting");
        Ok(())
    };
...
fn authoritative_focus_loss_before_edge_does_not_touch_device() {
    ...
    for down in [true, false] {
        ...
        let mut full = || {
            full_calls += 1;
            anyhow::bail!("foreground changed")
        };
        let mut checks = Checks {
            wait: &mut cheap,
            full: &mut full,
        };
        let mut wire = CheckedStroke {
            session: &mut session,
            checks: &mut checks,
        };
        ...
        let error = wire
            .stroke(38, down, &xkb::State::new(&original))
            .unwrap_err();
