...
mod tests {
    ...
    fn typesafe_mode_setup_and_privacy_are_bundled() {
        ...
        for required in [
            "off-by-default",
            "TYPESAFE_API_KEY",
            "computer.browser_goal",
            "computer.browser_step",
            "step/time budget",
            "partial_unknown",
            "DOM",
            ...
            assert!(
                ...
                "missing TypeSafe guidance: {required}"
            );
        }
        assert!(bundled_doc("Neoism Agent/Computer Use.md")
            .unwrap()
            .body
            .contains("[[TypeSafe Browser Mode]]"));
    }
