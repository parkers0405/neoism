...
fn grep_workspace(
    ...
    if request.path.is_file() {
        return streaming_grep(request, "exact file requested");
    }
    if request.include_hidden && request.path.is_dir() {
        return streaming_grep(request, "hidden-file discovery requested");
    }
    if broad_root(&request.root) {
    ...
    match registry.with_picker(&request.root, |picker| {
        ...
        let items = results
            ...
            .filter_map(|item| {
                ...
                if !discoverable_path(&path, request.include_hidden) {
                    return None;
                }
        ...
        Ok((mut items, files, searched, next, mode))
            if !items.is_empty() || searched > 0 =>
        {
            let overflow = items.len() > request.limit;
            ...
            Ok(GrepWorkspaceResult {
                bounds: WorkspaceSearchBounds {
                    ...
                    truncated: next != 0 || overflow || items.len() >= request.limit,
                    timed_out: false,
                },
                items,
    ...
    builder
        ...
        .filter_entry(move |entry| {
            relative_path(&filter_root, entry.path())
                .is_none_or(|path| !filter_excluded.matches(&path))
        });
    let wanted = request
        .offset
...
fn streaming_grep(
    ...
    for path in paths {
        ...
        let mut before = VecDeque::<(u64, String)>::new();
        ...
        loop {
            ...
            if matcher.matches(&line) {
                ...
                for (number, text) in &before {
                    if *number > last && items.len() <= request.limit {
                        items.push(grep_match(&relative, *number, text, None));
                        last = *number;
                    }
                }
                if line_no > last && items.len() <= request.limit {
                    items.push(grep_match(
                        &relative,
                        line_no,
                        &line,
                ...
                if line_no > last && items.len() <= request.limit {
                    items.push(grep_match(&relative, line_no, &line, None));
                    last = line_no;
                }
                after -= 1;
            }
            if request.context_lines > 0 {
                ...
                while before.len() > request.context_lines {
                    before.pop_front();
                }
            }
        }
        if timed_out || items.len() > request.limit {
            break;
        }
    }
    let overflow = items.len() > request.limit;
    ...
    Ok(GrepWorkspaceResult {
        bounds: WorkspaceSearchBounds {
            ...
            total_at_least: items.len(),
            next_cursor: None,
            truncated: timed_out || overflow,
            timed_out,
        },
        items,
        files_with_matches: files.len(),
        total_files_searched: searched,
        mode: matcher.label().into(),
        engine: Some(STREAMING_ENGINE_ID.into()),
        fallback_reason: Some(reason.into()),
    })
}
...
impl LineMatcher {
    ...
    fn matches(&self, line: &str) -> bool {
        match self {
            ...
            Self::Fuzzy(n, c) => fuzzy_match(line, n, *c),
        }
    }
...
fn fuzzy_match(haystack: &str, needle: &str, case: bool) -> bool {
    let (h, n) = if case {
    ...
    n.chars().all(|wanted| chars.by_ref().any(|c| c == wanted))
}
