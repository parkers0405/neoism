...
fn joined_legacy_listing_keeps_host_path_bytes_through_nested_reads() {
    ...
    assert!(tree.handle_service_reply(
        707,
        &serde_json::json!({
    ...
    for name in ["main 文.rs", "literal\\backslash.rs"] {
        ...
        assert_eq!(
            neoism_ui::editor::crdt::presence_buffer_id_for_path(path),
            host.join(&read).buffer_id()
        );
    }
}
...
fn joined_unix_backslash_directory_does_not_alias_nested_directory() {
    ...
    harness.run(|ctx| {
        tree.open_dir(Path::new("/host/literal"), ctx);
    });
    assert!(tree.handle_service_reply(
        708,
        &serde_json::json!([
    ...
    harness.run(|ctx| {
        tree.open_dir(Path::new("/host/literal/dir"), ctx);
    });
    assert!(tree.handle_service_reply(
        708,
        &serde_json::json!([
    ...
    assert_eq!(
        row.path.as_ref().unwrap().to_str().unwrap(),
        "/host/literal/dir/main.rs"
    );
