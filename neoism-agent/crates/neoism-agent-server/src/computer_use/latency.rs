...
use std::{cell::RefCell, collections::BTreeMap, time::{Duration, Instant}};
...
impl Scope {
    pub(super) fn start(queue: Duration)->Self {
        let previous=CURRENT.with(|current|current.replace(Some(Measurements::default())));
        Self {previous,started:Instant::now(),queue,finished:false}
    }
    pub(super) fn finish(mut self)->Snapshot {
        let worker=self.started.elapsed();
        let stats=CURRENT.with(|current|current.replace(self.previous.take())).unwrap_or_default();
        self.finished=true;
        Snapshot {queue:self.queue,worker,stages:stats.stages,counters:stats.counters}
    }
}
impl Drop for Scope {
    fn drop(&mut self) {
        if !self.finished {CURRENT.with(|current|{current.replace(self.previous.take());});}
    }
}
