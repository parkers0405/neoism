...
#[derive(Clone, Copy)]
pub enum PtyAction { Shells, List, Create, Get, Update, Remove, ConnectToken }

pub trait PtyHost: Send + Sync + 'static {
    fn execute<'a>(&'a self, action: PtyAction, request: RouteRequest) -> PluginFuture<'a, RouteResponse>;
    fn connect<'a>(&'a self, request: RouteRequest) -> PluginFuture<'a, Arc<dyn WebSocketSession>>;
}
...
impl PluginDefinition for PtyPlugin {
    ...
    fn contributions(&self, registrar: &mut PluginContributions) -> Result<(), PluginHostError> {
        for (id, method, suffix, action) in [
            ...
            ("v2.plugins.pty.create", RouteMethod::Post, "", PtyAction::Create),
            ("v2.plugins.pty.get", RouteMethod::Get, "/:pty_id", PtyAction::Get),
            ("v2.plugins.pty.update", RouteMethod::Put, "/:pty_id", PtyAction::Update),
            ("v2.plugins.pty.remove", RouteMethod::Delete, "/:pty_id", PtyAction::Remove),
            ("v2.plugins.pty.connectToken", RouteMethod::Post, "/:pty_id/connect-token", PtyAction::ConnectToken),
        ] {
            registrar.runtime_route(RouteContribution {
                ...
                handler: Arc::new(PtyRoute { host: self.host.clone(), action }),
            });
        }
        let id = "v2.plugins.pty.connect";
        registrar.runtime_websocket_route(WebSocketRouteContribution {
            descriptor: RouteDescriptor { id: id.into(), method: RouteMethod::Get, path: format!("/v2/plugins/{ID}/:pty_id/connect"), scope: RouteScope::Workspace, request_schema: None, response_schema: None },
            metadata: ContributionMetadata::new(id, ID, PluginScope::Workspace),
...
struct PtyRoute { host: Arc<dyn PtyHost>, action: PtyAction }
...
impl WebSocketRouteHandler for PtySocketRoute {
    fn prepare<'a>(&'a self, request: RouteRequest) -> PluginFuture<'a, Arc<dyn WebSocketSession>> { self.0.connect(request) }
}