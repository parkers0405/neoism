...
mod lifecycle_conformance_tests {
    use neoism_agent_plugin_api::{CapabilityGrants, HostCapability, PluginContext, PluginFactory, RuntimeScope, WorkspaceIdentity};
    ...
    fn context() -> PluginContext {
        let grants = [
            ...
            HostCapability::EventPublish, HostCapability::Network,
            HostCapability::ProcessSpawn, HostCapability::SecretRead,
        ].into_iter().fold(CapabilityGrants::default(), CapabilityGrants::allow);
        PluginContext::new(RuntimeScope::Workspace(WorkspaceIdentity { id: "test".into(), root: ".".into() }), grants)
    }
    ...
    async fn representative_first_party_factories_cover_services_sources_tools_and_routes() {
        ...
        for factory in factories {
            ...
            let policy = descriptor.manifest.api_prefix.as_ref().map_or_else(
                ...
                |prefix| neoism_agent_plugin_api::RoutePrefixPolicy::default().allow_legacy(prefix),
            );
            let report = neoism_agent_plugin_api::testkit::check_factory_with_policy(factory.as_ref(), context(), policy).await.unwrap();
            assert!(report.is_conformant(), "{}: {:?}", factory.descriptor().manifest.id, report.errors);
        }
        let installed = neoism_agent_plugin_api::PluginHost::default()
            .install(vec![Box::new(super::SystemPromptPlugin)], &[], context()).await.unwrap();
        let snapshot = installed.snapshot();
        let metadata = snapshot.service_metadata.values().next().expect("system prompt service metadata");
