...
use neoism_agent_core::{
    AgentInfo, CapabilityInfo, CommandInfo, PermissionRule, PluginManifestInfo,
    SkillInfo, PLUGIN_API_VERSION,
};
...
impl RegisteredRuntimeHook {
    ...
    pub fn for_test(plugin_id: impl Into<String>, runtime: Arc<dyn RuntimeHook>) -> Self {
        Self {
            ...
            runtime,
            lifecycle: Arc::new(RwLock::new(PluginLifecycle {
                active: true,
                reason: None,
            })),
        }
        ...
        match self.runtime.invoke(hook, context, value) {
            Ok(value) => {
                *self
                    .lifecycle
                    .write()
                    .expect("plugin lifecycle lock poisoned") = PluginLifecycle {
                    active: true,
                    reason: None,
                };
                Ok(value)
...
impl RegistrySnapshot {
    ...
    pub fn ensure_active(&self) -> Result<(), PluginRuntimeError> {
        self.is_active()
            .then_some(())
            .ok_or_else(|| PluginRuntimeError::new("plugin generation is shut down"))
    }
    ...
    pub fn system_context_services_by_priority(
        &self,
    ) -> Vec<&Arc<dyn SystemContextService>> {
        self.services_by_priority(&self.system_context_services, "SystemContextService")
    }

    pub fn services_by_priority<'a, T: ?Sized>(
        &'a self,
...
fn ordered_services<'a, T: ?Sized>(
    ...
    entries.sort_by(|(left_id, _), (right_id, _)| {
        ...
        right
            ...
            .cmp(&left.map_or(0, |item| item.priority))
...
impl PluginInstallError {
    fn new(error: PluginHostError) -> Self {
        Self {
            error,
            quarantine: None,
        }
    }
    pub fn error(&self) -> &PluginHostError {
        &self.error
    }
    pub fn quarantine(&self) -> Option<&InstalledPlugins> {
        self.quarantine.as_ref()
    }
    pub fn into_parts(self) -> (PluginHostError, Option<InstalledPlugins>) {
        (self.error, self.quarantine)
    }
}
...
impl std::fmt::Debug for PluginInstallError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("PluginInstallError")
...
impl PluginFactoryRegistration {
    pub fn new(factory: Box<dyn PluginFactory>) -> Self {
        Self {
            factory,
            route_prefix_policy: RoutePrefixPolicy::default(),
        }
    }
        ...
        match self.shutdown.compare_exchange(
            ...
            Err(_) => {
                return Err(PluginRuntimeError::new(
                    "plugin shutdown is already in progress",
                ))
            }
        }
...
impl PluginHost {
    pub async fn install(
        ...
        self.install_registered(
            factories
                .into_iter()
                .map(PluginFactoryRegistration::new)
                .collect(),
            disabled,
            context,
        )
    ...
    pub async fn install_registered(
        ...
        for id in order {
            ...
            let reason = if requested_disabled && !manifest.disableable {
                Some(
                    "plugin is not disableable until its core migration is complete"
                        .to_string(),
                )
            ...
            if enabled {
                ...
                if !matches!(
                    ...
                    return Err(cleanup_failure(
                        PluginHostError::Lifecycle {
                            plugin: id,
                            message: readiness.reason.unwrap_or_else(|| {
                                format!("plugin readiness is {:?}", readiness.state)
                            }),
                        },
                        Some(instance),
                        &instances,
                ...
                let workspace_id =
                    context.workspace().map(|workspace| workspace.id.as_str());
                ...
                stamp_services(
                    &mut registered.agents,
                    &id,
                    descriptor.scope,
                    workspace_id,
                );
                stamp_services(
                    &mut registered.config,
                    &id,
                    descriptor.scope,
                    workspace_id,
                );
                stamp_services(
                    &mut registered.system_context,
                    &id,
                    descriptor.scope,
                    workspace_id,
                );
                stamp_services(
                    &mut registered.prompts,
                    &id,
                    descriptor.scope,
                    workspace_id,
                );
                stamp_services(
                    &mut registered.providers,
                    &id,
                    descriptor.scope,
                    workspace_id,
                );
                stamp_services(
                    &mut registered.commands,
                    &id,
                    descriptor.scope,
                    workspace_id,
                );
                stamp_services(
                    &mut registered.skills,
                    &id,
                    descriptor.scope,
                    workspace_id,
                );
                if let Err(error) = validate_contributions(
                    descriptor,
                    &registered,
                    &route_prefix_policies[&id],
                ) {
                ...
                for route in registered.routes {
                    ...
                    if let Err(error) = validate_route_contract(
                        descriptor,
                        &route.descriptor,
                        &route_prefix_policies[&id],
                    ) {
                ...
                for route in registered.websocket_routes {
                    ...
                    if runtime_routes.contains_key(&key)
                        || runtime_websocket_routes
                            .insert(
                                key.clone(),
                                RegisteredWebSocketRouteContribution {
                                    plugin_id: id.clone(),
                                    route,
                                },
                            )
                ...
                for result in [
                    merge_services(
                        &mut agent_services,
                        &mut service_metadata,
                        registered.agents,
                        "AgentService",
                    ),
                    merge_services(
                        &mut command_services,
                        &mut service_metadata,
                        registered.commands,
                        "CommandService",
                    ),
...
async fn cleanup_failure(
    ...
    let shutdown = current
        .into_iter()
        .chain(instances.iter().rev().map(|(_, instance)| instance.clone()))
        .collect::<Vec<_>>();
    ...
    let error = PluginHostError::Cleanup {
        primary: primary.to_string(),
        cleanup: cleanup_error.unwrap_or_else(|| "unknown cleanup failure".into()),
    };
...
pub(crate) fn validate_route_contract(
    ...
    if route.path != prefix && !route.path.starts_with(&format!("{prefix}/")) {
        return Err(PluginHostError::Registration(format!(
            "route `{}` escapes plugin API prefix `{prefix}`",
            route.id
        )));
    }
    Ok(())
...
fn plugin_api_prefix(
    ...
    if !valid {
        return Err(PluginHostError::Registration(format!(
            "plugin `{}` has an invalid API prefix",
            descriptor.manifest.id
        )));
    }
    if prefix != canonical && !policy.allows(&prefix) {
        return Err(PluginHostError::Registration(format!(
            "plugin `{}` must use canonical API prefix `{canonical}`",
            descriptor.manifest.id
        )));
    }
    Ok(prefix)
...
pub(crate) fn validate_host_descriptor(
    ...
    if descriptor.scope != context.scope().kind() {
        return Err(PluginHostError::Registration(format!(
            "plugin `{id}` has the wrong runtime scope"
        )));
    }
    if let Some(capability) = descriptor
        .required_capabilities
        .iter()
        .find(|capability| !context.has(**capability))
    {
        return Err(PluginHostError::Registration(format!(
...
pub(crate) fn validate_contributions(
    ...
    for (kind, keys) in [
        (
            "SkillSource",
            contributions.skill_sources.keys().collect::<Vec<_>>(),
        ),
        (
            "CommandSource",
            contributions.command_sources.keys().collect(),
        ),
    ...
    for (id, tool) in &contributions.runtime_tools {
        if id.trim().is_empty() || tool.definition().id != *id {
            return Err(PluginHostError::Registration(format!(
                "runtime tool `{id}` has inconsistent identity"
            )));
        }
    }
    ...
    for route in &contributions.websocket_routes {
        if route.metadata.id != route.descriptor.id {
            return Err(PluginHostError::Registration(format!(
                "websocket route `{}` metadata and descriptor ids differ",
                route.descriptor.id
            )));
        }
        route
...
pub enum PluginHostError {
    ...
    #[error(
        "plugin `{plugin}` declares API major {declared}, but host requires {expected}"
    )]
    IncompatibleApi {
        plugin: String,
        expected: u16,
...
mod tests {
    ...
    async fn installs_dependencies_before_dependents_and_honors_wildcards() {
        ...
        assert!(snapshot
            ...
            .iter()
            .all(
                |capability| capability.plugin_id.as_deref() != Some("dev.neoism.child")
            ));
        assert!(snapshot
            .contributions
            .values()
    ...
    async fn retains_typed_runtime_contributions_in_the_snapshot() {
        let host = PluginHost::default();
        let installed = host
            .install(vec![Box::new(RuntimePlugin)], &[], test_context())
            .await
            .unwrap();
        let snapshot = installed.snapshot();
        assert!(snapshot.skill_sources.contains_key("example"));
        assert!(snapshot.contributions.contains_key("SkillSource:example"));
    }
    ...
    async fn retains_runtime_tools_and_permission_metadata() {
        ...
        let host = PluginHost::default();
        let installed = host
            .install(vec![Box::new(ToolPlugin)], &[], test_context())
            .await
            .unwrap();
        let snapshot = installed.snapshot();
    ...
    async fn host_disables_before_create_orders_dependencies_and_retries_reverse_shutdown(
        ...
        assert_eq!(
            *events.lock().unwrap(),
            vec![
                "create:dev.example.base",
                "start:dev.example.base",
                "create:dev.example.child",
                "start:dev.example.child",
            ]
        );
        ...
        let result = host
            .install(
                vec![
                    ...
                    Box::new(LifecycleFactory {
                        id: "dev.example.child",
                        requires: &["dev.example.base"],
                        ready: false,
                        events: events.clone(),
                    }),
                ],
            ...
            .await;
        assert!(matches!(
            result.as_ref().map_err(PluginInstallError::error),
            Err(PluginHostError::Lifecycle { .. })
        ));
        assert_eq!(
            &events.lock().unwrap()[4..],
            ["shutdown:dev.example.child", "shutdown:dev.example.base",]
    ...
    impl PluginFactory for RetryShutdownFactory {
        ...
        fn create<'a>(
            ...
            Box::pin(async move {
                Ok(Box::new(RetryShutdownInstance {
    ...
    impl PluginInstance for RetryShutdownInstance {
        ...
        fn shutdown<'a>(&'a self) -> PluginFuture<'a, ()> {
            Box::pin(async move {
                ...
                if call == 0 && self.pending_first {
                    std::future::pending::<()>().await;
                }
                if call == 0 && !self.pending_first {
                    return Err(PluginRuntimeError::new("retry me"));
                }
                Ok(())
            })
        }
    ...
    async fn shutdown_failure_and_cancellation_can_be_retried_without_reactivation() {
        for pending_first in [false, true] {
            ...
            let installed = PluginHost::default()
                .install(
                    vec![Box::new(RetryShutdownFactory {
                        calls: calls.clone(),
                        pending_first,
                    })],
                    &[],
    ...
    impl PluginDefinition for InvalidScopePlugin {
        ...
        fn contributions(
            &self,
            contributions: &mut PluginContributions,
        ) -> Result<(), PluginHostError> {
    ...
    async fn workspace_plugins_cannot_escape_their_route_namespace() {
        let result = PluginHost::default()
            .install(vec![Box::new(InvalidScopePlugin)], &[], test_context())
            .await;
        assert!(
            matches!(result.as_ref().map_err(PluginInstallError::error), Err(PluginHostError::Registration(message)) if message.contains("escapes"))
        );
    }
        ...
        fn descriptor(&self) -> PluginDescriptor {
            PluginDescriptor {
                manifest: PluginManifest {
                    ...
                    requires: Vec::new(),
                    event_namespaces: Vec::new(),
                    api_prefix: Some(self.prefix.into()),
                    config: BTreeMap::new(),
                },
                scope: PluginScope::Workspace,
                required_capabilities: Vec::new(),
    ...
    async fn third_party_legacy_and_reserved_api_prefixes_are_rejected_before_creation() {
        let result = PluginHost::default()
            .install(
                vec![Box::new(PrefixFactory {
                    id: "dev.example.reserved",
                    prefix: "/v2/sessions",
                })],
                &[],
                test_context(),
            ...
            .await;
        assert!(
            matches!(result.as_ref().map_err(PluginInstallError::error), Err(PluginHostError::Registration(message)) if message.contains("must use canonical"))
        );
    }
    ...
    async fn invalid_later_route_policy_causes_zero_plugin_starts() {
        ...
        assert!(matches!(
            result.as_ref().map_err(PluginInstallError::error),
            Err(PluginHostError::Registration(_))
        ));
    ...
    impl PluginFactory for IncompatibleFactory {
        fn descriptor(&self) -> PluginDescriptor {
            PluginDescriptor {
                manifest: PluginManifest {
                    id: "dev.example.incompatible".into(),
    ...
    async fn incompatible_api_is_rejected_before_factory_creation() {
        ...
        let result = PluginHost::default()
            .install(
                vec![Box::new(IncompatibleFactory {
            ...
            .await;
        assert!(
            matches!(result.as_ref().map_err(PluginInstallError::error), Err(PluginHostError::IncompatibleApi { expected: PLUGIN_API_MAJOR, declared, .. }) if *declared == PLUGIN_API_MAJOR + 1)
        );
    ...
    async fn incompatible_later_descriptor_causes_zero_plugin_starts() {
        ...
        assert!(matches!(
            result.as_ref().map_err(PluginInstallError::error),
            Err(PluginHostError::IncompatibleApi { .. })
        ));
        assert!(events.lock().unwrap().is_empty());
        assert_eq!(creates.load(Ordering::SeqCst), 0);
    }
    ...
    fn service_priority_is_descending_with_stable_id_ties() {
        ...
        let metadata = BTreeMap::from([
            (
                "ProviderService:z-low".to_string(),
                ContributionMetadata {
                    priority: 1,
                    ..ContributionMetadata::new(
                        "z-low",
                        "dev.example.priority",
                        PluginScope::Workspace,
                    )
                },
            ),
            (
                "ProviderService:b-high".to_string(),
                ContributionMetadata {
                    priority: 10,
                    ..ContributionMetadata::new(
                        "b-high",
            ...
            (
                "ProviderService:a-high".to_string(),
                ContributionMetadata {
                    priority: 10,
                    ..ContributionMetadata::new(
                        "a-high",
        ...
        assert_eq!(
            ordered.into_iter().map(|value| **value).collect::<Vec<_>>(),
            vec![3, 2, 1]
        );
    }
    ...
    impl PluginFactory for CleanupFailureFactory {
        ...
        fn create<'a>(
            ...
            Box::pin(async {
                Ok(Box::new(CleanupFailureInstance) as Box<dyn PluginInstance>)
            })
        }
    }
    ...
    async fn failed_install_reports_primary_and_cleanup_failures() {
        let result = PluginHost::default()
            .install(vec![Box::new(CleanupFailureFactory)], &[], test_context())
            .await;
        assert!(
            matches!(result.as_ref().map_err(PluginInstallError::error), Err(PluginHostError::Cleanup { primary, cleanup }) if primary.contains("start failed") && cleanup.contains("shutdown failed"))
        );
        assert!(match result {
            Ok(_) => false,
            Err(failure) => failure.quarantine().is_some(),
        });
    }
    ...
    impl PluginFactory for RetryCleanupFactory {
        ...
        fn create<'a>(
            ...
            Box::pin(async move {
                Ok(Box::new(RetryCleanupInstance(calls)) as Box<dyn PluginInstance>)
            })
        }
    }
        ...
        fn shutdown<'a>(&'a self) -> PluginFuture<'a, ()> {
            Box::pin(async move {
                if self.0.fetch_add(1, Ordering::SeqCst) < 3 {
                    Err(PluginRuntimeError::new("retry"))
                } else {
                    Ok(())
                }
            })
    ...
    async fn failed_install_retains_instances_for_later_cleanup_retry() {
        ...
        let failure = match PluginHost::default()
            .install(
                vec![Box::new(RetryCleanupFactory(calls.clone()))],
                &[],
                test_context(),
            )
            .await
    ...
    struct CapabilityFactory {
        seen: Arc<std::sync::Mutex<Option<BTreeSet<HostCapability>>>>,
    }
    impl PluginFactory for CapabilityFactory {
        fn descriptor(&self) -> PluginDescriptor {
            PluginDescriptor {
                manifest: PluginManifest {
                    id: "dev.example.capability".into(),
                ...
                scope: PluginScope::Workspace,
                required_capabilities: vec![HostCapability::ProcessSpawn],
                plugin_api_major: PLUGIN_API_MAJOR,
            }
        }
        fn create<'a>(
    ...
    async fn host_rejects_missing_capabilities_and_attenuates_extra_grants() {
        ...
        let result = PluginHost::default()
            .install(
                vec![Box::new(CapabilityFactory { seen: seen.clone() })],
                &[],
                context,
            )
            .await;
        assert!(
            matches!(result.as_ref().map_err(PluginInstallError::error), Err(PluginHostError::Registration(message)) if message.contains("ProcessSpawn"))
        );
