    ...
    for part in parts {
        if let Part::Tool(tool) = part {
            if tool.id.as_str() == part_id {
                ...
                if let Some(Value::Object(metadata)) = &mut tool.metadata {
                    metadata.remove("toolResult");
                }
...
pub(crate) fn set_tool_execution_result(
    parts: &mut [Part],
    part_id: &str,
    result: crate::tool::ToolExecutionResult,
) -> Option<Part> {
    if !result.is_error() {
        return set_tool_completed(
            parts,
            part_id,
            result.output,
            result.title,
            result.metadata.unwrap_or_else(|| json!({})),
        );
    }
    set_tool_error(parts, part_id, result.output.clone())?;
    let metadata = stable_tool_metadata(
        result.metadata.unwrap_or_else(|| json!({})),
        "tool-error",
        &result.title,
        &result.output,
    ...
    for part in parts {
        if let Part::Tool(tool) = part {
            if tool.id.as_str() == part_id {
                ...
                if !outer.is_object() {
                    *outer = json!({"previous":outer.take()});
                }
