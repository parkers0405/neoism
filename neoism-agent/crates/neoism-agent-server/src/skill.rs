...
pub(crate) fn skill_tool(
    ...
    async move {
        ...
        let available = skills
            ...
            .collect::<Vec<_>>()
            .join(", ");
        let skill = resolve_from_snapshot(context.plugin_snapshot()?, &directory, name)
            .await?
            .ok_or_else(|| {
                anyhow::anyhow!(
...
fn render_skill(
    skill: neoism_agent_plugin_api::SkillDocument,
    ...
    Ok(ToolExecutionResult {
        ...
        .join("\n"),
        metadata: Some(
            json!({"skill":{"name":skill.info.name,"description":skill.info.description,"path":skill.info.path,"dir":base}}),
        ),
    })
}
