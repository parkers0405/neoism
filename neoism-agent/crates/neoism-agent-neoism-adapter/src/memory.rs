...
impl NeoismMemoryService {
    ...
    async fn semantic_recall(
        ...
        query: &str,
        limit: usize,
    ) -> Result<Option<Vec<MemoryEntry>>, ServiceError> {
        let index = self.semantic.read().unwrap().clone();
        let Some(index) = index.filter(|index| index.available()) else {
            return Ok(None);
        };
        let Some(model) = index.model() else {
            return Ok(None);
        };
        ...
        for root in &roots {
            sync_semantic(index.as_ref(), root, &model).await?;
        }
...
impl MemoryService for NeoismMemoryService {
    fn scope_choices(&self) -> Vec<ScopeChoice> {
        vec![
            choice(
                "auto",
                "Project and user",
                "Search workspace memory and personal memory.",
            ),
            choice(
                "project",
                "Project",
                "Use agent-owned memory for this workspace.",
            ),
    ...
    fn write(&self, request: &MemoryWriteRequest) -> Result<MemoryEntry, ServiceError> {
        ...
        if let Some(existing) =
            find_similar(&root, &path, &request.name, &request.description)?
        {
            return Err(ServiceError::new(format!(
                "a memory covering this already exists at {}; read and update that file",
    ...
    fn search(
        ...
        let mut all = Vec::new();
        for root in self.roots(request, false)? {
            all.extend(keyword_search(&root, query, limit)?);
        }
        all.truncate(limit);
        Ok(all)
    }
    ...
    fn context_fragments(&self, working_directory: &Path) -> Vec<SystemContextFragment> {
        ...
        let Ok(roots) = self.roots(&request, false) else {
            return Vec::new();
        };
...
async fn sync_semantic(
    index: &dyn SemanticMemoryIndex,
    root: &Root,
    model: &str,
) -> Result<(), ServiceError> {
    let root_key = root.path.to_string_lossy().to_string();
    let indexed = index
        .hashes(&root_key, model)
        .await?
        .into_iter()
        .collect::<HashMap<_, _>>();
    ...
    for path in files(root)? {
        ...
        let input = format!(
            "{}\n{}\n{}",
            name,
            frontmatter(&text, "description").unwrap_or_default(),
            text
        )
        .chars()
        ...
        if indexed.get(&key) != Some(&hash) {
            stale.push((key, hash, input));
        }
    }
    for key in indexed.keys().filter(|key| !disk.contains(*key)) {
    ...
    for chunk in stale.chunks(16) {
        ...
        for ((key, hash, _), vector) in chunk.iter().zip(index.embed(&inputs).await?) {
            if !vector.is_empty() {
                index
                    .upsert(key, &root_key, hash, model, unix_millis(), &vector)
                    .await?;
            }
        }
    }
...
fn project_roots(cwd: &Path, include_missing: bool) -> Result<Vec<Root>, ServiceError> {
    ...
    if include_missing || root.path.is_dir() {
        Ok(vec![root])
    } else {
        Ok(Vec::new())
    }
...
fn project_root(cwd: &Path) -> Result<Root, ServiceError> {
    ...
    let root = Root {
        scope: "project",
        label,
        path: workspace.join(PROJECT_DIR),
    };
...
fn user_root() -> Result<Root, ServiceError> {
    let root = Root {
        scope: "user",
        label: "Neoism user".to_string(),
        path: memory_home().join(USER_DIR),
    };
    let old = neoism_workspace_index::default_notes_workspace()
        .notes_workspace_dir()
...
fn write_root(request: &MemoryRequest, kind: &str) -> Result<Root, ServiceError> {
    match request.scope_id.as_deref().unwrap_or("project") {
        "user" => user_root(),
        "project" | "all" => project_root(&request.working_directory),
        "auto" | "" if matches!(kind, "personal" | "preference") => user_root(),
        "auto" | "" => project_root(&request.working_directory),
        other => Err(ServiceError::new(format!("unknown memory scope {other}"))),
    }
}

fn memory_home() -> PathBuf {
    if let Some(path) = std::env::var_os("NEOISM_AGENT_MEMORY_HOME") {
        return PathBuf::from(path);
    }
    if let Some(path) = std::env::var_os("XDG_DATA_HOME") {
        return PathBuf::from(path).join("neoism/memory");
    }
    if let Some(home) = std::env::var_os("HOME") {
        return PathBuf::from(home).join(".local/share/neoism/memory");
    }
...
fn vault_memory_roots(cwd: &Path) -> Result<Vec<PathBuf>, ServiceError> {
    let Some(workspace) =
        neoism_workspace_index::linked_project_for_code_dir(cwd).map_err(error)?
    else {
        return Ok(Vec::new());
    };
...
fn available_import_path(path: &Path) -> PathBuf {
    ...
    (1..)
        .map(|index| parent.join(format!("{stem}_imported_{index}.md")))
        .find(|candidate| !candidate.exists())
        .expect("memory import path")
}
    ...
    let mut paths = entries
        ...
        .filter(|p| {
            p.is_file()
                ...
                && p.file_name().and_then(|v| v.to_str()) != Some(INDEX_FILE)
        })
        .collect::<Vec<_>>();
    paths.sort();
    Ok(paths)
...
fn safe_path(root: &Root, raw: &str) -> Result<PathBuf, ServiceError> {
    ...
    if path.is_absolute() {
        return path
            ...
            .ok_or_else(|| {
                ServiceError::new("memory path is outside the selected memory root")
            });
    }
    if path.components().any(|c| {
        ...
        return Err(ServiceError::new("memory path must be relative"));
    }
    let relative = path
        .strip_prefix(PROJECT_DIR)
        .or_else(|_| path.strip_prefix(USER_DIR))
        .unwrap_or(path);
...
fn existing_file<'a>(
    roots: &'a [Root],
    raw: &str,
) -> Result<(&'a Root, PathBuf), ServiceError> {
    for root in roots {
        if let Ok(path) = safe_path(root, raw) {
            if path.is_file() {
                return Ok((root, path));
            }
        }
    }
    Err(ServiceError::new(format!("no memory file {raw}")))
}
fn location(root: &Root) -> MemoryLocation {
    MemoryLocation {
        scope_id: root.scope.to_string(),
        label: root.label.clone(),
        storage_key: root.path.to_string_lossy().to_string(),
    }
}
fn entry(root: &Root, path: &Path, text: &str, content: Option<String>) -> MemoryEntry {
    MemoryEntry {
...
fn keyword_search(
    ...
    for path in files(root)? {
        ...
        if tokens.is_empty() || matched > 0 {
            let mut value = entry(root, &path, &text, None);
            value.snippet = Some(snippet(&haystack));
            hits.push((usize::MAX - matched, value));
        }
    }
...
fn frontmatter(source: &str, key: &str) -> Option<String> {
...
fn find_similar(
    ...
    let incoming = tokens(&format!("{name} {description}"));
    if incoming.len() < 3 {
        return Ok(None);
    }
    for path in files(root)? {
        ...
        let text = std::fs::read_to_string(&path).unwrap_or_default();
        let existing = tokens(&format!(
            "{} {}",
            frontmatter(&text, "name").unwrap_or_default(),
            frontmatter(&text, "description").unwrap_or_default()
        ));
...
fn memory_file_name(kind: &str, name: &str) -> String {
    ...
    format!(
        "{}.md",
        if name.starts_with(&format!("{kind}_")) {
            name
        } else {
            format!("{kind}_{name}")
        }
...
fn safe_file_name(value: &str) -> String {
    let value = value.trim().strip_suffix(".md").unwrap_or(value.trim());
...
fn truncate(text: &str, max: usize) -> String {
    ...
    out.push_str(
        "(index truncated - use memory.list or memory.recall for omitted entries)",
    );
    out
}
fn choice(id: &str, label: &str, description: &str) -> ScopeChoice {
    ScopeChoice {
        id: id.to_string(),
...
fn today_utc() -> String {
    let days = unix_millis() / 86_400_000;
    let z = days + 719468;
...
mod tests {
    ...
    impl NotesHome {
        fn new(label: &str) -> Self {
            ...
            unsafe {
                std::env::set_var("NEOISM_AGENT_MEMORY_HOME", root.join("agent-memory"));
            }
            Self {
                root,
                previous_notes,
    ...
    impl Drop for NotesHome {
        fn drop(&mut self) {
            ...
            match self.previous_memory.take() {
                Some(value) => unsafe {
                    std::env::set_var("NEOISM_AGENT_MEMORY_HOME", value)
                },
    ...
    fn memory_is_agent_owned_and_moves_old_user_memory() {
        ...
        std::fs::write(legacy_user.join("personal_legacy.md"), "legacy user fact")
            .unwrap();

        let service = NeoismMemoryService::new();
        service
            .init(&MemoryRequest::new(&cwd).with_scope("all"))
            .unwrap();
        service
            .write(&write_request(&cwd, "project", "feature", "project fact"))
            .unwrap();
        service
            .write(&write_request(&cwd, "user", "personal", "user fact"))
            .unwrap();

        assert!(cwd.join(".neoism/memory/feature_project_fact.md").is_file());
        assert!(home
            .root
            .join("agent-memory/user/personal_user_fact.md")
            .is_file());
        assert!(home
            .root
            .join("agent-memory/user/personal_legacy.md")
            .is_file());
    ...
    fn linked_vault_memory_is_moved_into_workspace_memory() {
        ...
        neoism_workspace_index::config::link_workspace_to_vault_project(
            &mut workspace,
            &cwd,
        )
        .unwrap();
        ...
        std::fs::write(
            vault.join("Memory/shared.md"),
            "---\ndescription: shared owning vault fact\ntype: project\n---\n",
        )
        .unwrap();
        let service = NeoismMemoryService::new();
        service
            .write(&write_request(&cwd, "project", "feature", "specific fact"))
            .unwrap();

        let entries = service
            .search(&MemoryRequest::new(&cwd).with_scope("project"), "fact", 10)
            .unwrap();
        assert!(entries
            .iter()
            .any(|entry| entry.path == "feature_specific_fact.md"));
        assert!(entries.iter().any(|entry| entry.path == "shared.md"));
        assert!(cwd
            .join(".neoism/memory/feature_specific_fact.md")
            .is_file());
