...
impl AgentPlugin for VcsPlugin {
    fn manifest(&self) -> PluginManifest {
        PluginManifest {
            ...
            internal: true, disableable: true, capabilities: vec!["neoism.vcs".into()],
            requires: Vec::new(), event_namespaces: vec!["vcs".into()],
...
pub fn info(services: &AgentServices, directory: &str) -> VcsInfo {
    VcsInfo {
        ...
        default_branch: git_output(services, directory, &["symbolic-ref", "refs/remotes/origin/HEAD"])
...
pub fn status(services: &AgentServices, directory: &str) -> Vec<VcsFileStatus> {
    let Some(output) = git_output_raw(services, directory, &["status", "--porcelain=v1", "-z"]) else { return Vec::new(); };
    ...
    while let Some(field) = fields.next() {
        ...
        if code.starts_with('R') || code.starts_with('C') { fields.next(); }
        let (additions, deletions) = stats.remove(&path).unwrap_or_else(|| {
            if normalize_status(&code) == "added" && is_untracked(services, directory, &path) { stat_untracked(directory, &path) } else { (0, 0) }
        });
        statuses.push(VcsFileStatus { file: path.clone(), path, status: normalize_status(&code).into(), additions, deletions });
    }
...
pub fn diff(services: &AgentServices, directory: &str) -> Vec<VcsFileDiff> {
    let mut stats = diff_stats(services, directory);
    status(services, directory).into_iter().map(|file| {
        ...
        let (added, removed) = stats.remove(&file.path).unwrap_or_else(|| count_patch_lines(&patch));
        let hunks = (!patch.is_empty()).then(|| vec![json!({ "patch": patch.clone() })]).unwrap_or_default();
        VcsFileDiff { file: file.path.clone(), path: file.path, status: file.status, added, removed, additions: added, deletions: removed, patch, hunks }
    }).collect()
}
...
pub fn apply(services: &AgentServices, directory: &str, patch: &str) -> VcsApplyResult {
    if patch.trim().is_empty() { return failure("missing patch"); }
    let git = match resolve_git(services) { Ok(path) => path, Err(error) => return failure(error.to_string()) };
    let mut command = hidden_command(git);
    let mut child = match command.args(["apply", "--whitespace=nowarn", "--"]).current_dir(directory)
        .stdin(Stdio::piped()).stdout(Stdio::piped()).stderr(Stdio::piped()).spawn() {
        Ok(child) => child, Err(error) => return failure(format!("failed to start git apply: {error}")),
    };
    if let Some(mut stdin) = child.stdin.take() {
        if let Err(error) = stdin.write_all(patch.as_bytes()) { return failure(format!("failed to write patch to git apply: {error}")); }
    }
    match child.wait_with_output() {
        Ok(output) if output.status.success() => VcsApplyResult { success: true, error: None },
        Ok(output) => {
...
fn diff_stats(services: &AgentServices, directory: &str) -> BTreeMap<String, (u64, u64)> {
    git_output(services, directory, &["diff", "--numstat", "HEAD"]).into_iter().flat_map(|output| {
        output.lines().filter_map(|line| {
            let mut parts = line.splitn(3, '\t');
            Some((parts.next()?.parse().ok()?, parts.next()?.parse().ok()?, parts.next()?.to_string()))
        }).map(|(added, removed, path)| (path, (added, removed))).collect::<Vec<_>>()
    }).collect()
}

fn git_output(services: &AgentServices, directory: &str, args: &[&str]) -> Option<String> {
    let output = git_output_raw(services, directory, args)?;
    let text = String::from_utf8_lossy(&output).trim().to_string();
    (!text.is_empty()).then_some(text)
}

fn git_output_raw(services: &AgentServices, directory: &str, args: &[&str]) -> Option<Vec<u8>> {
...
fn hidden_command(program: impl AsRef<std::ffi::OsStr>) -> Command {
    ...
    #[cfg(windows)] {
        ...
        command.creation_flags(windows_sys::Win32::System::Threading::CREATE_NO_WINDOW | windows_sys::Win32::System::Threading::CREATE_NEW_PROCESS_GROUP);
    }
    command
}

fn normalize_status(code: &str) -> &'static str {
    if code == "??" || code.contains('A') { "added" } else if code.contains('D') { "deleted" } else { "modified" }
}

fn is_untracked(services: &AgentServices, directory: &str, path: &str) -> bool {
    git_output(services, directory, &["ls-files", "--others", "--exclude-standard", "--", path])
...
fn untracked_patch(directory: &str, path: &str) -> Option<String> {
    ...
    for line in lines { patch.push('+'); patch.push_str(line); patch.push('\n'); }
    if !content.ends_with('\n') { patch.push_str("\\ No newline at end of file\n"); }
    Some(patch)
}

fn stat_untracked(directory: &str, path: &str) -> (u64, u64) {
    let Ok(bytes) = std::fs::read(PathBuf::from(directory).join(path)) else { return (0, 0); };
    if bytes.contains(&0) { return (0, 0); }
    let text = String::from_utf8_lossy(&bytes);
    ((if text.is_empty() { 0 } else { text.lines().count() }) as u64, 0)
}

fn patch_path(path: &str) -> String {
    Path::new(path).components().map(|part| part.as_os_str().to_string_lossy()).collect::<Vec<_>>().join("/")
}

fn count_patch_lines(patch: &str) -> (u64, u64) {
    patch.lines().fold((0, 0), |(added, removed), line| {
        if line.starts_with("+++") || line.starts_with("---") { (added, removed) }
        else if line.starts_with('+') { (added + 1, removed) }
        else if line.starts_with('-') { (added, removed + 1) }
        else { (added, removed) }
    })
}
