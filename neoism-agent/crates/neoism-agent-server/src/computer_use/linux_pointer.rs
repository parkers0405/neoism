...
use wayland_client::{
    protocol::{wl_callback, wl_output, wl_pointer, wl_registry, wl_seat},
    Connection, Dispatch, EventQueue, Proxy, QueueHandle,
};
use wayland_protocols::xdg::xdg_output::zv1::client::{
    zxdg_output_manager_v1 as xmanager, zxdg_output_v1 as xoutput,
};
...
impl Point {
    fn validate(self) -> anyhow::Result<()> {
            ...
            self.width > 0
                ...
                && self.x < self.width
                && self.y < self.height,
            "Invalid screenshot pointer coordinates"
        );
        Ok(())
    }
    fn between(self, end: Self, step: u32) -> Self {
        let axis = |a: u32, b: u32| {
            (i64::from(a) + (i64::from(b) - i64::from(a)) * i64::from(step) / 20) as u32
        };
        Self {
            x: axis(self.x, end.x),
            y: axis(self.y, end.y),
...
pub(super) enum Command {
    ...
    Click(Point, u32),
    Drag(Point, Point),
    Scroll { amount: i32, horizontal: bool },
}
#[derive(Default)]
struct State {
    outputs: Vec<(wl_output::WlOutput, Display)>,
    seats: Vec<wl_seat::WlSeat>,
    managers: Vec<manager::ZwlrVirtualPointerManagerV1>,
    xmanager: Option<xmanager::ZxdgOutputManagerV1>,
    removed: bool,
}
    ...
    fn event(
        ...
        match e {
            wl_registry::Event::Global {
                ...
                "wl_output" => s.outputs.push((
                    r.bind(name, version.min(4), q, ()),
                    Display {
                        id: String::new(),
                        x: 0,
                        y: 0,
                ...
                "zwlr_virtual_pointer_manager_v1" => {
                    s.managers.push(r.bind(name, version.min(2), q, ()))
                }
                "zxdg_output_manager_v1" => {
                    s.xmanager = Some(r.bind(name, version.min(3), q, ()))
                }
                _ => {}
            },
...
impl Dispatch<xoutput::ZxdgOutputV1, usize> for State {
    fn event(
        ...
        match e {
            ...
            xoutput::Event::LogicalPosition { x, y } => {
                d.x = x;
                d.y = y;
            }
            xoutput::Event::LogicalSize { width, height } => {
                d.width = width.max(0) as u32;
                d.height = height.max(0) as u32;
            }
            _ => {}
        }
    }
...
fn sync(
    ...
    while !done.load(Ordering::Relaxed) {
        ...
        let blocked = match conn.flush() {
            ...
            "Wayland pointer acknowledgement timed out; observe before retrying"
        );
        ...
        if let Some(read) = queue.prepare_read() {
            ...
            if n < 0 {
                ...
                if error.kind() != std::io::ErrorKind::Interrupted {
                    ...
                    "Wayland pointer connection closed"
                );
...
fn matching_output(observed: &Display, outputs: &[Display]) -> anyhow::Result<()> {
        ...
        !observed.id.is_empty()
            ...
            && observed.height > 0
            && outputs[0] == *observed,
        "Pointer output name/geometry differs from observation; take a new screenshot"
    );
    Ok(())
}
struct Native {
...
impl Native {
    ...
    fn frame(
        ...
        sync(&self.conn, &mut self.queue, &mut self.state, check)?;
            ...
            !self.state.removed
                && self.state.seats.len() == 1
                && self.state.managers.len() == 1,
            "Wayland topology changed during pointer action"
        );
        matching_output(
            &self.display,
            &self
    ...
    fn motion(
        ...
        p.validate()?;
        self.pointer
            .motion_absolute(self.time(), p.x, p.y, p.width, p.height);
        self.frame(check)
    }
    fn press(
        &mut self,
...
pub(super) fn preflight(
    ...
    check()?;
    let conn =
        Connection::connect_to_env().context("Cannot connect native Wayland pointer")?;
    let mut queue = conn.new_event_queue::<State>();
    let mut state = State::default();
    conn.display().get_registry(&queue.handle(), ());
    sync(&conn, &mut queue, &mut state, &mut check)?;
    ensure!(
        state.outputs.len() == 1 && state.seats.len() == 1 && state.managers.len() == 1,
        "Pointer requires one output, one seat and one supported virtual-pointer manager"
    );
    let xm = state
        .xmanager
        .as_ref()
        .context("Pointer requires xdg-output name and logical geometry")?;
    ensure!(xm.version() >= 2, "xdg-output names unsupported");
    for (i, (output, _)) in state.outputs.iter().enumerate() {
        xm.get_xdg_output(output, &queue.handle(), i);
    }
    sync(&conn, &mut queue, &mut state, &mut check)?;
    matching_output(
        display,
    ...
    ensure!(!state.removed, "Wayland topology changed");
    check()?;
    Ok(())
}
...
pub(super) fn send(
    ...
    match &command {
        ...
        Command::Click(p, b) => {
            p.validate()?;
            ensure!((0x110..=0x112).contains(b), "Unsupported button");
        }
        Command::Drag(a, b) => {
            a.validate()?;
            b.validate()?;
        ...
        Command::Scroll { amount, .. } => {
            ensure!((-20..=20).contains(amount), "Scroll outside -20..20")
        }
    }
    check()?;
    let conn =
    ...
    let pointer = if manager.version() >= 2 {
        manager.create_virtual_pointer_with_output(
            Some(&state.seats[0]),
            Some(&state.outputs[0].0),
            &queue.handle(),
            (),
        )
    } else {
        manager.create_virtual_pointer(Some(&state.seats[0]), &queue.handle(), ())
    };
    ...
    let result = (|| {
        ...
        match command {
            ...
            Command::Click(p, b) => {
                native.motion(p, &mut check)?;
                native.press(b, &mut check)?;
            }
            Command::Drag(a, b) => {
                native.motion(a, &mut check)?;
                native.press(0x110, &mut check)?;
                for step in 1..=20 {
                    native.motion(a.between(b, step), &mut check)?;
            ...
            Command::Scroll { amount, horizontal } => {
                ...
                if amount != 0 {
                    native.pointer.axis_discrete(
                        native.time(),
                        axis,
                        f64::from(amount) * 15.0,
                        amount,
                    );
                } else {
                    native.pointer.axis_stop(native.time(), axis);
                }
                native.frame(&mut check)?;
            }
        }
...
mod tests {
    use super::*;
    #[test]
    fn stalled_transport_and_cancellation_are_bounded_without_desktop_input() {
        ...
        let start = Instant::now();
        assert!(sync(&conn, &mut queue, &mut state, &mut || anyhow::bail!(
            "cancelled"
        ))
        .is_err());
    ...
    #[test]
    fn screenshot_ratios_are_not_rescaled() {
        let a = Point {
            x: 0,
            y: 0,
            width: 1600,
        ...
        assert_eq!(a.between(b, 20), b);
        assert_eq!(a.between(b, 10).x, 799);
        assert!(Point { x: 1600, ..a }.validate().is_err());
        assert!(Point { width: 0, ..a }.validate().is_err());
        assert_eq!(b.between(a, 20), a);
    }
    #[test]
    fn rejects_ambiguous_or_stale_outputs() {
        let d = Display {
            id: "DP-1".into(),
            x: -1920,
            y: 20,
        ...
        assert!(matching_output(&d, &[]).is_err());
        assert!(matching_output(&d, &[d.clone(), d.clone()]).is_err());
        for other in [
            Display {
                id: "DP-2".into(),
                ..d.clone()
            ...
            assert!(matching_output(&d, &[other]).is_err());
        }
    }
}
