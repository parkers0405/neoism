...
async fn failed_computer_batch_recovery_image_reaches_provider_with_error_status() {
    ...
    for (failed,image,preflight) in [(true,true,false),(true,false,false),(false,true,false),(true,false,true)] {
        ...
        if preflight {input["arguments"]["actions"]=json!([{"action":"click","frame":"fixture","x":0,"y":0},{"action":"type","text":"must not leak\n","method":"auto"}]);}
        ...
        if preflight {
            let payload=&result.metadata.as_ref().unwrap()["mcp"]["result"]["content"][0]["text"];
            let value:Value=serde_json::from_str(payload.as_str().unwrap()).unwrap();
            assert_eq!(value["status"],"failed");assert_eq!(value["completed"],0);assert_eq!(value["clipboard"],"unchanged");assert_eq!(value["partialActionPossible"],false);
        ...
        assert!(tool.content.contains(if preflight {"no C0/C1"}else{"recovery-frame"}));
...
async fn clipboard_consent_reaches_worker_only_through_explicit_registry_grant() {
    ...
    use neoism_agent_service_api::{BuiltinMcpCallResult,BuiltinMcpContent};
    let _revocation=crate::computer_use::TEST_REVOCATION_LOCK.lock().await;
    let root=std::env::temp_dir().join(format!("neoism-clipboard-consent-{}",Id::ascending(IdKind::Event)));
    std::fs::create_dir_all(root.join(".agent")).unwrap();
    std::fs::write(root.join(".agent/agent.json"),r#"{"mcp":{"computer":{"type":"local","command":["builtin","computer"],"enabled":true}}}"#).unwrap();
    let state=AppState::open_database_with_services(root.join("state.db"),crate::standard_services()).await.unwrap();
    let directory=root.to_str().unwrap();let snapshot=state.refreshed_plugin_snapshot(directory).await;
    let calls=Arc::new(AtomicUsize::new(0));let native_calls=calls.clone();
    crate::computer_use::with_test_backend(Arc::new(move |tool,_| {
        ...
        let missing=crate::agent_tool_registry::execute_mcp_tool_by_runtime_id(directory,"mcp__computer__batch",arguments.clone(),&base,None,Some(state.clone()),&snapshot).await.err().unwrap();
        ...
        let denied=crate::agent_tool_registry::execute_mcp_tool_by_runtime_id(directory,"mcp__computer__batch",arguments.clone(),&denied,None,Some(state.clone()),&snapshot).await.err().unwrap();
        ...
        let result=crate::agent_tool_registry::execute_mcp_tool_by_runtime_id(directory,"mcp__computer__batch",arguments.clone(),&allowed,None,Some(state.clone()),&snapshot).await.unwrap().unwrap();
    ...
    state.inner.store.close().await;let _=std::fs::remove_dir_all(root);
}
