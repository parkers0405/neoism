...
fn configured_endpoint() -> anyhow::Result<String> {
    let runtime = RUNTIME_BINDING
        .lock()
        .map_err(|_| anyhow::anyhow!("Browser binding state poisoned"))?
        .as_ref()
...
fn process_family(selected: u32, processes: &[ProcessInfo]) -> HashSet<u32> {
    ...
    for _ in 0..64 {
        let add = processes
            .iter()
            .filter(|p| family.contains(&p.ppid))
            .map(|p| p.pid)
            .collect::<Vec<_>>();
        let old = family.len();
        family.extend(add);
...
fn select_debug_process(
    ...
    for _ in 0..64 {
        ...
        if let Some(port) = processes
            .iter()
            .find(|p| p.pid == pid)
            .and_then(|p| debugging_port(&p.args))
        {
            flagged = Some((port, pid));
            break;
        }
        let Some(parent) = parents.get(&pid).copied().filter(|p| *p > 1) else {
            break;
        };
        pid = parent;
    }
    let flagged = flagged.context(
        "Selected browser was not started with an explicit --remote-debugging-port",
    )?;
    let family = process_family(flagged.1, processes);
    let inodes = listeners
        .get(&flagged.0)
        .context("The selected browser's debugging port is not listening on loopback")?;
...
fn proc_stat(value: &str) -> anyhow::Result<(u32, String)> {
    let fields = value
        ...
        .collect::<Vec<_>>();
    Ok((
        fields.get(1).context("Missing parent PID")?.parse()?,
        fields.get(19).context("Missing process birth")?.to_string(),
    ))
}
#[cfg(target_os = "linux")]
fn linux_processes(selected: u32, check: &Check) -> anyhow::Result<Vec<ProcessInfo>> {
    ...
    for entry in std::fs::read_dir("/proc")?.take(4096) {
        ...
        let Ok((ppid, birth)) = proc_stat(&stat) else {
            continue;
        };
    ...
    for _ in 0..64 {
        ...
        let Some(process) = result.iter_mut().find(|p| p.pid == pid) else {
            break;
        };
        let mut raw = Vec::new();
        if let Ok(file) = std::fs::File::open(format!("/proc/{pid}/cmdline")) {
            file.take(64 * 1024 + 1).read_to_end(&mut raw)?;
                ...
                raw.len() <= 64 * 1024,
                "Selected browser command line exceeds limit"
            );
            process.args = raw
                .split(|b| *b == 0)
                .filter(|s| !s.is_empty())
                ...
                .collect();
            if debugging_port(&process.args).is_some() {
                flagged = Some(pid);
                break;
            }
            process.args.clear();
    ...
    for process in result.iter_mut().filter(|p| family.contains(&p.pid)) {
        check.fast()?;
        process.exe = std::fs::read_link(format!("/proc/{}/exe", process.pid))
            .ok()
...
fn loopback_listeners() -> anyhow::Result<HashMap<u16, HashSet<u64>>> {
    ...
    for path in ["/proc/net/tcp", "/proc/net/tcp6"] {
        for line in std::fs::read_to_string(path)?.lines().skip(1).take(65536) {
            ...
            if loopback {
                if let (Ok(port), Ok(inode)) =
                    (u16::from_str_radix(port, 16), f[9].parse())
                {
                    result.entry(port).or_default().insert(inode);
                }
            }
        }
    }
...
fn http_version(port: u16, check: &Check) -> anyhow::Result<String> {
    ...
    let split = loop {
        if let Some(split) = raw.windows(4).position(|w| w == b"\r\n\r\n") {
            ...
            raw.len() < MAX_RESPONSE,
            "Browser version response headers too large"
        );
        check.fast()?;
        let mut chunk = [0; 4096];
        let limit = chunk.len().min(MAX_RESPONSE - raw.len());
        let read = stream
            .read(&mut chunk[..limit])
            .context("Could not read browser version response headers")?;
        ensure!(
            read > 0,
            "Malformed browser version response: truncated headers"
        );
    ...
    let mut lines = head.split("\r\n");
        ...
        lines.next().is_some_and(
            |s| s.starts_with("HTTP/1.1 200 ") || s.starts_with("HTTP/1.0 200 ")
        ),
        "Browser version endpoint did not return 200 (redirects are refused)"
    );
    let mut content_length = None;
    for line in lines {
        let (name, value) = line
            .split_once(':')
            .context("Malformed browser version response header")?;
        if name.eq_ignore_ascii_case("transfer-encoding") && !value.trim().is_empty() {
            bail!("Unsupported browser version response framing: Transfer-Encoding is not supported")
        }
        if name.eq_ignore_ascii_case("content-length") {
            ensure!(
                content_length.is_none(),
                "Malformed browser version response: duplicate Content-Length"
            );
            content_length = Some(
                value
                    .trim()
                    .parse::<usize>()
                    .context("Malformed browser version response Content-Length")?,
            );
        }
    }
    let content_length = content_length.context(
        "Unsupported browser version response framing: Content-Length is required",
    )?;
    let body_start = split + 4;
    let expected = body_start
        .checked_add(content_length)
        .context("Browser version response too large")?;
    ensure!(
        expected <= MAX_RESPONSE,
        "Browser version response too large"
    );
    while raw.len() < expected {
        check.fast()?;
        let mut chunk = [0; 4096];
        let limit = chunk.len().min(expected - raw.len());
        let read = stream
            .read(&mut chunk[..limit])
            .context("Could not read browser version response body")?;
        ensure!(
            read > 0,
            "Malformed browser version response: truncated body"
        );
        raw.extend_from_slice(&chunk[..read]);
    }
    let value: Value = serde_json::from_slice(&raw[body_start..expected])?;
    let ws = value["webSocketDebuggerUrl"]
        .as_str()
        .context("No Chromium browser WebSocket in /json/version")?;
    let (_, address) = endpoint(ws)?;
    ensure!(
        address.port() == port,
        "Discovered browser endpoint changed ports"
    );
    check.check()?;
    Ok(ws.to_owned())
}
fn native_fallback(reason: impl std::fmt::Display) -> BuiltinMcpCallResult {
    text(
        json!({"attached":false,"domAvailable":false,"reason":reason.to_string(),"nativeFallback":"Use computer.screenshot and computer.input with this same active-window target immediately. Do not perform DOM action fallback automatically after an uncertain failed DOM action, and do not ask for browser/profile/restart setup unless the user explicitly requests DOM access."}),
    )
}
...
fn attach(arguments: Value, check: &Check) -> anyhow::Result<BuiltinMcpCallResult> {
    ...
    let args: AttachArgs = serde_json::from_value(arguments)?;
    let window = windows::resolve(&args.target)?;
    ...
    if let Some(existing) = existing {
        if existing.window.pid == window.pid
            && existing.window.app == window.app
            && existing.birth == windows::process_birth(&window)?
        {
            validate_binding(Some(&window))?;
            let tabs =
                match with_connection(&existing.endpoint, &scoped, |c| c.tabs(&scoped)) {
                    Ok(tabs) => tabs,
                    Err(error) => {
                        scoped.fast()?;
                        return Ok(native_fallback(format!(
                            "DOM attachment failed: {error:#}"
                        )));
                    }
                };
            return Ok(text(
                json!({"attached":true,"domAvailable":true,"connection":"already_attached","tabs":tabs,"target":args.target,"next":"Call computer.focus with this target before browser_observe/browser_goal/browser_step if it is not already foreground. Attachment does not change focus."}),
            ));
        }
    ...
    let candidate = (|| -> anyhow::Result<(String, u32, String)> {
        let processes = linux_processes(window.pid, &scoped)?;
        let listeners = loopback_listeners()?;
        let (port, listener_pid) =
            select_debug_process(window.pid, &processes, &listeners)?;
        let listener = processes
            .iter()
            .find(|p| p.pid == listener_pid)
            .context("Browser listener process changed during discovery")?;
        let explicit = args
            .endpoint
            .as_deref()
            .map(|s| {
            ...
            .transpose()?;
        let known_firefox = listener.exe.to_ascii_lowercase().contains("firefox")
            || listener.exe.to_ascii_lowercase().contains("zen");
        let endpoint = match explicit {
            Some(value) => {
                ensure!(
                    endpoint(&value)?.1.port() == port,
                    "Explicit endpoint port does not match selected browser"
                );
                value
            }
            None => http_version(port, &scoped).or_else(|e| {
                if known_firefox {
                    Ok(format!("ws://127.0.0.1:{port}/session"))
                } else {
                    Err(e)
                }
            })?,
    ...
    let (candidate, listener_pid, listener_birth) = match candidate {
        Ok(v) => v,
        Err(e) => {
            scoped.fast()?;
            return Ok(native_fallback(format!("{e:#}")));
        }
    };
    #[cfg(target_os = "linux")]
    let verify_listener = || -> anyhow::Result<()> {
        let stat = std::fs::read_to_string(format!("/proc/{listener_pid}/stat"))?;
        ensure!(
            proc_stat(&stat)?.1 == listener_birth,
            "Browser listener process changed during attachment"
        );
        Ok(())
    };
    #[cfg(not(target_os = "linux"))]
    ...
    scoped.check()?;
    verify_listener()?;
    let tabs = match with_connection(&candidate, &scoped, |c| c.tabs(&scoped)) {
        Ok(tabs) => tabs,
        Err(error) => {
            scoped.fast()?;
            return Ok(native_fallback(format!("DOM attachment failed: {error:#}")));
        }
    };
    verify_listener()?;
    scoped.check()?;
    *RUNTIME_BINDING
        ...
        Some(RuntimeBinding {
            endpoint: candidate,
            window: window.clone(),
            birth: windows::process_birth(&window)?,
            listener_pid,
            listener_birth,
        });
    Ok(text(
        json!({"attached":true,"domAvailable":true,"connection":"attached","target":args.target,"tabs":tabs,"note":"Bound in memory to the selected browser process lifetime; no browser/profile/environment change and no Neoism restart.","next":"Call computer.focus with this target before browser_observe/browser_goal/browser_step if it is not already foreground. Attachment does not change focus."}),
    ))
}
...
fn validate_binding(target: Option<&windows::Window>) -> anyhow::Result<()> {
    ...
    let lifetime = (|| -> anyhow::Result<()> {
            ...
            windows::process_birth(&binding.window)? == binding.birth,
            "Attached browser process lifetime changed; attach again"
        );
        #[cfg(target_os = "linux")]
        {
            let stat =
                std::fs::read_to_string(format!("/proc/{}/stat", binding.listener_pid))?;
            ensure!(
                proc_stat(&stat)?.1 == binding.listener_birth,
                "Browser listener process lifetime changed; attach again"
            );
        }
        Ok(())
    })();
    if lifetime.is_err() {
        ...
        OBSERVATIONS
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .retain(|o| o.endpoint != binding.endpoint);
        return lifetime;
    }
    if let Some(target) = target {
        ensure!(
            windows::same_process(&binding.window, target)?,
            "Native target belongs to a different browser process than browser_attach"
        )
    }
    Ok(())
}
...
fn disconnect(check: &Check) -> anyhow::Result<BuiltinMcpCallResult> {
    ...
    if let Some((endpoint, mut connection)) = cached {
        ...
        *RUNTIME_BINDING
            .lock()
            .map_err(|_| anyhow::anyhow!("Browser binding state poisoned"))? = None;
        return Ok(text(
    ...
    let cleared = RUNTIME_BINDING
        .lock()
        .map_err(|_| anyhow::anyhow!("Browser binding state poisoned"))?
        .take()
        .is_some();
    Ok(text(
        json!({"disconnected":cleared,"browserClosed":false,"note":if cleared {"Runtime Chromium binding cleared; browser remains open."} else {"No runtime browser binding or owned Firefox session."}}),
    ))
}
...
pub(super) fn validate_goal_url(value: &str) -> anyhow::Result<()> {
        ...
        let operation = (|| -> anyhow::Result<Value> {
            if tool == "browser_act" {
                ...
                let element = args.element.as_deref();
                    ...
                    !element_action || element.is_some(),
                    "click/fill/select requires ref"
                );
                if let Some(element) = element {
                    ...
                    matches!(action, "click" | "back") || args.value.is_some(),
                    "fill/select/scroll/navigate requires value"
                );
                ensure!(
                    args.value
                        .as_ref()
                ...
                if action == "scroll" {
                        ...
                        matches!(args.value.as_deref(), Some("up" | "down")),
                        "scroll value must be up or down"
                    );
                }
                if action == "navigate" {
                    validate_page_url(args.value.as_deref().unwrap_or_default())
                        .context("navigate requires an exact HTTP(S) URL")?;
                }
                if action == "back" {
                    ensure!(args.value.is_none(), "back does not accept value");
                }
...
mod tests {
    ...
    fn firefox_endpoint_selection_is_explicit() {
        ...
        assert_eq!(
            select_configured(
                Some("ws://127.0.0.1:9333/session".into()),
                Some("ignored".into()),
                Some("ignored".into())
            )
            .unwrap(),
    ...
    fn process(pid: u32, ppid: u32, args: &[&str], sockets: &[u64]) -> ProcessInfo {
        ProcessInfo {
            pid,
            ppid,
            birth: format!("birth-{pid}"),
            exe: "zen-bin".into(),
            args: args.iter().map(|s| s.to_string()).collect(),
            sockets: sockets.iter().copied().collect(),
        }
    }
    #[test]
    fn selected_process_family_discovery_requires_flag_and_owned_loopback_socket() {
        let processes = vec![
            process(10, 1, &["zen", "--remote-debugging-port", "9223"], &[]),
            process(11, 10, &["zen", "-contentproc"], &[77]),
            process(99, 1, &["other"], &[88]),
        ];
        let listeners = HashMap::from([(9223, HashSet::from([77]))]);
        assert_eq!(
            select_debug_process(10, &processes, &listeners).unwrap(),
            (9223, 11)
        );
        assert!(select_debug_process(99, &processes, &listeners)
            .unwrap_err()
            .to_string()
            .contains("explicit"));
        assert!(select_debug_process(
            10,
            &processes,
            &HashMap::from([(9223, HashSet::from([88]))])
        )
        .unwrap_err()
        .to_string()
        .contains("not owned"));
    ...
    fn debugging_port_accepts_only_explicit_bounded_forms() {
        assert_eq!(
            debugging_port(&["zen".into(), "--remote-debugging-port=9223".into()]),
            Some(9223)
        );
        assert_eq!(
            debugging_port(&[
        ...
        assert_eq!(
            debugging_port(&["zen".into(), "--remote-debugging-port=0".into()]),
            None
        );
        ...
        assert_eq!(
            debugging_port(&["helium --remote-debugging-port 9222".into()]),
            Some(9222)
        );
        for title in [
            "helium prefix--remote-debugging-port=9222",
            "helium --remote-debugging-port=9222suffix",
            "helium --remote-debugging-port=0",
            "helium --remote-debugging-port=65536",
            "helium --remote-debugging-port",
        ] {
        ...
        assert_eq!(
            debugging_port(&[
                "helium".into(),
                "--title=example --remote-debugging-port=9222".into()
            ]),
            None
        );
        ...
        assert_eq!(
            select_debug_process(
                10,
                &processes,
                &HashMap::from([(9222, HashSet::from([77]))])
            )
            .unwrap(),
            (9222, 10)
        );
        assert!(select_debug_process(
            10,
            &processes,
            &HashMap::from([(9222, HashSet::from([88]))])
        )
        .is_err());
    }
    #[cfg(target_os = "linux")]
    ...
    fn wrong_target_rejection_preserves_healthy_binding_but_dead_listener_clears_it() {
        ...
        *RUNTIME_BINDING.lock().unwrap() = Some(RuntimeBinding {
            endpoint: "ws://127.0.0.1:9223/session".into(),
            window: window.clone(),
            birth,
            listener_pid: pid,
            listener_birth,
        ...
        wrong.app = "other".into();
        assert!(validate_binding(Some(&wrong))
            .unwrap_err()
            .to_string()
            .contains("different browser process"));
        assert!(
            RUNTIME_BINDING.lock().unwrap().is_some(),
            "wrong target must not discard a healthy attachment"
        );
        RUNTIME_BINDING
            .lock()
            .unwrap()
            .as_mut()
            .unwrap()
            .listener_birth = "stale".into();
        assert!(validate_binding(None).is_err());
        assert!(
            RUNTIME_BINDING.lock().unwrap().is_none(),
            "dead listener lifetime must invalidate attachment"
        );
    }
    #[test]
    ...
    fn owned_bidi_session_survives_tab_failure_for_explicit_cleanup() {
        ...
        let worker = std::thread::spawn(move || {
            ...
            socket
                .send(Message::Text(
                    json!({"type":"success","id":request["id"],"result":{}}).to_string(),
                ))
                .unwrap();
        });
        run_worker(
            ...
            Arc::new(AtomicBool::new(false)),
            |check| {
                assert!(with_connection(&endpoint, check, |c| c.tabs(check))
                    .unwrap_err()
                    .to_string()
                    .contains("fixture tab failure"));
                assert!(
                    BIDI_CONNECTION.lock().unwrap().is_some(),
                    "owned session must remain available to browser_disconnect"
                );
                assert_ne!(disconnect(check)?.is_error, Some(true));
                Ok(())
            },
    ...
    fn expectations_and_deltas_are_bounded_data() {
        ...
        assert_eq!(
            ...
            json!(["target", "tab", "observation", "action"])
        );
