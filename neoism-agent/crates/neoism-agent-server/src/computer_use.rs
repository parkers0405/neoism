...
#[derive(Clone)]
struct Frame { target: Option<windows::Window>, id: String, display: Display, width: u32, height: u32, region:capture::Region, native_width:u32, native_height:u32, created: Instant, epoch: u64 }
...
#[derive(Clone)]
struct Check { progress:Option<Arc<AtomicUsize>>, effects: Arc<Mutex<input_pipeline::Effects>>, clipboard_authorized:bool, target: Option<windows::Window>, require_foreground:bool, cancel: Arc<AtomicBool>, dropped: Arc<AtomicBool>, epoch: u64, started: Instant }
impl Check {
    ...
    fn check(&self) -> anyhow::Result<()> {
        self.check_probe(|| latency::measure("target_validation",|| {
            if let Some(target)=&self.target { windows::validate(target,self.require_foreground)?; }
            Ok(())
        }))
    }
...
fn run_worker<T>(epoch: u64, cancel: Arc<AtomicBool>, dropped: Arc<AtomicBool>, backend: impl FnOnce(&Check) -> anyhow::Result<T>) -> anyhow::Result<T> {
    ...
    let check = Check { progress:None, effects:Arc::default(),clipboard_authorized:false, target:None, require_foreground:true, epoch, cancel, dropped, started: Instant::now() };
    check.check()?;
...
fn execute_locked(tool: &str, mut arguments: Value, check: &Check) -> anyhow::Result<BuiltinMcpCallResult> {
    check.check()?;
    if matches!(tool,"browser_attach"|"browser_tabs"|"browser_observe"|"browser_act"|"browser_disconnect") {
    ...
    let scoped = Check { progress:check.progress.clone(), effects:check.effects.clone(), clipboard_authorized:check.clipboard_authorized, cancel:check.cancel.clone(), dropped:check.dropped.clone(),epoch:check.epoch,started:check.started,target,require_foreground:true };
    let check = if scoped.target.is_some() { &scoped } else { check };
    check.check()?;
    match tool {
        "screenshot" => {
            #[derive(Deserialize)] #[serde(deny_unknown_fields)] struct Args { display: String, crop:Option<capture::Region>, #[serde(default="default_settle_ms")] settle_ms:u64 }
            let args: Args = serde_json::from_value(arguments)?;
            #[cfg(target_os="linux")]
            ...
            ensure!(u64::from(native_width)*u64::from(native_height)<=64_000_000,"Physical display exceeds capture limit");
            let region=args.crop.unwrap_or(capture::Region{x:0,y:0,width:native_width,height:native_height});
            region.validate(native_width,native_height)?;
            let image=if args.crop.is_some() {image.crop_imm(region.x,region.y,region.width,region.height)} else {image};
            ...
            let frame = Frame { target:current_target, id: format!("{:032x}", rand::random::<u128>()), display, width: encoded.width, height: encoded.height, region,native_width,native_height, created: Instant::now(), epoch: check.epoch };
            check.check()?;
            let mut result = text(json!({"frame":frame.id,"display":frame.display,"imageWidth":frame.width,"imageHeight":frame.height,"crop":frame.region,"nativeWidth":native_width,"nativeHeight":native_height,"coordinates":"image pixels, origin top-left of crop; pass frame unchanged","expiresAfterSeconds":30,"targetWindow":frame.target,"settling":settling}));
...
fn pointer_action(action: Action, displays: &[Display], check: &Check) -> anyhow::Result<BuiltinMcpCallResult> {
            #[cfg(target_os="linux")]
            {
                ...
                let point=|id:&str,x,y|->anyhow::Result<(Display,Point)> {
                    ...
                    Ok((frame.display,Point{x,y,width:frame.native_width,height:frame.native_height}))
                };
...
struct Batch { actions: Vec<Value>, target: Option<String>, screenshot: Option<String>, crop:Option<capture::Region>, #[serde(default="default_settle_ms")] settle_ms:u64 }
fn validate_batch(args:&Batch)->anyhow::Result<()> {
    ...
    ensure!(args.crop.is_none() || args.screenshot.is_some(),"Batch crop requires a final screenshot");
    if let Some(crop)=args.crop {crop.validate(u32::MAX,u32::MAX)?;}
    ensure!(args.settle_ms<=3000,"settle_ms must be 0..3000");
...
fn coordinates(id: &str, x: u32, y: u32, displays: &[Display], input: &Enigo, check: &Check) -> anyhow::Result<(i32,i32)> {
    let frame=coordinate_frame(id,x,y,displays,check)?;
    let (x,y)=frame.region.point(x,y,frame.width,frame.height);
    platform::coordinates(&frame.display, x, y, frame.native_width, frame.native_height, displays, input)
}
...
mod tests {
    ...
    #[test] fn cropped_coordinates_map_to_native_display() {
        let region=capture::Region{x:400,y:200,width:800,height:400};
        ...
        assert!(capture::Region{x:u32::MAX,y:0,width:2,height:1}.validate(1920,1080).is_err());
        assert!(capture::Region{x:0,y:0,width:0,height:1}.validate(1920,1080).is_err());
        assert!(region.validate(500,300).is_err());
    }
    #[test] fn browser_tools_share_explicit_permission_and_no_direct_dispatch() {
        let tools=ComputerUse.tools();
        for name in ["browser_attach","browser_tabs","browser_observe","browser_act","browser_disconnect"] {
            assert!(tools.iter().any(|t|t.name==name));
            assert!(ComputerUse.call_tool(Path::new("/"),name,json!({})).is_err());
        }
        let action=tools.iter().find(|t|t.name=="browser_act").unwrap();
        assert!(action.input_schema["required"].as_array().unwrap().contains(&json!("target")));
        assert!(action.input_schema["required"].as_array().unwrap().contains(&json!("observation")));
    }
    ...
    #[tokio::test] async fn browser_setup_is_discoverable_before_permission_approval() {
        let result=ComputerUse.call_tool_authorized_async(Path::new("/"),"capabilities",json!({}),false,Arc::new(AtomicBool::new(false)),None).await.unwrap();
        let BuiltinMcpContent::Text{text,..}=&result.content[0] else {panic!("JSON capabilities result")};
        ...
        assert_eq!(browser["firefox"]["endpointVariable"],"NEOISM_BROWSER_BIDI_URL");
        assert_eq!(browser["firefox"]["endpointExample"],"ws://127.0.0.1:9223/session");
        assert!(browser["firefox"]["posixSetup"][1].as_str().unwrap().contains("--remote-debugging-port 9223"));
        assert!(browser["firefox"]["cleanup"].as_str().unwrap().contains("browser_disconnect"));
        assert_eq!(browser["chromium"]["endpointVariable"],"NEOISM_BROWSER_CDP_URL");
        assert!(browser["recommended"].as_str().unwrap().contains("browser_attach"));
        assert!(browser["environment"].as_str().unwrap().contains("Advanced"));
    }
    #[tokio::test] async fn unauthorized_async_calls_never_reach_backend() {
        for tool in ["input","batch","windows","focus","wait","screenshot","browser_attach","browser_tabs","browser_observe","browser_act","browser_disconnect"] {
    ...
    #[tokio::test] async fn batch_stops_before_next_action_on_cancel_or_revocation() {
        ...
        for revoke in [false,true] {
            let check=Check { progress:None,effects:Arc::default(),clipboard_authorized:false,target:None,require_foreground:true,cancel:Arc::new(AtomicBool::new(false)),dropped:Arc::new(AtomicBool::new(false)),epoch:STOP.load(Ordering::SeqCst),started:Instant::now() };
            let mut delivered=0;
    ...
    #[tokio::test] async fn batch_returns_progress_and_final_image_but_never_captures_after_cancel() {
        ...
        for (cancel,panics) in [(false,false),(true,false),(false,true)] {
            let check=Check { progress:Some(Arc::new(AtomicUsize::new(0))),effects:Arc::default(),clipboard_authorized:false,target:None,require_foreground:true,cancel:Arc::new(AtomicBool::new(false)),dropped:Arc::new(AtomicBool::new(false)),epoch:STOP.load(Ordering::SeqCst),started:Instant::now() };
    ...
    #[test] fn cancellation_before_paste_does_not_claim_clipboard_effects() {
        let _revocation=TEST_REVOCATION_LOCK.blocking_lock();
        let check=Check {progress:None,effects:Arc::default(),clipboard_authorized:false,target:None,require_foreground:true,cancel:Arc::new(AtomicBool::new(true)),dropped:Arc::new(AtomicBool::new(false)),epoch:STOP.load(Ordering::SeqCst),started:Instant::now()};
    ...
    #[test] fn stale_frames_topology_changes_and_out_of_bounds_are_rejected() {
        ...
        let mut frame = Frame { target:None, id:"frame".into(),display,width:1600,height:900,region:capture::Region{x:0,y:0,width:1920,height:1080},native_width:1920,native_height:1080,created:Instant::now(),epoch:7 };
    ...
    #[test] fn dropping_call_cancels_worker() {
        let dropped = Arc::new(AtomicBool::new(false));
        let check = Check { progress:None, effects:Arc::default(),clipboard_authorized:false, target:None, require_foreground:true, cancel:Arc::new(AtomicBool::new(false)), dropped:dropped.clone(),epoch:STOP.load(Ordering::SeqCst),started:Instant::now() };
