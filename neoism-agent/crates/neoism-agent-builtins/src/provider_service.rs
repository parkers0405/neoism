...
impl ProviderPlatform {
    pub fn from_env() -> Self {
        Self::new(Arc::new(neoism_agent_service_api::LocalProviderCredentialStore::from_environment()))
    }

    pub fn new(credentials: Arc<dyn neoism_agent_service_api::ProviderCredentialStore>) -> Self {
    ...
    async fn execute_route(&self, request: ProviderRouteRequest) -> anyhow::Result<Value> {
        ...
        let auth = self.auth.scoped(scope.clone(), request.connection_id.clone());
        match request.action {
            ProviderRouteAction::List => {
                let raw = self.catalog.providers().await?;
                let connected = self.registry.connected_ids(&raw).await?;
                let mut all = effective_provider_catalog(&raw, openai_codex_oauth(&auth).await);
            ...
            ProviderRouteAction::Configured => {
                let raw = self.catalog.providers().await?;
                let connected = self.registry.connected_ids(&raw).await?;
                let providers = usable_provider_catalog(&raw, &connected, openai_codex_oauth(&auth).await);
            ...
            ProviderRouteAction::AuthGet => Ok(Value::Bool(auth.get(&provider_id).await?.is_some())),
            ProviderRouteAction::AuthSet => {
                auth.set(&provider_id, serde_json::from_value::<AuthInfo>(request.body)?).await?;
                Ok(Value::Bool(true))
            }
            ...
            ProviderRouteAction::OAuthAuthorize => {
                ...
                let mut authorization: Option<ProviderAuthAuthorization> = crate::provider_auth::authorize(
                    &provider_id, &attempt_id, &input.method, &input.inputs, &providers, &auth, &self.oauth,
                ).await?;
                if let Some(authorization) = authorization.as_mut() {
                    ...
                    self.oauth_attempts.write().await.insert(attempt_id, OAuthAttempt {
                        provider_id: provider_id.clone(), scope, hosted: request.hosted,
                        connection_id: input.connection_id, label: input.label,
                        expires_at,
                    });
                }
            ...
            ProviderRouteAction::OAuthCallback => {
                ...
                let providers = self.catalog.providers().await?;
                let (pending_key, target_scope, target_connection, label) = if let Some(attempt_id) = input.attempt_id.as_deref() {
                    let attempt = self.oauth_attempts.write().await.remove(attempt_id).ok_or_else(|| anyhow::anyhow!("OAuth attempt is missing or already used"))?;
                    if attempt.expires_at < crate::now_millis() { anyhow::bail!("OAuth attempt expired") }
                    if attempt.provider_id != provider_id || attempt.scope != scope || attempt.hosted != request.hosted { anyhow::bail!("OAuth attempt scope does not match this request") }
                    (attempt_id.to_string(), attempt.scope, attempt.connection_id, attempt.label)
                } else {
                    (provider_id.clone(), scope, request.connection_id, None)
                };
                let credential = crate::provider_auth::callback(
                    &provider_id, &pending_key, &input.method, input.code.as_deref(), &providers, &self.oauth,
                ).await?;
                let target = self.auth.scoped(target_scope.clone(), target_connection.clone());
                let created = if target_connection.is_some() {
                    target.set(&provider_id, credential).await?;
                    None
                } else if input.attempt_id.is_some() {
                    Some(self.auth.service().create(neoism_agent_service_api::CreateProviderConnection {
                        provider_id: provider_id.clone(), label: label.unwrap_or_else(|| "Default".into()), scope: target_scope,
                        credential: into_credential(credential), set_default: false,
                    }).await.map_err(|error| anyhow::anyhow!(error.to_string()))?)
                } else {
                    target.set(&provider_id, credential).await?;
                    None
                };
                Ok(created.map(serde_json::to_value).transpose()?.unwrap_or(Value::Bool(true)))
            }
            ProviderRouteAction::ConnectionsList => Ok(serde_json::to_value(self.auth.service().list(Some(&provider_id), &scope).await.map_err(|error| anyhow::anyhow!(error.to_string()))?)?),
            ProviderRouteAction::ConnectionsCreate => {
                let input: CreateConnectionRequest = serde_json::from_value(request.body)?;
                let summary = self.auth.service().create(neoism_agent_service_api::CreateProviderConnection {
                    provider_id, label: input.label, scope, credential: into_credential(input.credential), set_default: input.set_default,
                }).await.map_err(|error| anyhow::anyhow!(error.to_string()))?;
                Ok(serde_json::to_value(summary)?)
            }
            ProviderRouteAction::ConnectionsRename => {
                let input: RenameConnectionRequest = serde_json::from_value(request.body)?;
                let connection = ProviderConnectionRef { provider_id, connection_id: request.connection_id.ok_or_else(|| anyhow::anyhow!("connection ID is required"))? };
                Ok(serde_json::to_value(self.auth.service().rename(&connection, &scope, &input.label).await.map_err(|error| anyhow::anyhow!(error.to_string()))?)?)
            }
            ProviderRouteAction::ConnectionsDelete => {
                let connection = ProviderConnectionRef { provider_id, connection_id: request.connection_id.ok_or_else(|| anyhow::anyhow!("connection ID is required"))? };
                Ok(Value::Bool(self.auth.service().delete(&connection, &scope).await.map_err(|error| anyhow::anyhow!(error.to_string()))?))
            }
            ProviderRouteAction::ConnectionsSetDefault => {
                let connection = ProviderConnectionRef { provider_id, connection_id: request.connection_id.ok_or_else(|| anyhow::anyhow!("connection ID is required"))? };
                Ok(serde_json::to_value(self.auth.service().set_default(&connection, &scope).await.map_err(|error| anyhow::anyhow!(error.to_string()))?)?)
            }
        }
    }
}
...
impl ProviderService for ProviderPlatform {
    ...
    fn stream<'a>(&'a self, request: ProviderGenerationRequest) -> PluginFuture<'a, ProviderStream> {
        Box::pin(async move {
            let stream = self.registry.stream(request).await.map_err(runtime_error)?;
            Ok(ProviderStream { provider_id: stream.provider_id, model_id: stream.model_id, events: Box::pin(stream.events.map(|event| event.map_err(runtime_error))) })
        })
    }
    ...
    fn model_metadata<'a>(&'a self, model: &'a UserModel) -> PluginFuture<'a, ProviderModelMetadata> {
        Box::pin(async move {
            ...
            let scoped = self.auth.scoped(CredentialScope::local(), model.connection_id.clone());
            let metadata = generation_metadata(&providers, model, openai_codex_oauth(&scoped).await);
...
#[derive(Deserialize)]
struct ProviderCallbackRequest { method: Value, code: Option<String>, #[serde(default, rename = "attemptId")] attempt_id: Option<String> }
...
fn opaque_attempt_id() -> String {
    format!("attempt_{}", rand::thread_rng().sample_iter(&Alphanumeric).take(40).map(char::from).collect::<String>())
}

#[derive(Deserialize)]
#[serde(rename_all = "camelCase")]
struct CreateConnectionRequest { label: String, credential: AuthInfo, #[serde(default)] set_default: bool }
#[derive(Deserialize)]
struct RenameConnectionRequest { label: String }

fn into_credential(info: AuthInfo) -> neoism_agent_service_api::ProviderCredential {
    match info {
        AuthInfo::Api { key, metadata } => neoism_agent_service_api::ProviderCredential::Api { key, metadata },
        AuthInfo::OAuth { refresh, access, expires, account_id, enterprise_url } => neoism_agent_service_api::ProviderCredential::OAuth { refresh, access, expires, account_id, enterprise_url },
        AuthInfo::WellKnown { key, token } => neoism_agent_service_api::ProviderCredential::WellKnown { key, token },
    }
}
