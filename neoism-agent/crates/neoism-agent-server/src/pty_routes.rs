...
mod executable_tests {
    ...
    fn pty_command_honors_injected_path_and_reports_missing_executable() {
        ...
        services.executables =
            Arc::new(FakeExecutableService::with("pty-shell", &injected));
        assert_eq!(
            resolve_pty_command(&services, "pty-shell", ".").unwrap(),
            injected
        );

        services.executables = Arc::new(FakeExecutableService::default());
        let error = resolve_pty_command(&services, "pty-shell", ".")
            .unwrap_err()
            .to_string();
...
pub(crate) async fn prepare_connection_with_runtime(
    ...
    Ok(std::sync::Arc::new(PtySocketSession {
        processes,
        info,
        cursor,
        publish_state,
...
impl neoism_agent_plugin_api::WebSocketSession for PtySocketSession {
    fn run<'a>(
        &'a self,
        socket: Box<dyn neoism_agent_plugin_api::PluginWebSocket>,
...
async fn find_pty_runtime(state: &AppState, pty_id: &str) -> Option<LeasedPtyRuntime> {
    for runtime in state.inner.workspace_runtimes.runtimes().await {
        ...
        let Some(ptys) = generation.pty_if_allocated() else {
            continue;
        };
        if ptys.infos.read().await.contains_key(pty_id) {
            return Some(LeasedPtyRuntime {
                _workspace: runtime,
