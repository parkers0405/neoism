...
const TEXT_POSITIONS: &[u32] = &[
    ...
    57, 58, 59, 60, 61, 65, 94, 97, 132, // ISO 102nd, JIS Ro and Yen.
];
...
fn transient(s: &xkb::State, group: u32) -> bool {
    s.serialize_mods(xkb::STATE_MODS_LATCHED | xkb::STATE_MODS_LOCKED) == 0
        ...
        && s.serialize_layout(xkb::STATE_LAYOUT_DEPRESSED | xkb::STATE_LAYOUT_LATCHED)
            == 0
}
...
fn forbidden(map: &xkb::Keymap) -> u32 {
    ["Control", "Mod1", "Mod4", "Alt", "Meta", "Super", "Hyper"]
        .iter()
        .fold(0, |mask, name| {
...
fn selector(map: &xkb::Keymap, group: u32, code: u32, sym: u32) -> bool {
    ...
    if s.key_get_one_sym(key).raw() != sym {
        return false;
    }
...
fn candidate(
    ...
    for modifier in modifiers {
        ...
        if !transient(&s, group)
            || s.serialize_mods(xkb::STATE_MODS_EFFECTIVE) & forbidden(map) != 0
        {
            return None;
        }
    }
    ...
    let ch = chars.next()?;
    if chars.next().is_some() || ch.is_control() || u32::from(ch) > 0xffff {
        return None;
    }
    // Dead keys and Multi_key must never initiate an application compose state.
    if !direct_printable(sym, ch) || s.key_get_utf8(key) != output {
        return None;
    }
    let before = s.serialize_mods(xkb::STATE_MODS_EFFECTIVE);
    s.update_key(key, xkb::KeyDirection::Down);
    if !transient(&s, group)
        || s.serialize_mods(xkb::STATE_MODS_EFFECTIVE) != before
        || s.key_get_utf8(key) != output
    {
        return None;
    }
    s.update_key(key, xkb::KeyDirection::Up);
    if !transient(&s, group) || s.serialize_mods(xkb::STATE_MODS_EFFECTIVE) != before {
        return None;
    }
    for modifier in modifiers.iter().rev() {
        s.update_key(xkb::Keycode::new(*modifier), xkb::KeyDirection::Up);
        if !transient(&s, group)
            || s.serialize_mods(xkb::STATE_MODS_EFFECTIVE) & !before != 0
        {
            return None;
        }
    }
...
pub(super) fn text(map: &xkb::Keymap, group: u32, text: &str) -> Result<Option<KeyPlan>> {
    super::validate_baseline(map)?;
        ...
        "Text cannot contain control characters; use explicit key actions"
    );
    if text.chars().any(|ch| u32::from(ch) > 0xffff) {
        return Ok(None);
    }
    let codes = TEXT_POSITIONS
        .iter()
        .copied()
        .filter(|code| map.key_get_name(xkb::Keycode::new(*code)).is_some())
        .collect::<Vec<_>>();
    let mut selector_codes = codes.clone();
    selector_codes.extend_from_slice(MODIFIER_POSITIONS);
    selector_codes.sort_unstable();
    selector_codes.dedup();
    let shifts = selector_codes
        .iter()
        .copied()
        .filter(|c| selector(map, group, *c, 0xffe1) || selector(map, group, *c, 0xffe2))
        .collect::<Vec<_>>();
    let level3 = selector_codes
        .iter()
        .copied()
        .filter(|c| selector(map, group, *c, 0xfe03))
        .collect::<Vec<_>>();
    ...
    modifiers.extend(
        shifts
            .iter()
            .flat_map(|s| level3.iter().map(move |l| vec![*s, *l])),
    );
    ...
    for mods in &modifiers {
        for code in &codes {
            if let Some(ch) = candidate(map, group, mods, *code) {
                candidates.entry(ch).or_insert_with(|| {
                    let mut unit = mods.clone();
                    unit.push(*code);
                    unit
                });
            }
    ...
    for ch in text.chars() {
        let Some(unit) = candidates.get(&ch) else {
            return Ok(None);
        };
    ...
    Ok(Some(KeyPlan {
        units,
        units_kind: "unicode_scalar",
        source: map.get_as_string(xkb::KEYMAP_FORMAT_TEXT_V1),
        group,
    }))
}
pub(super) fn keys(
    ...
    let codes = super::super::shortcuts::resolve_in_map(map, group, keys)?
        .into_iter()
        .map(u32::from)
        .collect::<Vec<_>>();
    let mut s = state(map, group);
    for code in &codes {
        s.update_key(xkb::Keycode::new(*code), xkb::KeyDirection::Down);
        ensure!(
            transient(&s, group),
            "Shortcut changes locked/latched modifiers or layout"
        );
    }
    for code in codes.iter().rev() {
        s.update_key(xkb::Keycode::new(*code), xkb::KeyDirection::Up);
    }
    ensure!(
        neutral(&s, group),
        "Shortcut does not release to neutral state"
    );
    Ok(KeyPlan {
        units_kind: "chord",
        units: if codes.is_empty() {
            vec![]
        } else {
            vec![codes]
        },
