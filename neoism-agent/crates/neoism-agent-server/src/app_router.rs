...
use crate::session_message_routes::{
    message_delete, message_get, part_delete, part_update,
};
...
use crate::session_routes::{
    session_create, session_delete, session_diff, session_directory_options,
    session_fork, session_get, session_set_pin, session_status, session_todo_list,
    session_update,
};
...
pub(crate) fn app_with_cors(state: AppState, allowed_origins: &[String]) -> Router {
    ...
    let router = Router::new()
        ...
        .route(
            "/v2/sessions/:session_id/prompt-async",
...
fn management_routes(state: &AppState) -> Router<AppState> {
    ...
    Router::new()
        .route(
            ...
            get(management::list_workspaces).post(management::create_workspace),
        )
        .route(
            "/v2/management/workspaces/:id",
            get(management::get_workspace)
                .put(management::update_workspace)
                .delete(management::delete_workspace),
        )
        .route(
            "/v2/management/repositories",
            get(management::list_repositories).post(management::create_repository),
        )
        .route(
            "/v2/management/repositories/:id",
            get(management::get_repository)
                .put(management::update_repository)
                .delete(management::delete_repository),
        )
        .route("/v2/management/agents", get(management::list_agents))
        .route(
            "/v2/management/agents/:id",
            get(management::get_agent)
                .post(management::create_agent)
                .put(management::update_agent)
                .delete(management::delete_agent),
        )
        .route("/v2/management/commands", get(management::list_commands))
        .route(
            "/v2/management/commands/:id",
            get(management::get_command)
                .post(management::create_command)
                .put(management::update_command)
                .delete(management::delete_command),
        )
        .route("/v2/management/skills", get(management::list_skills))
        .route(
            "/v2/management/skills/install",
            post(management::install_skill),
        )
        .route(
            "/v2/management/skills/:id",
            get(management::get_skill)
                .post(management::create_skill)
                .put(management::update_skill)
                .delete(management::delete_skill),
        )
        .route(
            "/v2/management/skills/:id/versions",
            get(management::list_skill_versions),
        )
        .route(
            "/v2/management/skills/:id/versions/:version",
            get(management::get_skill_version),
        )
        .route(
            "/v2/management/skills/:id/versions/:version/restore",
            post(management::restore_skill_version),
        )
}
...
async fn plugin_route_dispatch(
    ...
    let directory = if let Some(directory) = request_directory(&request) {
        ...
        match find_scoped_plugin_session(
            &state,
            request.uri().path(),
    ...
    if !snapshot.is_active() {
        return auth_error(
            StatusCode::GONE,
            "plugin.generation_closed",
            "The plugin generation is shut down",
        );
    }
    let websocket_route =
        snapshot
            ...
            .find_map(|registered| {
                (registered.route.descriptor.method.as_str() == request.method().as_str())
                    .then(|| {
                        match_plugin_path(
                            &registered.route.descriptor.path,
                            request.uri().path(),
                        )
                    })
                    .flatten()
    ...
    let runtime_route = snapshot.runtime_routes.values().find_map(|registered| {
        (registered.route.descriptor.method.as_str() == request.method().as_str())
            .then(|| {
                match_plugin_path(&registered.route.descriptor.path, request.uri().path())
            })
            .flatten()
            .map(|params| (registered, params))
    });
...
async fn resolve_scoped_plugin_session(
    ...
    Ok(Some(MatchedPluginSession {
        session_id: session.id.to_string(),
        directory: session.directory,
    }))
}

fn snapshot_matches_session_route(
    ...
    snapshot.runtime_routes.values().any(|registered| {
        ...
        .values()
        .any(|registered| {
            registered.route.descriptor.scope
...
impl neoism_agent_plugin_api::PluginWebSocket for HostWebSocket {
    fn receive<'a>(
        ...
        'a,
        Option<neoism_agent_plugin_api::WebSocketMessage>,
    > {
        Box::pin(async move {
            use neoism_agent_plugin_api::WebSocketMessage as PluginMessage;
            Ok(match self.0.recv().await {
                Some(Ok(Message::Text(value))) => {
                    Some(PluginMessage::Text(value.to_string()))
                }
                Some(Ok(Message::Binary(value))) => {
                    Some(PluginMessage::Binary(value.to_vec()))
                }
                Some(Ok(Message::Ping(value))) => {
                    Some(PluginMessage::Ping(value.to_vec()))
                }
                Some(Ok(Message::Pong(value))) => {
                    Some(PluginMessage::Pong(value.to_vec()))
                }
    ...
    fn send<'a>(
        &'a mut self,
        message: neoism_agent_plugin_api::WebSocketMessage,
    ) -> neoism_agent_plugin_api::PluginFuture<'a, ()> {
        Box::pin(async move {
            use neoism_agent_plugin_api::WebSocketMessage as PluginMessage;
            ...
            self.0.send(message).await.map_err(|error| {
...
async fn dispatch_websocket_route(
    ...
    if generation_lease.ensure_active().is_err() {
        return auth_error(
            StatusCode::GONE,
            "plugin.generation_closed",
            "The plugin generation is shut down",
    ...
    if let Err(response) = validate_plugin_route_scope(
...
async fn dispatch_runtime_route(
    ...
    if generation.ensure_active().is_err() {
        return auth_error(
            StatusCode::GONE,
            "plugin.generation_closed",
            "The plugin generation is shut down",
    ...
    let workspace_id = claims
        .and_then(|claims| claims.workspace_id.clone())
        .or_else(|| {
...
async fn validate_plugin_route_scope(
    ...
    match descriptor.scope {
        ...
        neoism_agent_plugin_api::RouteScope::Session => {
            let session_id = path.get("session_id").ok_or_else(|| {
                auth_error(
                    StatusCode::BAD_REQUEST,
            ...
            match state.inner.store.get_session(session_id).await {
                Ok(Some(session))
                    if crate::workspace_runtime::canonical_location(
                        &session.directory,
                    ) == crate::workspace_runtime::canonical_location(directory) =>
                {
                    Ok(())
                }
                Ok(Some(_)) => Err(auth_error(
                    StatusCode::FORBIDDEN,
                    "plugin.session_workspace_mismatch",
                    "The session does not belong to this workspace",
                )),
                Ok(None) => Err(auth_error(
                    StatusCode::NOT_FOUND,
                    "plugin.session_not_found",
                    "Session not found",
                )),
                Err(error) => Err(auth_error(
                    StatusCode::INTERNAL_SERVER_ERROR,
                    "plugin.session_lookup_failed",
                    &error.to_string(),
                )),
            }
        }
...
async fn authenticate_request(
    ...
    if let Some(claims) = claims {
        ...
        let mut owned_session = if plugin_path {
            matched_plugin_session
                .as_ref()
                .map(|matched| matched.session_id.clone())
                .or(query_session_id)
        } else {
            session_id_from_path(request.uri().path())
                .map(str::to_string)
                .or(query_session_id)
        };
            ...
            && requires_directory_scope(request.uri().path())
        {
            return auth_error(
                ...
                "This hosted route requires an authorized session or directory scope",
            );
        }
        if let Some(matched) = matched_plugin_session {
    ...
    if let Some(tenant_id) = audit_tenant {
        let entry = neoism_agent_core::AuditEntry {
            id: neoism_agent_core::Id::ascending(neoism_agent_core::IdKind::Audit)
                .to_string(),
            tenant_id,
...
fn is_mcp_oauth_callback_get(request: &Request<Body>) -> bool {
    request.method() == Method::GET
        && request
            .uri()
            .path()
            .starts_with("/v2/plugins/dev.neoism.mcp/")
...
mod hosted_plugin_authorization_tests {
    ...
    impl neoism_agent_plugin_api::RouteHandler for NoopRoute {
        fn handle<'a>(
            &'a self,
            _: neoism_agent_plugin_api::RouteRequest,
        ) -> neoism_agent_plugin_api::PluginFuture<
            'a,
            neoism_agent_plugin_api::RouteResponse,
        > {
            Box::pin(async {
                Ok(neoism_agent_plugin_api::RouteResponse::json(
                    200,
    ...
    fn route(
        id: &str,
        path: &str,
        scope: neoism_agent_plugin_api::RouteScope,
        ...
        neoism_agent_plugin_api::RegisteredRouteContribution {
            ...
            route: neoism_agent_plugin_api::RouteContribution {
                metadata: neoism_agent_plugin_api::ContributionMetadata::new(
                    id,
                    "dev.example.route-auth",
                    neoism_agent_plugin_api::PluginScope::Workspace,
                ),
                descriptor: neoism_agent_plugin_api::RouteDescriptor {
                    id: id.into(),
                    method: neoism_agent_plugin_api::RouteMethod::Get,
                    path: path.into(),
                    scope,
                    request_schema: None,
    ...
    fn hosted_plugin_session_fallback_requires_a_matched_session_descriptor() {
        ...
        snapshot.runtime_routes.insert(
            "workspace".into(),
            route(
                "workspace",
                "/v2/plugins/dev.example.route-auth/:resource_id",
                neoism_agent_plugin_api::RouteScope::Workspace,
            ),
        );
        assert!(!snapshot_matches_session_route(
            &snapshot,
            "GET",
            "/v2/plugins/dev.example.route-auth/ses_fake",
            "ses_fake"
        ));
        snapshot.runtime_routes.insert(
            "session".into(),
            route(
                "session",
                "/v2/plugins/dev.example.route-auth/sessions/:session_id",
                neoism_agent_plugin_api::RouteScope::Session,
            ),
        );
        assert!(snapshot_matches_session_route(
            &snapshot,
            "GET",
            "/v2/plugins/dev.example.route-auth/sessions/ses_real",
            "ses_real"
        ));
    }
    ...
    fn scoped_claims(directory: String, tenant_id: &str) -> crate::caller::CallerClaims {
        crate::caller::CallerClaims {
            ...
            directory_prefixes: vec![directory],
            hosted: false,
            max_sessions: None,
            max_artifacts: None,
    ...
    async fn session_route_fixture(
        ...
        let session = neoism_agent_core::SessionInfo {
            ...
            slug: "route-auth".into(),
            project_id: "global".into(),
            workspace_id: None,
            directory: root.to_string_lossy().into_owned(),
            path: None,
            parent_id: None,
            title: "Route auth".into(),
            agent: None,
            model: None,
    ...
    fn session_id_from_path_never_guesses_on_plugin_routes() {
        assert_eq!(
            session_id_from_path("/v2/plugins/dev.neoism.goals/ses_123"),
            None
        );
        assert_eq!(
        ...
        assert_eq!(
            session_id_from_path(
                "/v2/plugins/dev.neoism.subagents/sessions/ses_123/tasks"
            ),
            None
        );
        assert_eq!(
    ...
    async fn denied_scoped_non_hosted_credential_cannot_use_session_plugin_route() {
        ...
        let response = resolve_scoped_plugin_session(&state, &path, "GET", &denied)
            .await
            .unwrap_err();
    ...
    async fn allowed_scoped_non_hosted_credential_resolves_session_plugin_route() {
        ...
        let matched = resolve_scoped_plugin_session(&state, &path, "GET", &allowed)
            .await
            .unwrap()
            .unwrap();
        assert_eq!(matched.session_id, session.id.to_string());
        assert_eq!(
            crate::workspace_runtime::canonical_location(&matched.directory),
            crate::workspace_runtime::canonical_location(&session.directory)
        );
        state.shutdown().await.unwrap();
        let _ = std::fs::remove_dir_all(root);
    ...
    async fn shared_parent_authorizes_legacy_child_without_tenant_marker() {
        ...
        assert!(
            !allows_session_or_ancestor(&state, &wrong_workspace, &child)
                .await
                .unwrap()
        );
