...
fn splash_wordmark_z_index(occlusion_rects: &[[f32; 4]]) -> i32 {
    if occlusion_rects.is_empty() {
        // Keep the wordmark in the above-quad image pass on an unobstructed
        // splash, then let the UI text pass paint its labels over it. If a
        // modal is present, move the image below the quad pass so the modal
