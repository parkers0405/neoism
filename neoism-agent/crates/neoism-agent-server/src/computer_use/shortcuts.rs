...
pub(super) fn resolve(keys: &[Key]) -> anyhow::Result<Vec<NativeKey>> {
    #[cfg(target_os = "linux")]
    {
        ...
        keys.iter()
            .map(|key| linux::resolve_key(&map, group, *key).map(NativeKey::Raw))
            .collect()
    }
    #[cfg(target_os = "macos")]
    {
        keys.iter()
            .map(|key| macos::resolve_key(*key).map(NativeKey::Raw))
            .collect()
    }
    #[cfg(target_os = "windows")]
    {
        ...
        // keys are also fixed VKs, never Enigo's Unicode fallback.
            ...
            keys.iter().all(|k| !matches!(k, Key::Unicode(_))),
            "Unicode is not a Windows shortcut key"
        );
...
pub(super) fn read_map(
    file: &std::fs::File,
    size: u32,
) -> anyhow::Result<xkbcommon::xkb::Keymap> {
    linux::read_keymap(file, size)
}
#[cfg(target_os = "linux")]
pub(super) fn read_map_source(file: &std::fs::File, size: u32) -> anyhow::Result<String> {
    linux::read_keymap_source(file, size)
}
#[cfg(target_os = "linux")]
pub(super) fn resolve_in_map(
    map: &xkbcommon::xkb::Keymap,
    group: u32,
    keys: &[Key],
) -> anyhow::Result<Vec<u16>> {
    keys.iter()
        .map(|key| linux::resolve_key(map, group, *key))
        .collect()
}

struct Held<'a, K: Keyboard> {
    input: &'a mut K,
    keys: Vec<NativeKey>,
}
    ...
    fn release(&mut self) -> anyhow::Result<()> {
        ...
        for key in self.keys.drain(..).rev() {
            match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                key.send(self.input, Direction::Release)
            })) {
                Ok(Ok(())) => {}
                Ok(Err(e)) => error = Some(e),
                Err(_) => {
                    error = Some(anyhow::anyhow!(
                        "Native key release panicked; remaining releases still attempted"
                    ))
                }
            }
        }
...
mod linux {
            ...
            match event {
                wl_keyboard::Event::Keymap { format, fd, size } => {
                    let result = (|| -> anyhow::Result<xkb::Keymap> {
                            ...
                            "Unsupported Wayland keymap format"
                        );
    ...
    pub(super) fn read_keymap_source(
        file: &std::fs::File,
        size: u32,
    ) -> anyhow::Result<String> {
        ensure!(
            size > 0 && size <= 4 * 1024 * 1024,
            "Invalid/oversized Wayland keymap"
        );
        ...
        file.read_exact_at(&mut bytes, 0)
            .context("Truncated/unreadable Wayland keymap FD")?;
        while bytes.last() == Some(&0) {
            bytes.pop();
        }
        ensure!(
            !bytes.is_empty() && !bytes.contains(&0),
            "Empty or embedded-NUL Wayland keymap"
        );
        String::from_utf8(bytes).context("Wayland keymap is not UTF-8")
    }
    pub(super) fn read_keymap(
        file: &std::fs::File,
        size: u32,
    ) -> anyhow::Result<xkb::Keymap> {
    ...
    pub(super) fn current_keymap() -> anyhow::Result<(xkb::Keymap, u32)> {
        ...
        if let Some(error) = probe.error {
            ...
            probe.seats == 1,
            "Cannot safely select a shortcut keyboard on a multi-seat desktop"
        );
        let map = probe
            .map
            .context("No compositor keyboard keymap; cannot resolve shortcuts")?;
        ...
        let group = match probe.group {
            ...
            None => anyhow::bail!(
                "Compositor did not provide the active keyboard layout group"
            ),
        };
    ...
    pub(super) fn resolve_key(
        ...
        for code in
            map.min_keycode().raw().max(8)..=map.max_keycode().raw().min(u16::MAX as u32)
        {
            if map
    ...
    mod tests {
        ...
        #[derive(Default)]
        struct KeyboardLog {
            events: Vec<(u16, Direction)>,
        }
        impl Keyboard for KeyboardLog {
            fn fast_text(&mut self, _: &str) -> enigo::InputResult<Option<()>> {
                panic!("shortcuts must not inject text")
            }
            fn key(&mut self, _: Key, _: Direction) -> enigo::InputResult<()> {
                panic!("shortcuts must not re-resolve symbols after modifiers")
            }
        ...
        fn chord_press_release_codes_are_identical_and_cleanup_survives_cancellation() {
            let map = keymap("us");
            for keys in [
                vec![Key::Shift, Key::Unicode('a')],
                vec![Key::Shift, Key::Unicode('1')],
                vec![Key::Control, Key::Shift, Key::Unicode('a')],
            ] {
                let codes = keys
                    .iter()
                    .map(|k| resolve_key(&map, 0, *k).unwrap())
                    .collect::<Vec<_>>();
                let native = codes
                    .iter()
                    .copied()
                ...
                let expected = codes
                    .iter()
                    .map(|c| (*c, Direction::Press))
                    .chain(codes.iter().rev().map(|c| (*c, Direction::Release)))
                    .collect::<Vec<_>>();
        ...
        #[test]
        fn release_panic_does_not_skip_remaining_modifier_releases() {
            ...
            impl Keyboard for Panics {
                ...
                fn raw(
                    ...
                    if code == 2 {
                        panic!("partial native key event");
                    }
                    Ok(())
                }
            }
            ...
            assert!(
                std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| send(
                    &mut input,
                    &[NativeKey::Raw(1), NativeKey::Raw(2)],
                    || Ok(())
                )))
                .is_err()
            );
        ...
        fn keymap_file(bytes: &[u8]) -> std::fs::File {
            ...
            let path = std::env::temp_dir()
                .join(format!("neoism-keymap-{:032x}", rand::random::<u128>()));
        ...
        fn keymap_fd_at_eof_parses_without_changing_shared_offset() {
            ...
            for nul in [false, true] {
                ...
                assert!(
                    old_read.is_empty(),
                    "old offset-relative reader sees EOF and fails XKB parsing"
                );
                for offset in [bytes.len() as u64, 17] {
                    file.seek(SeekFrom::Start(offset)).unwrap();
                    let map = read_keymap(&file.try_clone().unwrap(), bytes.len() as u32)
                        .unwrap();
                    assert_eq!(
                        file.stream_position().unwrap(),
                        offset,
                        "pread must not alter the compositor's shared cursor"
                    );
        ...
        fn replacement_keymap_clears_old_error_without_using_stale_maps() {
            ...
            let source = keymap("us").get_as_string(xkb::KEYMAP_FORMAT_TEXT_V1);
            probe.record_keymap(read_keymap(
                &keymap_file(source.as_bytes()),
                source.len() as u32,
            ));
        ...
        fn live_wayland_keymap_read_only() {
            ...
            println!(
                "Parsed compositor keymap: {} layouts, group {group}",
                map.num_layouts()
            );
            for key in [
                Key::Return,
        ...
        fn shifted_chords_use_original_keycodes_and_symbols() {
            let map = keymap("us");
            for (letter, control, expected) in
                [('a', false, 'A'), ('1', false, '!'), ('a', true, 'A')]
            {
                ...
                if control {
                    state.update_key(
                        xkb::Keycode::new(
                            resolve_key(&map, 0, Key::Control).unwrap().into(),
                        ),
                        xkb::KeyDirection::Down,
                    );
                }
                state
                    .update_key(xkb::Keycode::new(shift.into()), xkb::KeyDirection::Down);
                assert_eq!(
                    state.key_get_one_sym(xkb::Keycode::new(key.into())),
                    xkb::utf32_to_keysym(expected as u32)
                );
        ...
        fn non_us_layout_is_resolved_not_assumed() {
            ...
            let de = keymap("de");
            assert_ne!(
                resolve_key(&us, 0, Key::Unicode('y')).unwrap(),
                resolve_key(&de, 0, Key::Unicode('y')).unwrap()
            );
...
mod macos {
    ...
    pub(super) fn resolve_key(key: Key) -> anyhow::Result<u16> {
        let Key::Unicode(ch) = key else {
            return u16::try_from(key)
                .map_err(|_| anyhow::anyhow!("Unsupported native macOS key"));
        };
        let source = Source(unsafe { TISCopyCurrentKeyboardLayoutInputSource() });
        ensure!(!source.0.is_null(), "No macOS keyboard layout");
        let data = unsafe {
            TISGetInputSourceProperty(source.0, kTISPropertyUnicodeKeyLayoutData)
        };
        ensure!(
            !data.is_null(),
            "No Unicode layout data; cannot resolve shortcut"
        );
        ...
        for code in 0..128 {
            ...
            if status == 0 && actual == 1 && u32::from(text[0]) == ch as u32 {
                return Ok(code);
            }
        }
