...
fn guard() -> Result<()> {
        ...
        std::env::var("XDG_RUNTIME_DIR")? == std::env::var("NEOISM_BROWSER_RUNTIME")?
    );
    ensure!(!std::path::Path::new("/mnt/wayland").exists());
    ensure!(!std::path::Path::new("/dev/input").exists());
    static TARGET: std::sync::Mutex<Option<super::windows::Window>> =
        std::sync::Mutex::new(None);
    let target = {
        let mut cached = TARGET
            .lock()
            .map_err(|_| anyhow::anyhow!("Browser guard poisoned"))?;
        if cached.is_none() {
            let active = hypr("activewindow")?;
            let pid: i64 = std::env::var("NEOISM_BROWSER_PID")?.parse()?;
            ensure!(
                active["pid"].as_i64() == Some(pid) && active["xwayland"] == false,
                "Wrong private browser: {active}"
            );
            let listing = super::windows::list()?;
            let token = listing["windows"]
                .as_array()
                .context("Native window list")?
                .iter()
                .find(|entry| {
                    entry["window"]["pid"].as_i64() == Some(pid)
                        && entry["window"]["app"] == "neoism-browser-regression"
                })
                .and_then(|entry| entry["target"].as_str())
                .context("Private browser target")?;
...
fn hypr(query: &str) -> Result<Value> {
    ...
    let output = Command::new("hyprctl")
        .args(["-i", &signature, "-j", query])
        .output()?;
    ensure!(output.status.success());
    Ok(serde_json::from_slice(&output.stdout)?)
}
fn report() -> Result<Value> {
    Ok(serde_json::from_slice(&fs::read(std::env::var(
        "NEOISM_BROWSER_REPORT",
    )?)?)?)
}
fn wait_for(label: &str, predicate: impl Fn(&Value) -> bool) -> Result<Value> {
    ...
    loop {
        if let Ok(value) = report() {
            if predicate(&value) {
                println!(
                    "ACK {label}: active={} values={} events={}",
                    value["activeElement"],
                    value["values"],
            ...
            "No browser ACK for {label}"
        );
        std::thread::sleep(Duration::from_millis(25));
    }
}
fn keys(keys: &[Key]) -> Result<()> {
    guard()?;
    super::linux_text::send_keys(keys, guard)
}
fn typed(
    content: &str,
    method: super::typing::Method,
    policy: super::typing::ClipboardPolicy,
    consent: bool,
) -> Result<super::typing::TextEffects> {
    use super::typing;
    let started = Instant::now();
    let mut checked = || {
        ensure!(
            started.elapsed() < Duration::from_secs(10),
            "Unified typing exceeded the public single-call budget"
        );
        guard()
    };
    let mut waiting = || {
    ...
    let plan = typing::prepare_text(
        ...
        false,
        consent.then(typing::ClipboardPermit::granted),
        &mut waiting,
    )?;
    let mut effects = None;
    typing::execute_text_with_wait(
        &mut session,
        &plan,
        &mut checked,
        &mut waiting,
        &mut |value| effects = Some(value.clone()),
    )?;
    session.finish()?;
    let effects = effects.context("Missing typing dispatch report")?;
    ensure!(
        effects.phase == "complete" && !effects.current_unit_uncertain,
        "Incomplete typing dispatch: {effects:?}"
    );
    println!(
        "TYPING METHOD: {} clipboard={} dispatched_units={}",
        effects.method, effects.clipboard, effects.completed_native_units
    );
    Ok(effects)
}
fn text(content: &str) -> Result<()> {
    let effects = typed(
        content,
        super::typing::Method::Auto,
        super::typing::ClipboardPolicy::Forbid,
        false,
    )?;
    ensure!(
        effects.method == "keyboard" && effects.clipboard == "unchanged",
        "Representable typing must not use clipboard: {effects:?}"
    );
    ensure!(
        effects.completed_native_units == content.chars().count(),
        "Incomplete native scalar dispatch"
    );
    Ok(())
}
fn value_is(report: &Value, id: &str, value: &str) -> bool {
    report["focused"] == true
        && report["activeElement"] == id
        && report["values"][id] == value
}
...
fn click(field: &str) -> Result<()> {
    use super::linux_pointer::{Command as PointerCommand, Point};
    guard()?;
    let before = report()?;
    let output = Command::new("python3")
        .args([
            &std::env::var("NEOISM_BROWSER_LAUNCHER")?,
            "--observe-pointer",
            &std::env::var("NEOISM_BROWSER_RUN")?,
            field,
        ])
        .output()?;
    ensure!(
        output.status.success(),
        "Screenshot observation failed: {}",
    ...
    let display = super::Display {
        id: d["id"].as_str().context("display name")?.into(),
        x: d["x"].as_i64().context("display x")? as i32,
        y: d["y"].as_i64().context("display y")? as i32,
        width: d["width"].as_u64().context("logical width")? as u32,
        height: d["height"].as_u64().context("logical height")? as u32,
    };
    let p = &observed["point"];
    let point = Point {
        x: p["x"].as_u64().context("pixel x")? as u32,
        y: p["y"].as_u64().context("pixel y")? as u32,
        width: p["width"].as_u64().context("screenshot width")? as u32,
        height: p["height"].as_u64().context("screenshot height")? as u32,
    };
    guard()?;
    ...
    let after = wait_for("native pointerdown and focus", |r| {
        r["pointerCount"].as_u64().unwrap_or(0) > serial && r["activeElement"] == field
    })?;
    ensure!(
        after["values"] == before["values"],
        "Click unexpectedly changed a field"
    );
    let event = after["events"]
        .as_array()
        .context("events")?
        .iter()
        .rev()
        .find(|e| e["type"] == "pointerdown")
        .context("pointerdown")?;
    ensure!(
        event["target"] == field,
        "Native click hit wrong target: {event}"
    );
    let rect = &before["rects"][field];
    let center = |axis: &str, extent: &str| -> Result<f64> {
        Ok(rect[axis].as_f64().context("rect axis")?
            + rect[extent].as_f64().context("rect extent")?
                * if axis == "y" { 0.75 } else { 0.5 })
    };
...
fn clear(field: &str) -> Result<()> {
    ...
    wait_for("shortcut clear", |r| {
        if field == "editable" {
            r["focused"] == true
                && r["activeElement"] == field
                && r["editableTextContent"] == ""
        } else {
            value_is(r, field, "")
        }
    })?;
...
fn paste(field: &str, content: &str) -> Result<()> {
    let effects = typed(
        content,
        super::typing::Method::Paste,
        super::typing::ClipboardPolicy::Replace,
        true,
    )?;
    ensure!(
        effects.method == "paste" && effects.clipboard == "changed",
        "Explicit paste method/effects missing: {effects:?}"
    );
    wait_for("explicit facade clipboard paste exact", |r| {
        value_is(r, field, content)
    })?;
    Ok(())
}
#[test]
...
fn hyprland_production_browser_roundtrip() -> Result<()> {
    ...
    let monitors = hypr("monitors")?;
        ...
        monitors.as_array().is_some_and(|m| m.len() == 1)
            && monitors[0]["scale"]
                .as_f64()
                .is_some_and(|s| (s - expected_scale).abs() < 0.001),
        "Requested output scale not active: {monitors}"
    );
    wait_for("browser-local ready", |r| {
    ...
    if let Ok(path) = std::env::var("NEOISM_BROWSER_SCREENSHOT") {
            ...
            Command::new("grim").arg(path).status()?.success(),
            "Private screenshot failed"
        );
    }
    let previous = report()?;
    ...
    let navigated = wait_for("native omnibox URL + Return navigation", |r| {
        r["generation"].as_str().is_some()
            && r["generation"] != previous["generation"]
            && r["href"] == local_url
            && r["query"] == query
            && r["ready"] == "complete"
            && r["focused"] == true
            && r["activeElement"] == "input"
    })?;
    ...
    let count = requests
        ...
        count == 1,
        "Expected exactly one local server navigation to {target}, observed {count}"
    );
    ...
    text(url)?;
    wait_for("Action::Text URL exact delivery", |r| {
        value_is(r, "input", url)
    })?;
    clear("input")?;
    let maximum = "A".repeat(512);
    text(&maximum)?;
    wait_for(
        "maximum-length shifted typing within public call budget",
        |r| value_is(r, "input", &maximum),
    )?;
    clear("input")?;
    ...
    for zoom in [100, 125] {
        if zoom == 125 {
            ...
            wait_for("125 percent browser zoom", |r| {
                r["viewport"]["dpr"]
                    .as_f64()
                    .is_some_and(|d| (d - baseline * 1.25).abs() < 0.02)
            })?;
        }
        ...
        for field in ["input", "textarea", "editable"] {
            ...
            clear(field)?;
            text(&keyboard)?;
            wait_for("original-layout ASCII punctuation and Shift exact", |r| {
                value_is(r, field, &keyboard)
            })?;
            let before = report()?;
            // An unsupported suffix must fail before typing its valid prefix.
            let invalid = format!("{keyboard}🦀");
            let error =
                text(&invalid).expect_err("unsupported text must reject before input");
            ...
            let after = fresh("unsupported typing rejection observed", &before)?;
                ...
                "Rejected text partially changed browser fields: before={} after={}",
                before["values"],
                after["values"]
            );
            let error = typed(
            ...
            .expect_err("model eligibility must not authorize clipboard publication");
                ...
                format!("{error:#}").contains("computer_clipboard"),
                "Wrong missing-consent error: {error:#}"
            );
            let after = fresh("missing clipboard consent observed", &after)?;
            ensure!(
                after["values"] == before["values"]
                    && after["inputCount"] == before["inputCount"],
                "Missing-consent request changed browser input"
            );
            clear(field)?;
            let effects = typed(
                full,
                ...
                effects.method == "paste" && effects.clipboard == "changed",
                "Auto fallback selected wrong method: {effects:?}"
            );
            wait_for("explicitly authorized auto fallback exact Unicode", |r| {
                value_is(r, field, full)
            })?;
            clear(field)?;
            let full = if field == "input" {
                full.to_owned()
            } else {
            ...
            clear(field)?;
            keys(&[Key::Unicode('x')])?;
            wait_for("post-paste shortcut and native key", |r| {
                value_is(r, field, "x")
            })?;
            if field == "textarea" {
                keys(&[Key::Return])?;
                wait_for("explicit Return inserts newline", |r| {
                    value_is(r, field, "x\n")
                })?;
            }
        }
