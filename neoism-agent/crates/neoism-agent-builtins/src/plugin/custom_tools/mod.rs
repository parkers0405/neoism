...
impl CustomToolsPlugin {
    pub fn new(host: Arc<dyn CustomToolsHost>) -> Self {
        Self(host)
    }
}
...
impl PluginDefinition for CustomToolsPlugin {
    fn manifest(&self) -> PluginManifest {
        PluginManifest {
            id: ID.into(),
            name: "Workspace custom tools".into(),
            version: env!("CARGO_PKG_VERSION").into(),
            internal: true,
            disableable: true,
            capabilities: Vec::new(),
            requires: vec![super::workspace_tools::ID.into()],
    ...
    fn required_capabilities(&self) -> Vec<neoism_agent_plugin_api::HostCapability> {
        ...
        vec![WorkspaceRead, WorkspaceWrite, ProcessSpawn, Network]
    }
    fn contributions(
        &self,
