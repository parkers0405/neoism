...
impl PluginDefinition for SubagentsPlugin {
    fn manifest(&self) -> PluginManifest {
        PluginManifest {
            ...
            internal: true, disableable: true, capabilities: vec!["neoism.subagents".into()],
            requires: vec![super::agents::ID.into()], event_namespaces: vec!["subagent".into()],
    ...
    fn contributions(&self, registrar: &mut PluginContributions) -> Result<(), PluginHostError> {
        ...
        for (id, suffix, method, action) in [
            ("v2.subagents.tasks.list", "tasks", RouteMethod::Get, SubagentAction::List),
            ("v2.subagents.tasks.stop", "stop", RouteMethod::Post, SubagentAction::Stop),
        ] {
            registrar.runtime_route(RouteContribution {
                descriptor: RouteDescriptor {
                    id: id.into(), method,
                    path: format!("/v2/plugins/{ID}/sessions/:session_id/{suffix}"),
                    scope: RouteScope::Workspace, request_schema: None, response_schema: None,
                },
                metadata: ContributionMetadata::new(id, ID, PluginScope::Workspace),
                handler: Arc::new(SubagentRoute { host: self.host.clone(), action }),
            });
        }
        Ok(())
    }
...
struct SubagentRoute { host: Arc<dyn SubagentsHost>, action: SubagentAction }
